...
set path_exe="D:\Program Files (x86)\Miner and Miner\PG&E Custom Components\CWOSL"
set file_name_exe=PGE.BatchApplication.CWOSL.exe
...
echo %date_log% %time_log%  >> %file_log_tmp% 2>&1
set command=call %file_exe% POST
echo Executing %command%... >> %file_log_tmp% 2>&1
               %command%    >> %file_log_tmp% 2>&1
set /A error_level=%error_level% + %ERRORLEVEL%
echo Executed  %command%.   >> %file_log_tmp% 2>&1
...
echo Exiting %filename_current% with error_level = %error_level% >> %file_log% 2>&1
exit /b %error_level%
rem ===========================================================================
