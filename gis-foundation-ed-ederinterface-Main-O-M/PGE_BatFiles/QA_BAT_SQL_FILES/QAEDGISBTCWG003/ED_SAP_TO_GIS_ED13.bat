echo off
rem ===========================================================================
rem EDG_SAP_to_GIS_ED13.bat
rem ---------------------------------------------------------------------------
rem This script runs an executable that transfers data from SAP to GIS.
rem Records will be written to the table WEBR.PGE_PMORDER.
rem 
rem Change default variable values below to match your:
rem   server_name
rem   path_log to log files
rem 
rem Open a command window.
rem Navigate to script location.
rem Usage: EDG_SAP_to_GIS_ED13.bat
...
set path_exe="D:\Program Files (x86)\Miner and Miner\PG&E Custom Components\SAP PMOrder synch"
set file_name_exe=PGE.Interfaces.SAP.PMOrder.exe
set file_exe=%path_exe%\%file_name_exe%
set file_name_exe_log=PGE.Interfaces.SAP.PMOrder.log
set file_exe_log=%path_exe%\%file_name_exe_log%
...
rem ===========================================================================
rem Append the executable log file content to the batch script log file.
rem ---------------------------------------------------------------------------

echo Appending interface log file content to the UC4 batch file log file... >> %file_log_tmp% 2>&1
type %file_exe_log%                                                         >> %file_log_tmp% 2>&1
echo Appended  interface log file content to the UC4 batch file log file... >> %file_log_tmp% 2>&1
