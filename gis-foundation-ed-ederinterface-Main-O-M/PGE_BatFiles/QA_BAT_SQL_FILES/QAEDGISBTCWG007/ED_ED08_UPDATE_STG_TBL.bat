...
set SQLScript_path1=D:\ED08\ED008_PopulateStaging.sql
...
echo Executing ED008_PopulateStaging Process on %Database%.....  >> %file_log% 2>&1



sqlplus %uname%/%PASSWORD%@%Database% @%SQLScript_path1% >> %file_log% 2>&1
echo %ERRORLEVEL%                                           >> %file_log% 2>&1

set /A error_level=%error_level% + %ERRORLEVEL%             >> %file_log% 2>&1

echo Completed executing ED008_PopulateStaging Data Processing on %Database%>> %file_log% 2>&1
