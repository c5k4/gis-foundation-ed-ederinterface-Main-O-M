exit 0
echo off
rem Script Name: EDG_SCHM_Kill_Schm_DC2_01.bat
...
rem for %%i in (%process_list%) do (
	...
	echo Executing !KillSchematics!...                             >> %file_log% 2>&1
	call !KillSchematics!                                          >> %file_log% 2>&1
    if !ERRORLEVEL! neq -1 set /A error_level=!error_level! + !ERRORLEVEL!
	echo Executed  !KillSchematics!.                               >> %file_log% 2>&1
	timeout 5
)
