echo off
rem ===========================================================================
rem Script Name: ED_ED08_MERGE_FEATURE_CLASS.bat
...
set SQLScript_path=D:\ED08\createED08Sequence.sql
...
echo Executing createED08Sequence Process on %Database%.....  >> %file_log% 2>&1

sqlplus  -s %uname%/%PASSWORD%@%Database% @%SQLScript_path% >> %file_log% 2>&1
set /A error_level=%error_level% + %ERRORLEVEL%    
echo Error Level: %error_level%                             >> %file_log% 2>&1         

echo Completed executing createED08Sequence Data Processing on %Database%>> %file_log% 2>&1

set Database1=EDER_QA

set uname1= PGEDATA

set SQLScript_path1=D:\ED08\ED08PrepTempTables.sql
for /f %%a in ('PGE_DBPasswordManagement.exe GETPASSWORD PGEDATA@EDER') do set "PASSWORD1=%%a"


echo Executing ED08PrepTempTables Process on %Database1%.....  >> %file_log% 2>&1

sqlplus  -s %uname1%/%PASSWORD1%@%Database1% @%SQLScript_path1% >> %file_log% 2>&1
set /A error_level=%error_level% + %ERRORLEVEL%  
echo Error Level: %error_level%                             >> %file_log% 2>&1           

echo Completed executing ED08PrepTempTables Data Processing on %Database%>> %file_log% 2>&1
