...
echo off
rem ===========================================================================
rem Script Name: ED_GISSUB_TRACE_TBL_COPY_GMC_DC2A.bat
...
set sqlDB=EDGMC_QA_DC2A

@echo off

for /f %%a in ('PGE_DBPasswordManagement.exe GETPASSWORD uc4admin@EDGMC_QA_DC2A') do set "sqlPWD=%%a"
...
sqlplus -s %sqlUSR%/%sqlPWD%@%sqlDB% @"D:\edgisdbmaint\Copy_trace_tbl_edgmc_qa_dc2a1.sql"          >> %file_log% 2>&1
set /A error_level=%error_level% + %ERRORLEVEL%
sqlplus -s %sqlUSR%/%sqlPWD%@%sqlDB% @"D:\edgisdbmaint\Copy_trace_tbl_edgmc_qa_dc2a2.sql"          >> %file_log% 2>&1
set /A error_level=%error_level% + %ERRORLEVEL%
sqlplus -s %sqlUSR%/%sqlPWD%@%sqlDB% @"D:\edgisdbmaint\Copy_trace_tbl_edgmc_qa_dc2a3.sql"          >> %file_log% 2>&1
set /A error_level=%error_level% + %ERRORLEVEL%
sqlplus -s %sqlUSR%/%sqlPWD%@%sqlDB% @"D:\edgisdbmaint\Copy_trace_tbl_edgmc_qa_dc2a4.sql"          >> %file_log% 2>&1
set /A error_level=%error_level% + %ERRORLEVEL%
