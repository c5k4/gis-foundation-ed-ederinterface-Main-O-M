echo off
rem ===========================================================================
rem Script Name: ED_GIS_TRACE_TBL_COPY_GMC_DC1A.bat
...
for /f %%a in ('PGE_DBPasswordManagement.exe GETPASSWORD uc4admin@EDGMC_QA') do set "sqlPWD=%%a"
...
sqlplus -s %sqlUSR%/%sqlPWD%@%sqlDB% @"D:\edgisdbmaint\Copy_trace_tbl_edgmc_qa_dc1a1.sql"          >> %file_log% 2>&1
set /A error_level=%error_level% + %ERRORLEVEL%
sqlplus -s %sqlUSR%/%sqlPWD%@%sqlDB% @"D:\edgisdbmaint\Copy_trace_tbl_edgmc_qa_dc1a2.sql"          >> %file_log% 2>&1
set /A error_level=%error_level% + %ERRORLEVEL%
sqlplus -s %sqlUSR%/%sqlPWD%@%sqlDB% @"D:\edgisdbmaint\Copy_trace_tbl_edgmc_qa_dc1a3.sql"          >> %file_log% 2>&1
set /A error_level=%error_level% + %ERRORLEVEL%
sqlplus -s %sqlUSR%/%sqlPWD%@%sqlDB% @"D:\edgisdbmaint\Copy_trace_tbl_edgmc_qa_dc1a4.sql"          >> %file_log% 2>&1
set /A error_level=%error_level% + %ERRORLEVEL%
