echo off
rem ===========================================================================
rem Script Name:ED_GIS_CUSTOMERINFO_COPY_PUB_DC2A.bat
...
set DbType=EDPUB_QA_DC2A
set EDGIS_Database=EDPUB_QA_DC2A

set uname=UC4ADMIN
for /f %%a in ('PGE_DBPasswordManagement.exe GETPASSWORD UC4ADMIN@EDPUB_QA_DC2A') do set "PWD=%%a"


set SQLScript_path=D:\edgisdbmaint\Copy_customer_data_EDER_to_EDPUBDC2A.sql
...
echo Executing Copy_customer_data_EDER_to_EDPUBDC2A on %EDGIS_Database%.....  >> %file_log% 2>&1
echo %uname%/%PWD%@%EDGIS_Database% @%SQLScript_path%
sqlplus -s  %uname%/%PWD%@%EDGIS_Database% @%SQLScript_path% >> %file_log%  2>&1
rem sqlplus -s %uname%/%PWD%@%EDGIS_Database% @%SQLScript_path% >> %file_log%  2>&1
set /A error_level=%error_level% + %ERRORLEVEL%             


echo Completed executing Copy_customer_data_EDER_to_EDPUBDC2A on %EDGIS_Database% >> %file_log% 2>&1
