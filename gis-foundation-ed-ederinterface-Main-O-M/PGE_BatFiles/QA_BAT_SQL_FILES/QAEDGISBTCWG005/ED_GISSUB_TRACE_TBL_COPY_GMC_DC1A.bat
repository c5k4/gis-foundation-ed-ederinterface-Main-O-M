...
set sqlDB=EDSUBGMC_QA_DC1A

@echo off

for /f %%a in ('PGE_DBPasswordManagement.exe GETPASSWORD uc4admin@EDSUBGMC_QA_DC1A') do set "sqlPWD=%%a"
...
sqlplus -s %sqlUSR%/%sqlPWD%@%sqlDB% @"D:\edgisdbmaint\Copy_trace_tbl_edsubgmc_qa_dc1a.sql"          >> %file_log% 2>&1
