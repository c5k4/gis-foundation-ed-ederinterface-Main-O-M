echo off
rem ===========================================================================
rem Script Name: ED_ED08_UPDATE_INTMDT_TBL.bat
...
set SQLScript_path1=D:\ED08\Update_SpatialJoinOutputTable.sql
for /f %%a in ('PGE_DBPasswordManagement.exe GETPASSWORD EDGISBO@EDER') do set "PASSWORD=%%a"
...
echo Executing Update_SpatialJoinOutputTable Process on %Database%.....  >> %file_log% 2>&1

sqlplus  -s %uname%/%PASSWORD%@%Database% @%SQLScript_path1% >> %file_log% 2>&1
set /A error_level=%error_level% + %ERRORLEVEL%             

echo Completed executing Update_SpatialJoinOutputTable Data Processing on %Database%>> %file_log% 2>&1
