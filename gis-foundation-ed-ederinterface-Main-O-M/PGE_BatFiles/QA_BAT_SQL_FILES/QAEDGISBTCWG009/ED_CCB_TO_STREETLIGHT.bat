...
echo I am here
if not exist "\\rcnas01-smb\edgisrearch-fs01\Streetlight\data_from_rss\SL-3679.csv" (
echo I am here
...
cd /d %emailPath%   
rem D:\Python27\ArcGIS10.8\python send_email.py  "Streetlight Weekly data"  email_msg.txt
call %pythonexe% %emailUtilPath%					   >> %file_log% 2>&1
set /A error_level=%error_level% + %ERRORLEVEL%
echo Error Level: %ERRORLEVEL%  					   >> %file_log% 2>&1
rem -------------------------------------------------------------------------
