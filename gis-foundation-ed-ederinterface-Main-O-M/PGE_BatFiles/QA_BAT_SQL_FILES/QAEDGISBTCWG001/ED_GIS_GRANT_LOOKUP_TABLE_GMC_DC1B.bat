...
set DbType=EDGMC_QA_DC1B
set EDGIS_Database=EDGMC_QA_DC1B

set unames=UC4ADMIN
for /f %%a in ('PGE_DBPasswordManagement.exe GETPASSWORD UC4ADMIN@EDGMC_QA_DC1B') do set "PWDS=%%a"

set SQLScript_path=D:\edgisdbmaint\Grant_Lookup_Table_to_ADMSRO_EDGMCDC1B.sql
...
echo Executing Grant_Lookup_Table_to_ADMSRO_EDGMCDC1B on %DbType%.....  >> %file_log% 2>&1

sqlplus -s %unames%/%PWDS%@%DbType% @%SQLScript_path% >> %file_log%  2>&1
set /A error_level=%error_level% + %ERRORLEVEL%
echo sqlplus -s %unames%/%PWDS%@%DbType% @%SQLScript_path% >> %file_log%  2>&1

echo Completed executing Grant_Lookup_Table_to_ADMSRO_EDGMCDC1B on %DbType% >> %file_log% 2>&1
