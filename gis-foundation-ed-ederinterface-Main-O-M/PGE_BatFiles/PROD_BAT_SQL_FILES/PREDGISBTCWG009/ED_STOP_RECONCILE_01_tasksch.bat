...
set ServiceName1=EDER_RECONCILE_03
set ServiceName1=EDERSUB_RECONCILE_03
...
rem ---------------------------------------------------------------------------
IF EXIST "D:\edgisdbmaint\track_reconcile_stop.txt" (

taskkill /F /IM GeodatabaseManagerServices.exe				>> %file_log% 2>&1
echo Stopping GDBM Services...                              >> %file_log% 2>&1
...
del D:\edgisdbmaint\track_reconcile_stop.txt
) 
set DbType=eder
set EDGIS_Database=EDER

set uname=UC4ADMIN
for /f %%a in ('PGE_DBPasswordManagement.exe GETPASSWORD UC4ADMIN@EDER') do set "UC4ADMIN_PWD=%%a"

set SQLScript_path=D:\edgisdbmaint\ED_RESTRICT_RECONCILE_RESET.sql

echo Executing ED_RESTRICT_RECONCILE_RESET Processing on %EDGIS_Database%.....  >> %file_log% 2>&1

sqlplus -s %uname%/%UC4ADMIN_PWD%@%EDGIS_Database% @%SQLScript_path% >> %file_log%  2>&1
set /A error_level=%error_level% + %ERRORLEVEL%             


echo Completed executing ED_RESTRICT_RECONCILE_RESET Processing on %EDGIS_Database% >> %file_log% 2>&1
