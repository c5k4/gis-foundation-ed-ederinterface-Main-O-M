...
rem ===========================================================================
rem Script Name: ED_GISSUB_TRACE_TBL_COPY_PUB_DC1A.bat
...
set sqlDB=EDSUBPUB_PR_DC1A

@echo off

for /f %%a in ('PGE_DBPasswordManagement.exe GETPASSWORD uc4admin@EDSUBPUB_PR_DC1A') do set "sqlPWD=%%a"

for /f %%a in ('PGE_DBPasswordManagement.exe GETPASSWORD EDGIS@EDERSUB') do set "sqlPWDS=%%a"
...
sqlplus -s %sqlUSR%/%sqlPWD%@%sqlDB% @"D:\edgisdbmaint\Copy_trace_tbl_edsubpub_prod_dc1a.sql" %sqlPWDS%		>> %file_log% 2>&1
