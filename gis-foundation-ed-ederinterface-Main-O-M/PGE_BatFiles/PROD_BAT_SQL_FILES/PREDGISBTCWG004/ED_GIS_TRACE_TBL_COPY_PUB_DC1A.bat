...
set sqlDB=EDPUB_PR_DC1A

rem echo off

echo passowrd
for /f %%a in ('PGE_DBPasswordManagement.exe GETPASSWORD uc4admin@EDPUB_PR_DC1A') do set "sqlPWD=%%a"
...
sqlplus -s %sqlUSR%/%sqlPWD%@%sqlDB% @"D:\edgisdbmaint\Copy_trace_tbl_edpub_prod_dc1a1.sql" %sqlPWDS%	>> %file_log% 2>&1
echo Error level from script 1: %ERRORLEVEL% >> %file_log% 2>&1
set /A error_level=%error_level% + %ERRORLEVEL%
sqlplus -s %sqlUSR%/%sqlPWD%@%sqlDB% @"D:\edgisdbmaint\Copy_trace_tbl_edpub_prod_dc1a2.sql" %sqlPWDS%	>> %file_log% 2>&1
echo Error level from script 2: %ERRORLEVEL% >> %file_log% 2>&1
set /A error_level=%error_level% + %ERRORLEVEL%
sqlplus -s %sqlUSR%/%sqlPWD%@%sqlDB% @"D:\edgisdbmaint\Copy_trace_tbl_edpub_prod_dc1a3.sql" %sqlPWDS%	>> %file_log% 2>&1
echo Error level from script 3: %ERRORLEVEL% >> %file_log% 2>&1
set /A error_level=%error_level% + %ERRORLEVEL%
sqlplus -s %sqlUSR%/%sqlPWD%@%sqlDB% @"D:\edgisdbmaint\Copy_trace_tbl_edpub_prod_dc1a4.sql" %sqlPWDS%	>> %file_log% 2>&1
