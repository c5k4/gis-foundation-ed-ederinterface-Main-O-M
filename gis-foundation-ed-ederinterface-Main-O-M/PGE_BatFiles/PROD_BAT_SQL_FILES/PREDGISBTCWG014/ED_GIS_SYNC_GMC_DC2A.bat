...
rem set pythonexe=D:\Python27\ArcGIS10.8\python.exe
rem if not exist %pythonexe% set pythonexe=D:\Python27\ArcGISx6410.6\python.exe
rem if not exist %pythonexe% set pythonexe=D:\python27\arcgis10.7\python.exe
rem if not exist %pythonexe% set pythonexe=D:\Python27\ArcGISx6410.7\python.exe
rem if not exist %pythonexe% set pythonexe=D:\python27\arcgis10.8\python.exe
rem if not exist %pythonexe% set pythonexe=D:\Python27\ArcGISx6410.8\python.exe
rem if not exist %pythonexe% set pythonexe=D:\python27\arcgis10.9\python.exe
rem if not exist %pythonexe% set pythonexe=D:\Python27\ArcGISx6410.9\python.exe
rem ===========================================================================
...
for /f %%a in ('PGE_DBPasswordManagement.exe GETSDEFILE EDGIS@EDGMC_PR_DC2A') do set "targetsde=%%a"

set replicaname=EDER_DEFAULT_TO_EDGMC
set LOB=EDGMC
set pythonscript=D:\edgisdbmaint\LOB_to_GeoMart_ReplicaSync_Disconnected_Reconcile.py
...
echo ERRORLEVEL returned after making the log directory: %ERRORLEVEL%
...
rem echo ERRORLEVEL returned after python script= %ERRORLEVEL% >> %file_log_tmp% 2>&1
rem set /A error_level=%error_level% + %ERRORLEVEL%
echo Executed  %command%.   >> %file_log_tmp% 2>&1
echo Error level after Synchronization: %error_level% >> %file_log_tmp% 2>&1
echo Error level after Synchronization: %error_level%

rem ===========================================================================
REM ----- This section of code will copy delta fgdb to geomart
rem set export_remote_dir=\\PRETAPPGENWC019\LOB_GeoMart\Replication\export
set export_remote_dir=\\PRGTAPPGENWC010\LOB_GeoMart\Replication\export
IF not exist H:\ (
	echo Mapping %export_remote_dir% to H: >> %file_log_tmp% 2>&1
	CALL NET USE H: %export_remote_dir%
)

set pythonscript1=D:\edgisdbmaint\copygdbtogeomart\copy.py
...
set /A error_level=%error_level% + %ERRORLEVEL%
echo Executed  %command%.   >> %file_log_tmp% 2>&1
echo Error level after copying GDB: %error_level% >> %file_log_tmp% 2>&1
echo Error level after copying GDB: %error_level%

rem CALL NET USE H: /DELETE
rem echo Removed the Mapping %export_remote_dir% from H: >> %file_log_tmp% 2>&1
