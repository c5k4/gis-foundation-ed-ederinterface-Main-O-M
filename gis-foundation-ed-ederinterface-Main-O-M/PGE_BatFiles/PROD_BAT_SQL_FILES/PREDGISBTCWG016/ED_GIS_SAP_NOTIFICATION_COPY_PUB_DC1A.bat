echo off
rem ===========================================================================
rem Script Name: ED_GIS_SAP_NOTIFICATION_COPY_PUB_DC1A.bat
rem Created: June 03, 2022
...
for /f %%a in ('PGE_DBPasswordManagement.exe GETPASSWORD EDGIS@EDPUB_PR') do set "sqlPWDS=%%a"
...
sqlplus -s %sqlUSR%/%sqlPWD%@%sqlDB% @"D:\edgisdbmaint\Copy_SAP_Notifications.sql" %sqlPWDS%	>> %file_log% 2>&1
set /A error_level=%error_level% + %ERRORLEVEL%
