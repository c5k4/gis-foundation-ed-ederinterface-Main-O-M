...
-- Insert data for each table
   for c_schematics in cur_schematics loop
       for c_tables in cur_tables(c_schematics.owner,'SCH'||c_schematics.id||'E_%') loop
           ...
           if length(v_columns) > 0 then
              v_sql:= 'insert into edgis.EDSCHEM_GUIDTOTAL (OWNER,SCH_TABLE,UCID,UOID,UGUID,MINX,MINY,MAXX,MAXY,OID,'||v_columns2||') '||v_str;
              dbms_output.put_line (v_sql);
              execute immediate v_sql;
           else
              execute immediate 'insert into edgis.EDSCHEM_GUIDTOTAL (OWNER,SCH_TABLE,UCID,UOID,UGUID,MINX,MINY,MAXX,MAXY,OID) '||v_str;
           end if;
       end loop;
       commit;
   end loop;
