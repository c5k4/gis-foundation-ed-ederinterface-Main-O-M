...
set inifile=EDPUB_DC2Bconfig.ini
...
for /f %%a in ('PGE_DBPasswordManagement GETPASSWORD SDE@EDPUB_PR_DC2B') do set sqlPWD=%%a

echo Username:%sqlUSR%
echo Password:%sqlPWD%
echo Database:%sqlDB%
SETLOCAL ENABLEDELAYEDEXPANSION
rem cd /d %path_current%
cd /d %file_path% 

set prepsqlCommand=EDPUBpreparedeleteVersions.sql
