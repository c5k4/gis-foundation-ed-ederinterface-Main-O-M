...
echo off
rem Script Name: EDG_SCHM_Update_Command.bat
...
set cmdLine="PGESchematicUpdateCommand.py"
set SQLScript_path=D:\edgisdbmaint\Reset_SchematicStatus.sql
set uname=EDGIS
for /f %%a in ('PGE_DBPasswordManagement.exe GETPASSWORD EDGIS@EDSCHM') do set "PWD=%%a"
set Database=EDSCHM
...
rem if not exist %pythonexe% set pythonexe=d:\python27\arcgis10.2\python.exe
rem if not exist %pythonexe% set pythonexe=c:\python27\arcgis10.2\python.exe
set cmdLine=%pythonexe% %cmdLine%

echo Executing Reset_SchematicStatus on %Database%.....  >> %file_log% 2>&1

sqlplus -s %uname%/%PWD%@%Database% @%SQLScript_path% >> %file_log%  2>&1
set /A error_level=%error_level% + %ERRORLEVEL%             


echo Completed executing Reset_SchematicStatus on %Database% >> %file_log% 2>&1
