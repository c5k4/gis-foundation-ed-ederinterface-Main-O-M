...
for /f %%a in ('PGE_DBPasswordManagement.exe GETSDEFILE EDGIS@EDSUBGMC_PR_DC2A') do set "targetsde=%%a"

set replicaname=EDERSUB_TO_EDSUBGMCDC2A_Replica
set LOB=EDSUBGMC
...
IF not exist T:\ (
	echo Mapping %export_remote_dir% to T: >> %file_log_tmp% 2>&1
	CALL NET USE T: %export_remote_dir%
)

set pythonscript1=D:\edgisdbmaint\copygdbtosubgeomart_dc2a\copy.py
...
rem CALL NET USE T: /DELETE
rem echo Removed the Mapping %export_remote_dir% from T: >> %file_log_tmp% 2>&1
