...
set sqlDB=EDSUBGMC_PR_DC2B

@echo off

for /f %%a in ('PGE_DBPasswordManagement.exe GETPASSWORD uc4admin@EDSUBGMC_PR_DC2B') do set "sqlPWD=%%a"
...
sqlplus -s %sqlUSR%/%sqlPWD%@%sqlDB% @"D:\edgisdbmaint\Copy_trace_tbl_edsubgmc_prod_dc2b.sql" %sqlPWDS% >> %file_log% 2>&1
