...
set DbType=EDGMC_PR_DC1A
set EDGIS_Database=EDGMC_PR_DC1A

set DbType=EDER_PR
set EDGIS_Databases=EDER_PR

REM set uname=SDE
REM for /f %%a in ('PGE_DBPasswordManagement.exe GETPASSWORD SDE@EDGMC_PR_DC1A') do set "PWD=%%a"

set unames=UC4ADMIN
for /f %%a in ('PGE_DBPasswordManagement.exe GETPASSWORD UC4ADMIN@EDER_PR') do set "PWDS=%%a"

set unamess=UC4ADMIN
for /f %%a in ('PGE_DBPasswordManagement.exe GETPASSWORD UC4ADMIN@EDGMC_PR_DC1A') do set "PWDSS=%%a"
...
sqlplus -s %unames%/%PWDSS%@%EDGIS_Database% @%SQLScript_path% %PWDS% >> %file_log%  2>&1
set /A error_level=%error_level% + %ERRORLEVEL%
