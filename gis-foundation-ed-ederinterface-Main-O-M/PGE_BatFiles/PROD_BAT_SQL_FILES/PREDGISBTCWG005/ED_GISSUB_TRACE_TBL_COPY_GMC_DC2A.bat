...
set sqlDB=EDSUBGMC_PR_DC2A

@echo off

for /f %%a in ('PGE_DBPasswordManagement.exe GETPASSWORD uc4admin@EDSUBGMC_PR_DC2A') do set "sqlPWD=%%a"
...
sqlplus -s %sqlUSR%/%sqlPWD%@%sqlDB% @"D:\edgisdbmaint\Copy_trace_tbl_edsubgmc_prod_dc2a.sql"          >> %file_log% 2>&1
