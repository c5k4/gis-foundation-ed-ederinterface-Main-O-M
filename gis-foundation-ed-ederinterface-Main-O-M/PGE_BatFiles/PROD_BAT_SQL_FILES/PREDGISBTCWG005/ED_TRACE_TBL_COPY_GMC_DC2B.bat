...
set sqlDB=EDGMC_PR_DC2B

@echo off

for /f %%a in ('PGE_DBPasswordManagement.exe GETPASSWORD EDGIS@EDGMC_PR_DC2B') do set "sqlPWD=%%a"
...
sqlplus -s %sqlUSR%/%sqlPWD%@%sqlDB% @"D:\edgisdbmaint\Copy_trace_tbl_edgmc_prod_dc2b1.sql" %sqlPWDS% >> %file_log% 2>&1
echo Error level from script 1: %ERRORLEVEL% >> %file_log% 2>&1
set /A error_level=%error_level% + %ERRORLEVEL%
sqlplus -s %sqlUSR%/%sqlPWD%@%sqlDB% @"D:\edgisdbmaint\Copy_trace_tbl_edgmc_prod_dc2b2.sql" %sqlPWDS% >> %file_log% 2>&1
echo Error level from script 2: %ERRORLEVEL% >> %file_log% 2>&1
set /A error_level=%error_level% + %ERRORLEVEL%
sqlplus -s %sqlUSR%/%sqlPWD%@%sqlDB% @"D:\edgisdbmaint\Copy_trace_tbl_edgmc_prod_dc2b3.sql" %sqlPWDS% >> %file_log% 2>&1
echo Error level from script 3: %ERRORLEVEL% >> %file_log% 2>&1
set /A error_level=%error_level% + %ERRORLEVEL%
sqlplus -s %sqlUSR%/%sqlPWD%@%sqlDB% @"D:\edgisdbmaint\Copy_trace_tbl_edgmc_prod_dc2b4.sql" %sqlPWDS% >> %file_log% 2>&1
