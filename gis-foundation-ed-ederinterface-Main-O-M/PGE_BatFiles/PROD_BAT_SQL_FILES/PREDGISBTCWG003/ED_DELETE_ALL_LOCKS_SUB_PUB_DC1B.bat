...
rem ===========================================================================
rem DELETE_ALL_LOCKS_SUB_PUB.bat
rem ---------------------------------------------------------------------------
rem This SQL Script
...
rem Usage:  DELETE_ALL_LOCKS_SUB_PUB.bat
rem         
...
set file_path="D:\Program Files (x86)\Miner and Miner\PG&E Custom Components\PGE.PublicationUpdate\Delete_Locks"

set cmdinifile=ini.cmd
set inifile=EDSUBPUB_PR_DC1BUnlockconfig.ini
...
rem for /f %%a in ('D:\Program Files (x86)\Miner and Miner\PG&E Custom Components\PGE.PasswordMgmt\PGE_DBPasswordManagement.exe GETPASSWORD UC4ADMIN@EDSUBPUB_PR_DC1B') do set "sqlPWD=%%a"
for /f %%a in ('PGE_DBPasswordManagement GETPASSWORD %sqlUSR%@%sqlDB%') do set "sqlPWD=%%a"
...
set CleanTablesLocksCommand=CleanTablesLocks.sql

rem ===========================================================================
rem Get the Date into a variable.
rem ---------------------------------------------------------------------------
reg copy       "HKCU\Control Panel\International" "HKCU\Control Panel\Int-Temp-Del-Lck-SUBPUB-DC1B" /f >nul
...
set date_log=%date%
reg copy       "HKCU\Control Panel\Int-Temp-Del-Lck-SUBPUB-DC1B" "HKCU\Control Panel\International" /f >nul
reg delete     "HKCU\Control Panel\Int-Temp-Del-Lck-SUBPUB-DC1B"                                    /f >nul
rem ===========================================================================
...
rem ---------------------------------------------------------------------------
rem Execute Prepare Remove Locks Sql.
rem ===========================================================================

cd /d %file_path%                                              
echo Executing %CleanTablesLocksCommand%... >> %file_log_tmp% 2>&1

sqlplus -s %sqlUSR%/"%sqlPWD%"@%sqlDB% @%CleanTablesLocksCommand%   >> %file_log_tmp% 2>&1
       
set /A error_level=%error_level% + %ERRORLEVEL%
echo Executed  %CleanTablesLocksCommand%.  >> %file_log_tmp% 2>&1                              
