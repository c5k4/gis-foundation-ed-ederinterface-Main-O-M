...
set sqlDB=EDPUB_PR_DC1B

@echo off

for /f %%a in ('PGE_DBPasswordManagement.exe GETPASSWORD uc4admin@EDPUB_PR_DC1B') do set "sqlPWD=%%a"
...
sqlplus -s %sqlUSR%/%sqlPWD%@%sqlDB% @"D:\edgisdbmaint\Copy_trace_tbl_edpub_prod_dc1b1.sql"          >> %file_log% 2>&1
echo Error level from script 1: %ERRORLEVEL% >> %file_log% 2>&1
set /A error_level=%error_level% + %ERRORLEVEL%
sqlplus -s %sqlUSR%/%sqlPWD%@%sqlDB% @"D:\edgisdbmaint\Copy_trace_tbl_edpub_prod_dc1b2.sql"          >> %file_log% 2>&1
echo Error level from script 2: %ERRORLEVEL% >> %file_log% 2>&1
set /A error_level=%error_level% + %ERRORLEVEL%
sqlplus -s %sqlUSR%/%sqlPWD%@%sqlDB% @"D:\edgisdbmaint\Copy_trace_tbl_edpub_prod_dc1b3.sql"          >> %file_log% 2>&1
echo Error level from script 3: %ERRORLEVEL% >> %file_log% 2>&1
set /A error_level=%error_level% + %ERRORLEVEL%
sqlplus -s %sqlUSR%/%sqlPWD%@%sqlDB% @"D:\edgisdbmaint\Copy_trace_tbl_edpub_prod_dc1b4.sql"          >> %file_log% 2>&1
