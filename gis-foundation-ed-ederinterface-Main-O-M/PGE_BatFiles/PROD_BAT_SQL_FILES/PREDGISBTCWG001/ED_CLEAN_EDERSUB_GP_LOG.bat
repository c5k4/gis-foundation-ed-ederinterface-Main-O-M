...
rem ===========================================================================
rem ED_REMOVE_GP_HISTORY_EDER.bat
rem ---------------------------------------------------------------------------
rem This script calls a python script performs the
rem Removes GP  History
...
rem Usage:  ED_REMOVE_GP_HISTORY_EDER.bat
rem         
...
set config_name=SDE@EDERSUB_PR
set schema_include_list=EDGIS
set filename_script_01=RemoveGPHistory_AGS_10_8_EGDB_v5.py
set file_path=D:\edgisdbmaint
...
rem ---------------------------------------------------------------------------
cd /d %file_path%
set command=call %pythonexe% %file_script_01% %config_name% %schema_include_list%
echo Executing %command%... >> %file_log_tmp% 2>&1
               %command%    >> %file_log_tmp% 2>&1
echo ERRORLEVEL after the script is %ERRORLEVEL%	>> %file_log_tmp% 2>&1
set /A error_level=%error_level% + %ERRORLEVEL%
...
rem Forcefully setting error level to 0
set /A error_level=0
