...
namespace PGE.Common.Delivery.Geodatabase.ChangeDetection
{
    ...
    public class DifferenceManager : IDifferenceManager 
    {
        ...
        public DifferenceManager(IVersion targetVersion, IVersion sourceVersion)
        {
        ...
        public bool HasChanges(string className, esriDataChangeType dataChangeType)
        {
            ...
            if (fIDSet2 != null)
            {
                // not sure whether IFIDSet2.Count() is faster.
                int fID;
                fIDSet2.Reset();
                fIDSet2.Next(out fID);
                if (fID != -1)
                {
                    changed = true;
                }
        ...
        public List<string> ModifiedClassNames
        {
            get
            {
                if (_modifiedClassNames == null)
                {
                    ...
                    IDataChanges dataChanges = (IDataChanges)versionDataChangesInit;
                    ...
                    {
                        //_modifiedClassNames.Add(modifiedClassInfo.ParentClassName.ToUpper());
                        _modifiedClassNames.Add(modifiedClassInfo.ChildClassName.ToUpper());

                        modifiedClassInfo = enumModifiedClassInfo.Next();
                    }
                    //_modifiedClassNames = GetModifiedClassNames(dataChanges);
                }
