...
decimal-separator="." grouping-separator=","/>
	...
	<xsl:template match="DESIGN">
		...
		<xsl:if test="CU">
			...
			<table id="My CU table" style="behavior:url('{$XSLDirectory}\tableAct.htc');BORDER: black 1px solid; WIDTH: 99%; background-color:#eeeecc;"
	...
	<xsl:template match="WORKLOCATION">
		...
		<xsl:choose>
			<xsl:when test="DESCRIPTION!=''">
				<b>Work Location <xsl:value-of select="ID"/>:  <xsl:value-of select="DESCRIPTION"/></b>
			</xsl:when>
			<xsl:otherwise>
				<b>Work Location <xsl:value-of select="ID"/></b>
			</xsl:otherwise>
		</xsl:choose>
	...
	<xsl:template match="CU" mode="rows">
		<tr>
			...
			<td width="100" align="center">
				<xsl:if test="LENGTH!=''">
					<xsl:value-of select="format-number(LENGTH,'#.##','engUS')"/>
					<xsl:choose>
