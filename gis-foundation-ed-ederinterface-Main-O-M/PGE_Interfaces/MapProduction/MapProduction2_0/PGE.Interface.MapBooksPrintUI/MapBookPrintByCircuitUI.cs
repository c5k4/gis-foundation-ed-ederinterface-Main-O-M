...
{
    public partial class MapBookPrintByCircuitUI : Form
    {
        ...
        private List<string> _mapGridList = null;
        private IAsyncResult _asyncInvokeResult = null;
        private DataTable _circuitToMapNumTable = null;
        ...
        #region Constructor
        public MapBookPrintByCircuitUI()
        {          
            ...
            this._grpNetworkTop = grpCircuit.Top;
            ...
            this.Size = new Size(this.Width, _WindowFullHeight - grpCircuit.Height - grpFilter.Height);
        ...
        private void btnAdvanced_Click(object sender, EventArgs e)
        {
            if (grpFilter.Visible == true)
            {
                ...
                if (grpCircuit.Visible)
                {
                    grpCircuit.Top = grpFilter.Top;
            ...
            if (!grpCircuit.Visible && grpFilter.Visible)
            {
                this.Size = new Size(this.Width, _WindowFullHeight - grpCircuit.Height);
                this.Height = _WindowFullHeight - grpCircuit.Height;
            }
            else if (grpCircuit.Visible && grpFilter.Visible)
            {
                grpCircuit.Top = _grpNetworkTop;
        ...
        public void DefaultDropDown_IndexChangeCompleted(object sender, EventArgs e)
        {
            ...
            if (_tracedMapByCircuitList != null && _tracedMapByCircuitList.Count > 0)
            {
                GetCircuitMaps();
            }
        ...
        private void MapBookPrintByCircuitUI_Load(object sender, EventArgs e)
        {
            PopulateCircuitList();
            _tracedMapByCircuitList = new List<string>();
        }

        private void PopulateCircuitList() //PopulateCircuitList(IGeometricNetwork geomNetwork)
        {
            CsvUtil util = new CsvUtil();

            string csvFilePath = ConfigurationManager.AppSettings["CircuitMapNumCSVFilePath"];
            _circuitToMapNumTable = util.CsvToDataTable(csvFilePath);

            DataView dv = new DataView(_circuitToMapNumTable);
            DataRow[] distinctCircuitIDs = dv.ToTable(true, "CIRCUITID").Select().OrderBy(row => row["CIRCUITID"] as string).ToArray();
            foreach (DataRow dRow in distinctCircuitIDs)
            {
                ListViewItem item = new ListViewItem();
                item.Text = dRow["CIRCUITID"] as string;
                item.Name = dRow["CIRCUITID"] as string;
                lstCircuitList.Items.Add(item);
            }
        }

        private void SelectMapByCircuit(List<String> result)
        {
            if (_asyncInvokeResult != null)
            {
                cboGridNumber.EndInvoke(_asyncInvokeResult); //Wait for cboGridNumber to be populated completely

                cboGridNumber.Enabled = false; cboGridNumber.ClearItems();

                List<String> cboItems = new List<String>();
                foreach (var item in result)
                {
                    if (_mapGridList.Contains(item.ToString()))
                    {
                        cboItems.Add(item);
                    }
                }
                ...
                MethodInvoker inv = delegate()
                {
                    cboGridNumber.BeginUpdate();
                    result.Sort();
                    foreach (string s in cboItems) { cboGridNumber.AddItem(s); }
                    cboGridNumber.EndUpdate();
                    cboGridNumber.Enabled = true;
                };
                ...
                cboGridNumber.EndInvoke(_asyncInvokeResult); 
        ...
        private void lstCircuitList_ItemChecked(object sender, ItemCheckedEventArgs e)
        {
            ...
            int circiuiListindex = e.Item.Index; //e.Index;
            try
            {
                ListViewItem[] selectedItems = lstSelectedCircuitList.Items.Find(e.Item.Name, false);
                if (selectedItems.Length >= 1)
                {
                    if (!e.Item.Checked)
                    {
                        lstSelectedCircuitList.Items.Remove(selectedItems[0]);
                    }                   
                }
                else if (e.Item.Checked)
                {
                    ...
                    item.Checked = true;
                    lstSelectedCircuitList.Items.Add(item);
                    lstSelectedCircuitList.Refresh();
                }
            }
            catch{ }
        ...
        private void btnCircuitSearch_Click(object sender, EventArgs e)
        {
            if (cboScale.SelectedItem != null)
            {
                GetCircuitMaps();
                UpdateMultiSelectIndicator(0);
            }
            ...
            {
                MessageBox.Show("Please select Map Scale.", "Required field is not selected", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
            }
        }

        private void GetCircuitMaps()
        {
            ...
            if (_asyncInvokeResult != null)
            {
                ...
                if (!string.IsNullOrEmpty(cboScale.SelectedItem.ToString()) && lstCircuitList.CheckedIndices.Count > 0)
                {
                    KeyValuePair<string, string> cboScaleSelectedItem = (KeyValuePair<string, string>)cboScale.SelectedItem;

                    StringBuilder circuitIDs = new StringBuilder();

                    for (int i = 0; i <= lstCircuitList.CheckedIndices.Count - 1; i++)
                    {
                        if (i == lstCircuitList.CheckedIndices.Count - 1)
                            circuitIDs.Append("'" + lstCircuitList.Items[lstCircuitList.CheckedIndices[i]].Text + "'");
                        else
                            circuitIDs.Append("'" + lstCircuitList.Items[lstCircuitList.CheckedIndices[i]].Text + "'" + ",");
                    }

                    DataRow[] rows = _circuitToMapNumTable.Select("MAPSCALE = " + cboScaleSelectedItem.Key + " AND CIRCUITID IN (" + circuitIDs.ToString() + ")").OrderBy(row => row["MAPGRIDNUM"] as string).ToArray();
                    foreach (DataRow row in rows)
                    {
                        string mapGridNum = row["MAPGRIDNUM"] as string;
                        result.Add(mapGridNum);
                    }
        ...
        private void lstSelectedCircuitList_ItemCheck(object sender, ItemCheckEventArgs e)
        {
            try
            {
                ListViewItem[] selectedItems = lstCircuitList.Items.Find(lstSelectedCircuitList.Items[e.Index].Name, false);
                if (selectedItems.Length >= 1)
                {
                    selectedItems[0].Checked = (e.NewValue == CheckState.Checked);
                }
            }
            catch { }
        }
