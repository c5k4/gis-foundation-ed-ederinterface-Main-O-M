...
def main():
    ...
    dpi = arcpy.GetParameterAsText(8)
    if dpi == '#' or not dpi:
        dpi = 300 # provide a default value if unspecified


    # Get Tiff file path to create
    outputTiffPath = arcpy.GetParameterAsText(9)
    if outputTiffPath == '#' or not outputTiffPath:
        arcpy.AddError("Cannot proceed without specifying an output path for the Tiff file")
        return
    ...
    for Layout in LayoutArray:
        ...
        # Export Tiff
        arcpy.AddMessage("Saving as TIFF")
        arcpy.mapping.ExportToTIFF(mapDoc,os.path.join(outputTiffPath,fileName+".tif"),dataFrame,dataFrame.extent.width,dataFrame.extent.height,dpi,True,"24-BIT_TRUE_COLOR","LZW",True)
