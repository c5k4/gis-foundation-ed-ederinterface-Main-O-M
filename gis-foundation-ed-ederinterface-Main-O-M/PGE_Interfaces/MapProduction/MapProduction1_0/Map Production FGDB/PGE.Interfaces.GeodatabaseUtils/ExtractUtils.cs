...
namespace PGE.Interfaces.GeodatabaseUtils
{
    public class ExtractUtils
    {
        ...
        public ExtractUtils(FileInfo logfileInfo, ISpatialReference toSpatialRef, ITransformation transformation)
        {
                ...
                while (pSourceAnnoFeature != null)
                {
                    ...
                    if (includeFeature)
                    {
                        ...
                        if (newFeatureCount == 5000)
                        {
                            ...
                            {
                                WriteToLogfile(pDestDS.BrowseName + " is having element collection error. ");
                            }
                        }
                    }
            ...
            string filter)
        {
            ...
            try
            {
                ...
                if (pSourceFeature != null)
                {
                    ...
                    while (pSourceFeature != null)
                    {
                        //WriteToLogfile("Insert featurebuffer on: " +
                        // pDestDS.Name + " oid: " + pSourceFeature.OID.ToString());
                        //Loop through the features to clip   
                        recCount++;
                        ...
                        while (enumFields.MoveNext())
                        {
                            ...
                            else
                            {
                                try
                                {
                                    ...
                                    object theValue = pSourceFeature.
                                        get_Value(srcFieldIndex);
                                    string srcname = pSourceFeature.Fields.get_Field(srcFieldIndex).Name;
                                   
                                    if (pInsertFB.Fields.FindField(srcname) != -1)
                                    {
                                        string dstname = pInsertFB.Fields.get_Field(pInsertFB.Fields.FindField(srcname)).Name;
                                        if(dstname == "STRTCHDT")
                                    { }
                                        if (!string.IsNullOrEmpty(theValue.ToString()))
                                        {
                                            if ((dstname != "OBJECTID") && (dstname != "GLOBALID"))
                                            {
                                                pInsertFB.set_Value(pInsertFB.Fields.FindField(srcname),
                                                    theValue);
                                            }
                                        }
                                    }
                                    ...
                                    {
                                        WriteToLogfile("Field not matched-" + pSourceFeature.Fields.get_Field(srcFieldIndex).Name);
                                    }
                                ...
                                catch (Exception ex)
                                {
                                    ...
                                    string ss = pSourceFeature.get_Value(pSourceFeature.Fields.FindField("SAPEQUIPID")).ToString();
                                    string sn = pSourceFeature.Fields.get_Field(srcFieldIndex).Name;
                                    WriteToLogfile("Failed to set value of field :  " + sn + " on: " +
                                       pDestDS.Name + " SAPEQUIPID: " + ss);
                                    WriteToLogfile("Exception Message - " + ex.Message.ToString());
                        ...
                        {
                            //IFeature pff = pDestFC.CreateFeature();
                            //pff.Shape = pSourceFeature.ShapeCopy;
                            //pff.Store();
                            string ss = pSourceFeature.get_Value(pSourceFeature.Fields.FindField("SAPEQUIPID")).ToString();
                            string sn = pSourceFeature.Fields.get_Field(srcFieldIndex).Name;
                            WriteToLogfile("unable to insert feature  : " +
                            pDestDS.Name + " SAPEQUIPID: " + ss);
                            WriteToLogfile("Exception Message - " + ex.Message.ToString());
                            // throw new Exception("Unable to insert feature");
                        }
