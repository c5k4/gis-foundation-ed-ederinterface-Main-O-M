...
namespace Powerbase_To_GIS
{
    public class MainClass
    {
        ...
        public static bool StartProcess()
        {
            ...
            {               
                try
                {
                    ...
                    Common._log.Info("Product Name : " + productName + ",  Product Version : " + productVersion);
                }
                catch (Exception exp)
                {
                ...
                // Changes done to skip previous session status check based on config value
                if (Convert.ToBoolean(ReadConfigurations.GetValue(ReadConfigurations.CheckPreviousSessionStatus)))
                {
                    strSessionStatus = VersionOperations.CheckPreviousRunSessionStatus(connstringpgedata, connstringpbuser);
                }
                else
                {
                    strSessionStatus = string.Empty;
                }
                ...
                if (strSessionStatus == ReadConfigurations.SESSION_STATUS_INPROGRESS || strSessionStatus == ReadConfigurations.SESSION_STATUS_PENDING_QAQC || strSessionStatus == ReadConfigurations.SESSION_STATUS_HOLD || strSessionStatus == ReadConfigurations.SESSION_STATUS_POST_QUEUE || strSessionStatus == ReadConfigurations.SESSION_STATUS_POST_ERROR || strSessionStatus == ReadConfigurations.SESSION_STATUS_RECONCILE_ERROR || strSessionStatus == ReadConfigurations.SESSION_STATUS_CONFLICT || strSessionStatus == ReadConfigurations.SESSION_STATUS_DATA_PROCESSING || strSessionStatus == ReadConfigurations.SESSION_STATUS_QA_QC_ERROR)
                {
                    ...
                    Program.comment = "The previous Session status is : " + strSessionStatus;
                    ...
                    strSessionName = VersionOperations.GetSessionNameToCheck(connstringpgedata);
                    EmailMainClass.ComplieAndSendMailForSessionInProgress(strSessionName, strSessionStatus);
                    return false;
                }
                else if (strSessionStatus == ReadConfigurations.SESSION_STATUS_POSTED || strSessionStatus == ReadConfigurations.SESSION_STATUS_DELETED)
                {
                   ...
                   // Program.comment = "The previous Session status is : " + strSessionStatus;
                    ...
                    Common.MoveDataToArchiveTable(connstringpgedata, ReadConfigurations.GetValue(ReadConfigurations.TB_PB_Session), ReadConfigurations.GetValue(ReadConfigurations.TB_PB_Session_Archive), string.Empty);
                }
                ...
                string connection_string = ReadEncryption.GetSDEPath(ConfigurationManager.AppSettings["ConnString_EDWorkSpace"].ToUpper()); 
                ...
                string queryToGetDataToUpdate = "Select * from " + ReadConfigurations.GetValue(ReadConfigurations.TB_PowerbaseStage)+ ReadConfigurations.GetValue(ReadConfigurations.whereClause_PowerbaseStage);
                ...
                {
                    Common._log.Info("Total records to update : " + dtDataToUpdate.Rows.Count);
                    ...
                    if (bUpdateStatus)
                    {
                        ...
                        if (bSessionSubmitToGDBM)
                        {
                            ...
                            // Changes done to skip previous session status check based on config value - skipping to store session as well.
                            if (Convert.ToBoolean(ReadConfigurations.GetValue(ReadConfigurations.CheckPreviousSessionStatus)))
                            {
                        ...
                        {
                            Program.comment = "There is some error while submitting the session to GDBM : " + strSessionName;
                    ...
                    {
                        Program.comment = "There is some error while saving the features in session : " + strSessionName;
                ...
                {   Program.comment=("There is no data present in input table")+ReadConfigurations.GetValue(ReadConfigurations.TB_PowerbaseStage) + " to update in EDGIS.";                
                    ...
                    Common._log.Info("Exiting the process.");
                    // Prefer to send mail
                    EmailMainClass.ComplieAndSendMailForNoRecordsInStageTable();
                }
        ...
        public static IWorkspace GetWorkspace(string argStrworkSpaceConnectionstring)
        {
            ...
            {
                Program.comment = "Error in getting SDE Workspace, function GetWorkspace: " + "Exception : " + exp.Message;
        ...
        public static bool PreRequiste()
        {
            ...
            {
                Program.comment = "Error in Initializing Arc GIS/ArcFM License, function PreRequiste " + "Exception : " + exp.Message;
       ...
       /* private static bool UpdateStagingtableStatusandMovingDataToArchiveTable(DataTable argdtRecords,string argStrConnstring)
        {
            ...
            {               
                ...
                Common.MoveDataToArchiveTable(argStrConnstring, ReadConfigurations.GetValue(ReadConfigurations.TB_PowerbaseStage), ReadConfigurations.GetValue(ReadConfigurations.TB_PowerbaseStage_Archive), ReadConfigurations.GetValue(ReadConfigurations.whereClause_PowerbaseStage));
