...
namespace Powerbase_To_GIS
{
    public class Common
    {
        ...
        public static void InitializeESRILicense()
        {
            ...
            {
                Program.comment = "ESRI License check out failed" + exp.Message;
                ...
                foreach (String fcName in dictFeatureClasses.Keys)
                {
                    ...
                    try
                    {
                        ...
                        foreach (DataRow pRow in pRows)
                        {
                            try
                            {
                                //Give message how many records processed yet 
                                iRecordsprocessed++;
                                if (iRecordsprocessed % 500 == 0)
                                {
                                    Common._log.Info("Records processed yet : " + iRecordsprocessed);
                                }
                                ...
                                if (pfeature != null)
                                {
                                    ...
                                    foreach (DataRow pColumnRow in pColumnRows)
                                    {
                                        try
                                        {
                                            PBColumnName = Convert.ToString(pColumnRow[ReadConfigurations.col_PBFieldName]).Trim();
                                            GISColumnName = Convert.ToString(pColumnRow[ReadConfigurations.col_GISFieldName]).Trim();
                                            valueToSave = Convert.ToString(pRow[PBColumnName]).Trim();
                                            ...
                                            if (!string.IsNullOrEmpty(GISColumnName) && !(valueToSave == ReadConfigurations.GetValue(ReadConfigurations.PB_Value_NO_UPDATE) || valueToSave == ReadConfigurations.GetValue(ReadConfigurations.PB_Value_NOT_RELEVANT)))
                                            {
                                                ...
                                                else
                                                {
                                                    ...
                                                    if (iFieldIndex != -1)
                                                    {
                                                        ...
                                                        IDomain pDomain = Common.GetDomain(pfeature, pField);
                                                        //string pDOmainName = pDomain.Name;
                                                        ...
                                                        if (pDomain != null && pDomain is ICodedValueDomain)
                                                        {
                                                            ...
                                                            if (!ChecktheGivenValueIsPresentInDomainValues(valueToSave, pCodedValueDomain, ref strAllAcceptableValues))
                                                            {
                                            ...
                                            else
                                            {
                                                ...
                                                //comment = "Given field is not relevant for given EDGIS featureclass/Table as per mapping file.";                                                
                                                //Common._log.Info(comment);
                                            }
                                        }
                                        catch (System.Exception exp)
                                        {
                                            Program.comment = "Error for feature class" + exp.Message;
                                            Common._log.Error("Error for feature class : " + gisFeatureclassName + " , GUID : " + strGLOBALID + ", Exception : " + exp.Message + " |  Stack Trace | " + exp.StackTrace + " at " + exp.ToString());
                                            commentsToStore = "Error occured";
                                        }
                                    }
                            ...
                            {
                                Program.comment = "Error in processing record with GUID : " + strGLOBALID;
                                Common._log.Error("Exception : " + exp.Message + " |  Stack Trace | " + exp.StackTrace + " at " + exp.ToString());
                                Common._log.Error(comment);
                                pRow[ReadConfigurations.col_RECORD_STATUS] = ReadConfigurations.value_FAILED;
        ...
        ///// <returns></returns>
        //private static IDomain GetDomain(IFeature argpfeature, IFeatureClass argpFeatureClass, IField argpField)
        //{            
        //    IDomain pDomain = null;
        //    try
        //    {
        //        pDomain = argpField.Domain;

        //        if (pDomain == null)
        //        {
        //            IRowSubtypes rowSubtypes = (IRowSubtypes)argpfeature;
        //            ISubtypes subtypes = (ISubtypes)argpFeatureClass;
        //            string fieldName = argpField.Name;
        //            pDomain = subtypes.get_Domain(rowSubtypes.SubtypeCode, fieldName);
        //        }                
        //    }
        //    catch (System.Exception exp)
        //    {
        ...
        public static bool MakeChangesForPBSessionTable(string argStatus, string argstrSessionName, string argStrOperation)
        {
            ...
            {
                Program.comment = "Error in Updating the Custom Session Table . Exception : " + exp.Message;
                Common._log.Error("Exception : " + exp.Message + " |  Stack Trace | " + exp.StackTrace + " at " + exp.ToString());
                ErrorCodeException ece = new ErrorCodeException(exp);
                Environment.ExitCode = ece.CodeNumber;
        ...
        /// <returns></returns>
        public static bool MoveDataToArchiveTable(string argstrConnString, string argStrMainTable, string argStrArchiveTable, string argStrWhereClause)
        {
            string sQuery = null;
            try
            {
                sQuery = "insert into " + argStrArchiveTable + " select * from " + argStrMainTable + argStrWhereClause;

                int intUpdateResult = DBHelper.ExecuteQueryOnDatabase(argstrConnString, sQuery);
                Common._log.Info("Ran query : " + sQuery + " and result count: " + intUpdateResult);

                sQuery = "delete from " + argStrMainTable + argStrWhereClause;
        ...
        private static bool ChecktheGivenValueIsPresentInDomainValues(string argStrValue, ICodedValueDomain argCodeValueDomain, ref string strAllAcceptableValues)
        {
            ...
            try
            {
                ...
                // Exceptional condition to update Null value in GIS for a column which is controlled by a domain 
                //Add value in config file # seperated, here check value present in config, dont check for single value from config. By this way it will be easier to control through config
                // Pass domain name in this function
                //if (Convert.ToString( argCodeValueDomain.Name) == "")
                //{
                //    bValueExist = true;
                //}
            }
