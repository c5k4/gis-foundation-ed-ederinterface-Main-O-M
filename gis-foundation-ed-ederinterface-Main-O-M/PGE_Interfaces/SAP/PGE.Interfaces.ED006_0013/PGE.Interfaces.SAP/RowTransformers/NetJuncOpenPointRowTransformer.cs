...
{
    class NetJuncOpenPointRowTransformer : SAPDeviceRowTransformer
    {
        ...
        public override bool IsValid(IRow sourceRow, out string errorMsg)
        {
            bool retValue = true;
            errorMsg = "";
            retValue = (IsNetworkFeature(sourceRow));
            if (retValue == false)
            {
                errorMsg = "Not a Network Feature";
                GISSAPIntegrator._logger.Info(sourceRow.OID + " Not a Network Feature ");
                GISSAPIntegrator._logger.Error(sourceRow.OID + " Not a Network Feature ");
            }

                return retValue;
            //return IsNetworkFeature(sourceRow);
