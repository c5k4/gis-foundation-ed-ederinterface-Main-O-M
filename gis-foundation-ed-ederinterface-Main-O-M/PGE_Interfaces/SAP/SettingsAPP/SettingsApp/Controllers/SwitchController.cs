...
namespace SettingsApp.Controllers
{
    ...
    {
        private const string DEVICE_NAME = "Switch";
        private const string DEVICE_TABLE_NAME = "SM_SWITCH";
        private const string DEVICE_HIST_TABLE_NAME = "SM_SWITCH_HIST";
        ...
        public ActionResult Index(string globalID, string layerName, string layerType)
        {
            ...
            SwitchModel model = getDevice(globalID, HtmlExtensions.PageMode.Current);
        ...
        public ActionResult Future(string globalID, string layerName,string layerType)
        {
            ...
            ViewBag.LayerName = layerName;
            ViewBag.layerType = layerType;
            SwitchModel model = getDevice(globalID, HtmlExtensions.PageMode.Future);
        ...
        public ActionResult History(string globalID, string layerName, string layerType)
        {
            ...
            using (SettingsEntities db = new SettingsEntities())
            {
                ...
                List<History> swi_hist = (from c in db.SM_COMMENT_HIST
                                          join s_h in db.SM_SWITCH_HIST.Where(s => s.GLOBAL_ID == globalID) on c.HIST_ID equals s_h.ID
                                          select new History
                                          {
                                              ID = c.ID,
                                              GlobalID = c.GLOBAL_ID,
        ...
        public ActionResult HistoryDetails(string globalID, string layerName, decimal ID, string layerType)
        {
            ...
            using (SettingsEntities db = new SettingsEntities())
            {
                SM_SWITCH_HIST e = db.SM_SWITCH_HIST.FirstOrDefault(s => s.GLOBAL_ID == globalID && s.ID == ID);
        ...
        public ActionResult Files(string globalID, string layerName,string layerType, string settingsError = null, string peerReviewError = null)
        {
            ...
            using (SettingsEntities db = new SettingsEntities())
            {
                SM_SWITCH e = db.SM_SWITCH.SingleOrDefault(s => s.GLOBAL_ID == globalID && s.CURRENT_FUTURE == "C");
        ...
        [HttpPost]
        public ActionResult SaveFuture(SwitchModel model, string globalID, string layerName, string layerType)
        {
            ViewBag.GlobalID = globalID;
            ViewBag.LayerName = layerName;
            ViewBag.layerType = layerType;
            ...
            try
            {
                if (ModelState.IsValid)
                {
                    ...
                    using (var transaction = db.Connection.BeginTransaction())
                    {
                        if (model.Release)
                        {
                            SM_SWITCH_HIST history = this.saveHistory(db, globalID, model);
                            ...
                            db.SM_SWITCH_HIST.AddObject(history);
                            db.SaveChanges();
        ...
        [HttpPost]
        public ActionResult SwitchTypeChanged(SwitchModel model, string globalID, string layerName, string layerType)
        {
            ViewBag.GlobalID = globalID;
            ViewBag.LayerName = layerName;
            ViewBag.layerType = layerType;
        ...
        [HttpPost]
        public ActionResult ScadaChanged(SwitchModel model, string globalID, string layerName, string layerType)
        {
            ViewBag.GlobalID = globalID;
            ViewBag.LayerName = layerName;
            ViewBag.layerType = layerType;
        ...
        private SwitchModel getDevice(string globalID, HtmlExtensions.PageMode pageMode)
        {
            SwitchModel model = new SwitchModel();
            ...
            using (SettingsEntities db = new SettingsEntities())
            {
                SM_SWITCH e = db.SM_SWITCH.SingleOrDefault(s => s.GLOBAL_ID == globalID && s.CURRENT_FUTURE == currentFuture);
                if (e == null)
        ...
        private void saveDevice(SettingsEntities db, string globalID, HtmlExtensions.PageMode pageMode, SwitchModel model)
        {
            string currentFuture = (pageMode == HtmlExtensions.PageMode.Future) ? "F" : "C";
            SM_SWITCH entity = db.SM_SWITCH.SingleOrDefault(s => s.GLOBAL_ID == globalID && s.CURRENT_FUTURE == currentFuture);
            entity.CURRENT_FUTURE = currentFuture;
            model.PopulateEntityFromModel(entity);
            entity.DATE_MODIFIED = DateTime.Now;
        }

        private SM_SWITCH_HIST saveHistory(SettingsEntities db, string globalID, SwitchModel model)
        {
            SM_SWITCH_HIST history = new SM_SWITCH_HIST();
            SM_SWITCH e = db.SM_SWITCH.SingleOrDefault(s => s.GLOBAL_ID == globalID && s.CURRENT_FUTURE == "C");
        ...
        private void initializeDropDowns(SwitchModel m)
        {
            m.AtsFeatureList = new SelectList(SiteCache.GetLookUpValues("SM_SWITCH", "ATS_FEATURE"), "Key", "Value");
            m.ScadaTypeList = new SelectList(SiteCache.GetLookUpValues("SM_SWITCH", "SCADA_TYPE"), "Key", "Value");
            m.UnbalanceDetectList = new SelectList(SiteCache.GetLookUpValues("SM_SWITCH", "UNBALANCE_DETECT"), "Key", "Value");
            m.DwellTimerList = new SelectList(SiteCache.GetLookUpValues("SM_SWITCH", "DWELL_TIMER"), "Key", "Value");
            m.SelectReturnList = new SelectList(SiteCache.GetLookUpValues("SM_SWITCH", "SELECT_RETURN"), "Key", "Value");
            m.EngineeringDocumentList = new SelectList(SiteCache.GetLookUpValues("SM_SWITCH", "ENGINEERING_DOCUMENT"), "Key", "Value");
            m.SelectPreferredList = new SelectList(SiteCache.GetLookUpValues("SM_SWITCH", "SELECT_PREFERRED"), "Key", "Value");
            m.SelectTransitionList = new SelectList(SiteCache.GetLookUpValues("SM_SWITCH", "SELECT_TRANSITION"), "Key", "Value");
            m.OperatingModeList = new SelectList(SiteCache.GetLookUpValues("SM_SWITCH", "OPERATING_MODE"), "Key", "Value");
            m.SCADARadioManufacturerList = new SelectList(SiteCache.GetLookUpValues("SM_SWITCH", "RADIO_MANF_CD"), "Key", "Value");
            m.SwitchTypeList = new SelectList(SiteCache.GetLookUpValues("SM_SWITCH", "SWITCH_TYPE"), "Key", "Value");
            m.ControlUnitTypeList = new SelectList(SiteCache.GetLookUpValues("SM_SWITCH", "CONTROL_UNIT_TYPE"), "Key", "Value");
            m.SectionalizingFeatureList = new SelectList(SiteCache.GetLookUpValues("SM_SWITCH", "SECTIONALIZING_FEATURE"), "Key", "Value");
            ...
            {
                m.FieldsToDisplay = SiteCache.GetFieldsToDisplayForSwitch(m.SwitchType, m.ATSCapable);
            }
            else
            {
                m.FieldsToDisplay = SiteCache.GetFieldsToDisplayForSwitch("22","N");
            }

            m.RTUManufactureList = new SelectList(SiteCache.GetLookUpValues("SM_SWITCH", "RTU_MANF_CD"), "Key", "Value");
