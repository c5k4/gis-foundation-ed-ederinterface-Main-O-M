                ...
                using (conOraEDER = OpenConnection(connstringpgedata))
                {
                    foreach (DataRow pRow in argdtRecords.Rows)
                    {
                        try
                        {
                            ...
                            sqlQueryToInsert = "INSERT INTO " + ReadConfigurations.GetValue(ReadConfigurations.TB_PB_GIS_REJECTED_RECORDS) + " values(" + ibatchID + ",'" + strFeatureClass + "','" + strGLOBALID + "','" + strOperatingNumber + "'," +ipb_rlid+",'"+ strAttributeName + "','" + strAttributeValue + "','" + strComments + "',TO_DATE('" + strDateTime + "', 'MM/DD/YYYY HH:MI:SS AM')" + ")";
