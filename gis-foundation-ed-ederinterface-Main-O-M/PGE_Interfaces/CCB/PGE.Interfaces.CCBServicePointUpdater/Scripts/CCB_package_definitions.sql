...
from 
(
	select distinct STG.SERVICEPOINTID, STG.DATECREATED from PGEDATA.PGE_CCBTOEDGIS_STG STG where STG.SERVICEPOINTID in(
		select distinct STAGETABLE.SERVICEPOINTID from PGEDATA.PGE_CCBTOEDGIS_STG STAGETABLE 
			...
			and STAGETABLE.UNIQUESPID not in (select distinct UNIQUESPID from EDGIS.ZZ_MV_SERVICEPOINT) 
			minus 
		select distinct MVVIEW.SERVICEPOINTID from EDGIS.ZZ_MV_SERVICEPOINT MVVIEW)
) TABLEI;
commit;
...
from 
(
	select distinct STG2.SERVICEPOINTID, STG2.DATECREATED from PGEDATA.PGE_CCBTOEDGIS_STG STG2 where STG2.SERVICEPOINTID in(
		select distinct STAGETABLE2.SERVICEPOINTID from PGEDATA.PGE_CCBTOEDGIS_STG STAGETABLE2 
			where STAGETABLE2.SERVICEPOINTID is not NUll and STAGETABLE2.DATECREATED = 
			(select max(DATECREATED) from PGEDATA.PGE_CCBTOEDGIS_STG where SERVICEPOINTID = STAGETABLE2.SERVICEPOINTID)
			and STAGETABLE2.UNIQUESPID in (select distinct UNIQUESPID from EDGIS.ZZ_MV_SERVICEPOINT)
	minus 
		select distinct MVVIEW.SERVICEPOINTID from EDGIS.ZZ_MV_SERVICEPOINT MVVIEW)
) TABLER;
commit;
...
select
      ...
      -- '' as TRANSFORMERGUID
		from PGEDATA.PGE_CCBTOEDGIS_STG STAGE
			where STAGE.SERVICEPOINTID in (select ACTION.SERVICEPOINTID from PGEDATA.PGE_CCB_SP_ACTION ACTION 
			where ACTION.ACTION = 'GISI'
);
commit;
