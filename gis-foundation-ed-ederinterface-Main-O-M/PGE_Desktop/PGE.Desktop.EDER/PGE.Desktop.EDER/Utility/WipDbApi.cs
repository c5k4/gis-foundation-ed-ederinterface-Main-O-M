...
namespace PGE.Desktop.EDER.Utility
{
    public class WipDbApi
    {
        ...
        private readonly INameLog _log;
        private readonly String _serviceConfigName;

        [Obsolete]
        public WipDbApi(String username, String password, INameLog log = null, String serviceConfigName = "")
        {
            ...
            try
            {
                foreach (String jobNum in jobNums.Keys)                                     // PROCESS EACH JOB NUMBER GATHERED FROM THE SESSION
                {
                    ...
                    if (opNumber.Trim().Length != 0)
                    {
                        ...
                        if (procOpNum)
                        {
                            _log.Info("WipDbApi.CheckRetireInJETJobsTable: Retired Operating Numbers (" + opNumber + ").");
                        }
                        else
                        {
                            _log.Info("WipDbApi.CheckRetireInJETJobsTable: Retired CGC12 Numbers (" + opNumber + ").");
                        }
