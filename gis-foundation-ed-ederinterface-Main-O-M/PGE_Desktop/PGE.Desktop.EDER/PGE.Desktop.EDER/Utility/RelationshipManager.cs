...
namespace PGE.Desktop.EDER.Utility
{
    ...
    internal static class RelationshipManager
    {
        ...
        internal static void ClearRelCache()
        {
            ...
            _logger.Info("Relationship cache cleared.");
        }
        /// <summary>
        ...
        internal static List<IObject> GetRelated(IObject obj, esriRelRole relationshipRole, string classMN, KeyValuePair<int, int>? deletedRelatedObject)
        {
            ...
            foreach (IRelationshipClass relClass in eligibleRelClasses)
            {
                ...
                {
                    // INC000003955512 - EDER Enhancement: AnnoTFMR Bank MVA Rating Change
                    if (relObj.Fields.FindField("ISSPAREIDC") != -1)
                    {
                        if ((Convert.ToString(relObj.get_Value(relObj.Fields.FindField("ISSPAREIDC"))) == "SHARED") || (Convert.ToString(relObj.get_Value(relObj.Fields.FindField("ISSPAREIDC"))) == "DEDICATED"))
                            continue;
                    }
