...
namespace PGE.Desktop.EDER.GDBM.CircuitProcessing
{
    public class PGE_CircuitColor : PxActionHandler
    {
        ...
        {
            if ((new CommonFunctions()).CheckUsage(version, this.Log, ServiceConfiguration) == true)
            {
                return true;
            }
        ...
        private void PerformFieldUpdates(List<string> whereInStringList, IObjectClass oc, IDictionary<string, CircuitSourceData> circuitDataMap)
        {
            ...
            Log.Info(ServiceConfiguration.Name, substationCounter + " had their Circuit Name updated for " + ((IDataset)oc).BrowseName);//S2NN: BugFix for CIRCUITNAME AU
        }
