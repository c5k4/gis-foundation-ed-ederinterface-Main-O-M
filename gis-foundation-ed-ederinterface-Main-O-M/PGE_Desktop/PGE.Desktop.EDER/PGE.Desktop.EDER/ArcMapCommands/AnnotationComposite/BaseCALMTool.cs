...
using PGE.Desktop.Custom.Utility.Annotation;
using PGE.Desktop.Custom.Commands.AnnotationComposite.UI;

namespace PGE.Desktop.EDER.Custom.Commands.AnnotationComposite
{
    /// <summary>
    ...
    public abstract class BaseCALMTool : BaseTool
    {
        ...
        protected bool ReadSettings()
        {
            ...
            try
            {
                ...
                //Font size
                if (CALMSettingsUC.FontSize != null && Double.TryParse(CALMSettingsUC.FontSize, out _size))
                {
                    if (_size <= 0)
                    {
                        MessageBox.Show("Please enter a size greater than 0.", "CALM", MessageBoxButtons.OK,
                            MessageBoxIcon.Error);
                        return false;
                    }
                }
                else _size = AnnotationFacade.DoNotChangeFontSize;
        ...
        protected void TransformAnno(List<IFeature> annosToTransform)
        {
            try
            {
                ...
                {
                    //Start Fix : Issue INC000004289099
                    //if ((UI.CALMSettingsUC.Rotation != null /*&& (UI.CALMSettingsUC.Rotation != "0" )*/) &&
                    //             Double.TryParse(UI.CALMSettingsUC.Rotation, out _angle))
                    //{ }
                    //else
                    //{
                    //    IAnnotationFeature2 pAnno = anno as IAnnotationFeature2;
                    //    IElement pElem = (IElement)pAnno.Annotation;

                    //    ISymbolCollectionElement pSymbolCollectionElement = (ISymbolCollectionElement)pElem;
                    //    IGeometry pgeo = (IGeometry)pElem.Geometry;

                    //    if (pgeo is IPolyline)
                    //    {
                    //        IPolyline pline = (IPolyline)pgeo;
                    //        ILine ln = new Line();
                    //        ln.ToPoint = pline.ToPoint;
                    //        ln.FromPoint = pline.FromPoint;
                    //        double dAngleDegree = (180 * ln.Angle) / (4 * Math.Atan(1));
                    //        _angle = dAngleDegree;
                    //    }
                    //    else if (pgeo is IPoint)
                    //    {
                    //        double dAngleDegree = (180) / (4 * Math.Atan(1));
                    //        _angle = Math.Round(dAngleDegree, 2);
                    //    }
                    //}
                    //End Fix:  Incident INC000004289099 

                    if (CALMSettingsUC.Rotation == null || !Double.TryParse(CALMSettingsUC.Rotation, out _angle)) _angle = AnnotationFacade.DoNotRotate;
                ...
                foreach (CALMAnnotationParentGroup parentGroup in parentsToProcess)
                {
                    ...
                    if (parentGroup.RelationshipClass != null)
                        ...
                        AnnotationFacade.ApplySettingsToNewAnno(parentGroup.RelationshipClass, parentGroup.ParentObject,
                            _alignment, _angle, parentGroup.AnnotationClassID, _inlineGroupAnno, _xOffset, _yOffset, _size);
                }
            }
            catch (Exception ex)
            {
