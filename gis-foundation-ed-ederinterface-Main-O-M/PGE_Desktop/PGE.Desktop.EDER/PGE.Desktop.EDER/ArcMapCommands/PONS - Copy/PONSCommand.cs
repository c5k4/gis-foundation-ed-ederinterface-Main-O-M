...
namespace Telvent.PGE.ED.Desktop.ArcMapCommands.PONS
{
    ...
    [Guid("fdd3e184-a076-4f03-9191-c8e68e1f61b1")]
    [ComponentCategory(ComCategory.ArcMapCommands)]
    [ComVisible(true)]
    [ProgId("Telvent.PGE.ED.Desktop.ArcMapCommands.PONS")]
    public sealed class PONSCommand : BaseCommand
    {
        ...
        public PONSCommand()
        {
            ...
            base.m_name = "Telvent.PGE.ED.Desktop.ArcMapCommands.PONS";   //unique id, non-localizable (e.g. "MyCategory_ArcMapCommand")
                            ...
                            if (PGEGlobal.WORKSPACE_EDER == null || PGEGlobal.WORKSPACE_EDERSUB == null)
                            {
                                ...
                                if (!string.IsNullOrEmpty(_SDEFilePath))
                                {
                                    ...
                                    if (!string.IsNullOrEmpty(_SDEFilePath))
                                    {
                                        ...
                                        configValue = utilityFunctions.ReadConfigurationValue("EDER_OSUSER_SDE_FILENAME");
                                        ...
                                        {
                                            configValue = utilityFunctions.ReadConfigurationValue("EDER_PONSUSER_SDE_FILENAME");
                                            if (!string.IsNullOrEmpty(configValue))
                                            {
                                                PGEGlobal.WORKSPACE_EDER = utilityFunctions.GetWorkspaceFromSDEFile(_SDEFilePath, configValue);
                                                //pSDEWSFactory = new SdeWorkspaceFactoryClass();
                                        ...
                                        configValue = utilityFunctions.ReadConfigurationValue("EDERSUB_OSUSER_SDE_FILENAME");
                                        if (!string.IsNullOrEmpty(configValue))
                                        {
                                        ...
                                        {
                                            configValue = string.Empty;
                                            configValue = utilityFunctions.ReadConfigurationValue("EDERSUB_PONSUSER_SDE_FILENAME");
                                            if (!string.IsNullOrEmpty(configValue))
                                            {
                                                PGEGlobal.WORKSPACE_EDERSUB = utilityFunctions.GetWorkspaceFromSDEFile(_SDEFilePath, configValue);
                                                //pSDEWSFactory = new SdeWorkspaceFactoryClass();
