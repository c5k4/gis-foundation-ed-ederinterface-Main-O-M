            ...
            try
            {
                    ...
                    else if (Convert.ToString(this.dgdeviceView.Rows[rownum].Cells["SearchType"].Value).Equals("BetweenDevicesSearch"))
                    {
                        ...
                        //if (startPGEFeatClass.LayerAddedToMap && (bEndDeviceMentioned ? endPGEFeatClass.LayerAddedToMap : true))
                        {
                            ...
                            if (startPGEFeatClass.FeatureClass != null)
                            {
                                ...
                                bStatus = objTracing.RunTrace(startFeature, endFeature, !true, sVersionName, out tempTransCGCs, out tempPriCGCs);
                                if (!bStatus || (tempTransCGCs.Count <= 0 && tempPriCGCs.Count <= 0))
                                    ...
                                    bStatus = objTracing.RunTrace(endFeature, startFeature, !true, sVersionName, out tempTransCGCs, out tempPriCGCs);
                ...
                if (pEditor.EditState == esriEditState.esriStateEditing)
                {
                    ...
                    while ((pFeat_CS = pFCursor.NextFeature()) != null)
                    {
                        ...
                        bool bStatus = false;
                        //bStatus = objTracing.RunTrace(startFeature, endFeature, this.checkBoxExclude.Checked, sVersionName, out tempTransCGCs, out tempPriCGCs);
                        bStatus = objTracing.RunTrace(pFeat_CS, null, !true, GetCurrentVersionName(((IDataset)pPFClass_CisuitSource.FeatureClass).Name), out tempTransCGCs, out tempPriCGCs);
                ...
                for (int iCount = 0; iCount < pReport.AffectedCustomers_Original.Count; ++iCount)
                {
                    ...
                    if (pReport.AffectedCustomers_Original.Count(c => c.ServicePointID == pReport.AffectedCustomers_Original[iCount].ServicePointID) > 1)
                    {
                        if (pReport.AffectedCustomers_Original[iCount].ORD == 2) continue;
                        pReport.AffectedCustomers.Add(pReport.AffectedCustomers_Original[iCount]);
                        if (!pReport.ExcludeCustomer_Checked)
                            pReport.AffectedCustomers.Add(pReport.AffectedCustomers_Original.First(c => c.ServicePointID == pReport.AffectedCustomers_Original[iCount].ServicePointID && c.ORD != pReport.AffectedCustomers_Original[iCount].ORD));
                    }
                    else
                    {
                        pReport.AffectedCustomers.Add(pReport.AffectedCustomers_Original[iCount]);
        ...
        private void LoadConsumerstoGrid()
        {
            try
            {
                ...
                foreach (Customer customer in pReport.AffectedCustomers)
                {
                    ...
                    dgCustomerList.Rows.Add(!customer.Excluded, customer.CustomerType, customer.ServicePointID, sNameTemp, sAddressTemp, customer.MeterNumber, customer.SNO);
