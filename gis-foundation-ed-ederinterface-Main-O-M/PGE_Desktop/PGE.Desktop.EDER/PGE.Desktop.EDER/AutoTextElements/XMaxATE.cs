...
{
    /// <summary>
    /// AutoTextElement that returns the XMax of the bounding rectangle of the grid being plotted.
    /// </summary>
    [Guid("32CEFB7E-AF3C-4DB6-8760-111D377F1AD9")]
    [ProgId("PGE.Desktop.EDER.XMaxATE")]
    [ComponentCategory(ComCategory.MMCustomTextSources)]
    public class XMaxATE : BaseAutoTextElement
    {
        ...
        /// <summary>
        /// Creates an instance of <see cref="XMaxATE"/>
        /// </summary>
        public XMaxATE()
            : base("PGE.Desktop.EDER.XMaxATE", MapProductionFacade.PGEXMaxCaption, MapProductionFacade.PGEXMaxCaption, MapProductionFacade.PGEXMaxCaption)
        {
        ...
        /// <returns></returns>
        /// <remarks>
        /// Basically, returns the xMax coordinate of the printing,exporting grid.
        /// </remarks>
        protected override string OnStart(mmAutoTextEvents eTextEvent, IMMMapProductionInfo pMapProdInfo)
        {
            ...
            return Math.Round(transformedMapGridEnvelopeGeometry.Envelope.XMax,6).ToString();
        }
        ...
        private IFeature GetCurrentMapGrid(IPageLayout pageLayout, IMap map)
        {
            ...
            try
            {
                ...
                _logger.Debug("MapGrid Info:" + mapGridNumber);
                ...
                else if (gridNameMapNoandOfficeValues.Length > 1)
                {
                    ...
                    _logger.Debug("Featurelayer used:" + mapGridLayers[0].Name); 
                }
                else
                {
