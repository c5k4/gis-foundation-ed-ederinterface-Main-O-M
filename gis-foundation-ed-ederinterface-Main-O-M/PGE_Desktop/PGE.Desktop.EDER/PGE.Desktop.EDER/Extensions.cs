...
namespace PGE.Desktop.EDER
{
    ...
    {
        /// <summary>
        /// Tests if this string contains the given string 'other'.
        /// </summary>
        /// <param name="self">The string to check.</param>
        /// <param name="other">The string which may be contained in this string.</param>
        /// <param name="comparisonType">The type of comparison to use when looking for the string.</param>
        /// <returns>True of this string contains the other; otherwise false.</returns>
        ...
        /// <summary>
        /// Translates a C-style ESRI iteration to a C# enumerable.
        /// </summary>
        /// <param name="self">The iterator to translate.</param>
        ...
        /// <summary>
        /// Retrieves the first FieldInstance with the given model name.
        /// </summary>
        /// <param name="self">The object where the a field with the given model name might be found.</param>
        /// <param name="modelName">The model name of the field.</param>
        /// <returns>Returns a new FieldInstance if the model name was found; otherwise null.</returns>
        public static FieldInstance FieldInstanceFromModelName(this IObject self, string modelName)
        {
            return FieldInstance.FromModelName(self, modelName);
        }
        ...
        {
            IObjectClass selfClass = FeederManager2.GetObjectClassFromObject(self);
            var subtypes = selfClass as ISubtypes;
        ...
        {
            IObjectClass selfClass = FeederManager2.GetObjectClassFromObject(self);
            return ModelNameFacade.ContainsAllClassModelNames(selfClass, modelNames);
        }
        ...
        {
            IObjectClass selfClass = FeederManager2.GetObjectClassFromObject(self);
            return ModelNameFacade.FieldFromModelName(selfClass, modelName).AliasName;
        }
        ...
        public static object GetFieldValue(this IObject self, string fieldName = null, bool useDomain = true, string modelName = null)
        {
            ...
            var _logger = new Log4NetLogger(callingMethod.DeclaringType, "EDERDesktop.log4net.config");
            ...
            if (fieldName == null)
            {
                ...
                field = ModelNameFacade.FieldFromModelName(selfClass, modelName);
                if (field == null)
                {
            ...
            pos = selfClass.FindField(fieldName);
        ...
        /// <param name="domainDescription">The code to look for in the domain.</param>
        /// <param name="defaultValue">An optional default value to return in liu of a result.</param>
        /// <returns></returns>
        public static string LookupDomainDescription(this IObject self, object domainCode, string fieldModelName, string defaultValue = null)
        {
            var callingMethod = new StackFrame(1).GetMethod();
            var _logger = new Log4NetLogger(callingMethod.DeclaringType, "EDERDesktop.log4net.config");
            IField field = ModelNameFacade.FieldFromModelName(self.Class, fieldModelName);
            if (field == null)
            {
                _logger.Debug("Couldn't find field with model name " + fieldModelName);
                return null;
            }
            var domain = field.Domain;
            if (domain == null)
            {
                domain = ((ISubtypes)self.Table).HasSubtype
                    ? ((ISubtypes)self.Table).get_Domain(((IRowSubtypes)self).SubtypeCode, field.Name)
                    : null;
                _logger.Debug("Domain for " + fieldModelName + " is: " + domain.Name);
            }

            if (domain != null)
            {
                var result = domain.Lookup(domainCode.ToString());
                _logger.Debug(field.Name + " domain description for domain code:  " + domainCode.ToString() + " is: " + result.ToString());
                if (result != null)
                {
            ...
            where T : struct
        {
            ...
            var subtypes = (ISubtypes)selfClass;
        ...
        public static Dictionary<string, T> ToDictionary<T>(this IDomain domain, T defaultValue = default (T))
        {
            if (domain is ICodedValueDomain)
            {
                ...
                return codedValueDomain.CodeCount == 0
                           ...
                           : Enumerable.Range(0, codedValueDomain.CodeCount)
        ...
        #region Nested Types
        /// <summary>
        /// A facade for rows such that they can be released C# style.
        /// </summary>
