...
# ---------------------------------------------------------------------------
  ...
  # -------------------------------------------------------------------------
  strConnName = Config.get('Replica_Config', 'sourceFileConn')
  ...
  # -------------------------------------------------------------------------
  # print Running the Sync DataSet/Feature Class...
  # -------------------------------------------------------------------------
  import arcpy
  try:
    arcpy.env.workspace = strFileConn
    strReplicaName       = Config.get('Replica_Config', 'replica_name')
    strFileConnTo        = Config.get('Replica_Config', 'targetFileConn')
	
    print 'Sync Started'
    arcpy.SynchronizeChanges_management(strFileConn, strReplicaName, strFileConnTo, "FROM_GEODATABASE1_TO_2", "IN_FAVOR_OF_GDB1", "BY_OBJECT", "DO_NOT_RECONCILE")
    print 'Sync Successful'
  except Exception, ex:
          print "Error in Syncing Exception " + str(ex)
          intReturnCode = 1
