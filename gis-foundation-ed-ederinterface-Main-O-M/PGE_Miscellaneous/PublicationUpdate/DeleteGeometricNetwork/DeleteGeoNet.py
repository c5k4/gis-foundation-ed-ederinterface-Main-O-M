...
# ---------------------------------------------------------------------------
  ...
  mycmd = r'C:\Program Files (x86)\Miner and Miner\PG&E Custom Components\PGE.PasswordMgmt\PGE_DBPasswordManagement.exe'
  myarg = "GETSDEFILE" + " " + strConnName
  ...
  try:
    ...
    geoNetworks = Config.get('DB_Conn', 'geoNetworks')
    print 'Geometric Networks: ' + geoNetworks
    geometricNetworks = geoNetworks.split(",")
    for geoNetwork in geometricNetworks:
      print strFileConn + '\\' + geoNetwork
      arcpy.Delete_management((strFileConn + '\\' + geoNetwork), "GeometricNetwork")
      print geoNetwork + ' Deleted'
  except Exception, ex:
          print "Error in Deleting geometric network " + geoNetworks + " Exception " + str(ex)
          intReturnCode = 1
