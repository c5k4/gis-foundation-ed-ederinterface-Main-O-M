...
dbms_output.put_line('*****Checking Versioned Tables****** ');
  FOR table_info IN versioned_tables LOOP
    ...
    IF row_cnt = 1 then
        sqlstm := 'select sum(total) from ( select count(*) as total from '||table_info.owner||'.'||table_info.table_name||' where length(CIRCUITID)=8 union select count(*) as total from '||table_info.owner||'.A'||table_info.registration_id||' where length(CIRCUITID)=8 ) ';
        -- dbms_output.put_line(sqlstm);
        row_cnt := -1 ;
        execute immediate sqlstm into row_cnt ;               
        IF row_cnt>0 then 
            dbms_output.put_line('! ! ! ! ! CIRCUITID has len8 count of '||row_cnt||' for '||table_info.owner||'.'||table_info.table_name);
        END IF;
    END IF;
    ...
    IF row_cnt = 1 then
        sqlstm := 'select sum(total) from ( select count(*) as total from '||table_info.owner||'.'||table_info.table_name||' where length(CIRCUITID2)=8 union select count(*) as total from '||table_info.owner||'.A'||table_info.registration_id||' where length(CIRCUITID2)=8 ) ';
        -- dbms_output.put_line(sqlstm);
        row_cnt := -1 ;
        execute immediate sqlstm into row_cnt ;               
        IF row_cnt>0 then 
            dbms_output.put_line('! ! ! ! ! CIRCUITID2 has len8 count of '||row_cnt||' for '||table_info.owner||'.'||table_info.table_name);
        END IF;
    END IF;
  END LOOP;
dbms_output.put_line('*****Checking Non-Versioned Tables****** ');  
  FOR table_info IN non_versioned_tables LOOP
    SELECT COUNT(*) INTO row_cnt FROM sde.column_registry where TABLE_NAME = table_info.table_name and OWNER=table_info.owner and column_name='CIRCUITID';
    IF row_cnt = 1 then
        sqlstm := 'select count(*)  from '||table_info.owner||'.'||table_info.table_name||' where length(CIRCUITID)=8 ';
        -- dbms_output.put_line(sqlstm);
        row_cnt := -1 ;
        execute immediate sqlstm into row_cnt ;               
        IF row_cnt>0 then 
	...
	SELECT COUNT(*) INTO row_cnt FROM sde.column_registry where TABLE_NAME = table_info.table_name and OWNER=table_info.owner and column_name='CIRCUITID2';
    IF row_cnt = 1 then
        sqlstm := 'select count(*)  from '||table_info.owner||'.'||table_info.table_name||' where length(CIRCUITID2)=8 ';
        -- dbms_output.put_line(sqlstm);
        row_cnt := -1 ;
        execute immediate sqlstm into row_cnt ;               
        IF row_cnt>0 then 
