...
namespace PGE.BatchApplication.GDBMAHBatchJobs
{
    public  class GeoDBHelper
    {
        ...
        public  IWorkspace GetWorkspace(string argStrworkSpaceConnectionstring)
        {
            ...
            try
            {
                CommonFuntions.WriteLine_Info("Connection string used in process ." + ReadConfigurations.GetValue(ReadConfigurations.SDEConnectionString));
                ...
                if (workspace == null)
                {
                    CommonFuntions.WriteLine_Error("Workspace not found for conn string : " + ReadConfigurations.GetValue(ReadConfigurations.SDEConnectionString));
        ...
        public  bool CreateArcFMConnection( )
        {
            ...
            try
            {
                ...
                string smDB = ConnectionInfo[0].Split('=')[1];
                string smUser = ConnectionInfo[1].Split('=')[1];
                string smPwd = ConnectionInfo[2].Split('=')[1];
        ...
        internal  IWorkspace CreateVersion(out int sessionid)
        {
            sessionid = -1;
        ...
        public void SubmitSessionToGDBM(IWorkspace pworkspace, int SessionID)
        {
            ...
            try
            {
                ...
                if (RowExist == false)
                {
                    CommonFuntions.WriteLine_Info("Send Session to GDBM Queue started with SessionName = SN_" + SessionID.ToString() + " ,at,");
                    ...
                    Miner.Interop.Process.IMMPxSubtask subtask = new SetPostingPriority() ;
                    ...
                    SetNodeState(MainClass.m_pxApplication, node,Convert.ToInt32(ReadConfigurations.POST_QUEUE_STATE));
                    InsertSessionIntoPostingQueue(_mmSession, pworkspace);
                    CommonFuntions.WriteLine_Info("Send Session to GDBM Queue Completed  " + " at,");
                }
            }
            catch (Exception ex)
            {
