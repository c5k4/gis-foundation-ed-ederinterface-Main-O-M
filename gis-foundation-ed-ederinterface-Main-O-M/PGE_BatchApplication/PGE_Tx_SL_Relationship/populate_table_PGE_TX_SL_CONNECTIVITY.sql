...
BEGIN
	...
	sqlstmt := 'select count(*) from cat where table_name=''PGE_TX_SL_CONNECTIVITY'' and table_type=''TABLE''';
	execute immediate sqlstmt into rowcnt;
	if rowcnt>0 then
		sqlstmt:= 'delete from EDGIS.PGE_TX_SL_CONNECTIVITY';
		execute immediate sqlstmt;
	END IF;
	
	--Create the TX_SL_CONNECTIVITY table
	-- sqlstmt := 'create table EDGIS.PGE_TX_SL_CONNECTIVITY (FEEDERID VARCHAR2(32), TRANSFORMEROID number(38), SERVICELOCATIONOID number(38), TRANSFORMER VARCHAR2(38), SERVICELOCATION VARCHAR2(38))';
	-- execute immediate sqlstmt;
