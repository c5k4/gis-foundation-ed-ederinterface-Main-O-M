...
namespace PGE.BatchApplication.PGE_SSD_Initialization
{
    public class OracleConnectionControl
    {
        ...
        {
            string connection = connection = String.Format("Data Source=(DESCRIPTION=(ADDRESS_LIST=(ADDRESS=(PROTOCOL=TCP)(HOST={0})(PORT=1521)))(CONNECT_DATA=(SERVER=DEDICATED)(SERVICE_NAME={1})));", server, sid);
        ...
        public bool InsertCircuits(List<string> circuits, CircuitType circuitType)
        {
            ...
            try
            {
                ...
                using (var cmd = new OracleCommand("LOCK TABLE EDGIS.PGE.BatchApplication.PGE_SSD_Initialization IN EXCLUSIVE MODE", OracleConn))
                {
                    cmd.Transaction = tr;
                    cmd.ExecuteNonQuery();
                    cmd.Dispose();
        ...
        {
            int rowsChanged = 0;
            string insertsql = "INSERT into EDGIS.PGE.BatchApplication.PGE_SSD_Initialization" +
        ...
        {
            OracleTransaction tr = null;
            string sql = "UPDATE EDGIS.PGE.BatchApplication.PGE_SSD_Initialization SET error = '" + error + "',CircuitStatus = " + (int)circuitStatus + " where FEEDERID = '" + feederID + "'";
            try
            {
                OpenConnection();
                tr = OracleConn.BeginTransaction();
            ...
            {
                _logger.Error("Error updating circuit status in EDGIS.PGE.BatchApplication.PGE_SSD_Initialization table.", ex);
                if (tr != null) { tr.Rollback(); }
                throw ex;
            }
            finally
        ...
        public bool CanProcessCircuit(string feederID)
        {
            ...
            OracleTransaction tr = null;
            string sql = "SELECT CIRCUITSTATUS FROM EDGIS.PGE.BatchApplication.PGE_SSD_Initialization where FEEDERID = '" + feederID + "'";
            try
            {
                OpenConnection();
                tr = OracleConn.BeginTransaction();
                ...
                {
                    _logger.Error("Error locking table EDGIS.PGE.BatchApplication.PGE_SSD_Initialization.", ex);
                    throw ex;
                }
                ...
                {
                    _logger.Error("Error determining the current status of circuit from EDGIS.PGE.BatchApplication.PGE_SSD_Initialization. SQL: " + sql, ex);
                    throw ex;
                }
                ...
                try
                {
                    if (!isInProgress)
                    {
                        sql = "UPDATE EDGIS.PGE.BatchApplication.PGE_SSD_Initialization SET CIRCUITSTATUS = " + ((int)CircuitStatus.InProgress) + " where" +
                            " FEEDERID = '" + feederID + "'";
                        using (var cmd = new OracleCommand(sql, OracleConn))
                        {
                            cmd.Transaction = tr;
                ...
                {
                    _logger.Error("Error updating EDGIS.PGE.BatchApplication.PGE_SSD_Initialization with circuits to process. SQL: " + sql, ex);
                    throw ex;
                }
                tr.Commit();
            }
            ...
            {
                if (tr != null) { tr.Rollback(); }
                _logger.Error("Error determining if circuit can be processed from EDGIS.PGE.BatchApplication.PGE_SSD_Initialization.", ex);
                throw ex;
            }
            finally
            {
        ...
        public List<string> GetNextCircuitToProcess(CircuitType circuitType, int count)
        {
            ...
            OracleTransaction tr = null;
            string sql = "SELECT FEEDERID FROM EDGIS.PGE.BatchApplication.PGE_SSD_Initialization where CIRCUITSTATUS IS NULL AND CIRCUITTYPE = " + (int)circuitType;
            try
            {
                OpenConnection();
                tr = OracleConn.BeginTransaction();
                ...
                {
                    _logger.Error("Error obtaining list of circuits from EDGIS.PGE.BatchApplication.PGE_SSD_Initialization. SQL: " + sql, ex);
                    throw ex;
                }
            ...
            {
                if (tr != null) { tr.Rollback(); }
                _logger.Error("Error reading next circuit to process record from EDGIS.PGE.BatchApplication.PGE_SSD_Initialization.", ex);
                throw ex;
            }
            finally
            {
        ...
        public List<string> GetUltimateSourceCircuitsLeft()
        {
            ...
            {
                OpenConnection();
                selectCmd = "select FeederID from EDGIS.PGE.BatchApplication.PGE_SSD_Initialization where CIRCUITSTATUS <> " + (int)CircuitStatus.Finished + " AND CIRCUITTYPE = " + (int)CircuitType.UltimateSource;
        ...
        public List<string> GetSubstationCircuitsLeft()
        {
            ...
            {
                OpenConnection();
                selectCmd = "select FeederID from EDGIS.PGE.BatchApplication.PGE_SSD_Initialization where CIRCUITSTATUS <> " + (int)CircuitStatus.Finished + " AND CIRCUITTYPE = " + (int)CircuitType.Substation;
                using (var cmd = new OracleCommand(selectCmd, OracleConn))
                {
                    cmd.CommandType = CommandType.Text;
                    OracleDataReader reader = cmd.ExecuteReader();
        ...
        public List<string> GetCircuitsTraced()
        {
            ...
            {
                OpenConnection();
                string selectCmd = "select FeederID from EDGIS.PGE.BatchApplication.PGE_SSD_Initialization where CIRCUITSTATUS = " + (int)CircuitStatus.Finished;
                using (var cmd = new OracleCommand(selectCmd, OracleConn))
                {
                    cmd.CommandType = CommandType.Text;
