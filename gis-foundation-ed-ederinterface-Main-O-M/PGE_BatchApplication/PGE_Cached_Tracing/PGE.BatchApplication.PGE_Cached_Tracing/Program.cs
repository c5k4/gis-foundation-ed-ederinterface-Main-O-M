...
namespace PGE.BatchApplication.PGE_Cached_Tracing
{
    class Program
    {
        private static LicenseInitializer m_AOLicenseInitializer = new PGE.BatchApplication.PGE_Cached_Tracing.LicenseInitializer();
            ...
            try
            {
                ...
                else
                {
                    ...
                    if (!string.IsNullOrEmpty(Common.EDSDEConnectionFile) && !string.IsNullOrEmpty(Common.SUBSDEConnectionFile))
                    {
                        ...
                        if (licenseCheckoutSuccess)
                        {
                            if (isChild)
                            {
                                ...
                                PGE_Cache_Tracing cache_Tracing = new PGE_Cache_Tracing();
                    ...
                    else
                    {
                        ...
                        Console.WriteLine("Invalid configuration.  Please check the PGE.BatchApplication.PGE_Cached_Tracing.exe.config file" +
