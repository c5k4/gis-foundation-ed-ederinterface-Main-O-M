...
namespace PGE.BatchApplication.GISViewUpdatePLC
{
    public class TableData : IDisposable
    {
        ...
        /// </summary>
        protected IFeatureClass _table = null;
        /// <summary>
       ...
       // protected log4net.ILog _logger = LogManager.GetLogger(System.Reflection.MethodBase.GetCurrentMethod().DeclaringType);
        ...
        /// </summary>
        public virtual string PLDBFieldName
        {
            get;
            set;
        }
        ...
        public virtual IList<IRowData2> SearchTable(string whereClause, string[] fieldOrder)
        {
            ...
            IFeatureCursor cursor = _table.Search(filter, true);
        ...
        /// </summary>
        public virtual Dictionary<string,string> LoadCache()
        {
            _jobOrders = new List<string>();
            Dictionary<string, string> dictPLdbiD = new Dictionary<string, string>();
            IQueryFilter filter = new QueryFilterClass();
            filter.WhereClause = PLDBFieldName + " is not null";
            IFeatureCursor cursor = _table.Search(filter, true);
            int pldbIndex = _table.FindField(PLDBFieldName);

            IRow row = null;
            ulong counter = 0;
            while ((row = cursor.NextFeature()) != null)
            {
                _jobOrders.Add(row.get_Value(pldbIndex).ToString());
                dictPLdbiD.Add("PLDBID" + counter, row.get_Value(pldbIndex).ToString());
                counter++;
            }
            Marshal.ReleaseComObject(cursor);
            return dictPLdbiD;
        ...
        protected IFeatureClass GetTable(IWorkspace wSpace, string tableName)
        {
        ...
        private IDictionary<string, int> PrepareFieldIndexDictionary(string[] fieldNames, IFeatureClass table)
        {
