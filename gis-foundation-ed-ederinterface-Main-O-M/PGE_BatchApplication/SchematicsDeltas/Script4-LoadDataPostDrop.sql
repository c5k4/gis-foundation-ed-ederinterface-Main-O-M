...
declare
   ...
   and column_name not in ('UCID','UOID','USID','SHAPE','UGUID','OID') and data_type not in ('BLOB');
begin
--- Truncate table and reset sequence.
   open cur_reg ('SDE','SCHEM_GUID_POST_DROP_TEMP');
   fetch cur_reg into v_myRegID;
   close cur_reg;
   open cur_rowid ('SDE','SCHEM_GUID_POST_DROP_TEMP');
   fetch cur_rowid into v_myRowID;
   close cur_rowid;
   execute immediate 'TRUNCATE TABLE SDE.SCHEM_GUID_POST_DROP_TEMP';
   --execute immediate 'ALTER SEQUENCE SDE.R'||v_MyRegID||' minvalue 0';
-- Insert data for each table
   for c_schematics in cur_schematics loop
       for c_tables in cur_tables(c_schematics.owner,'SCH'||c_schematics.id||'E_%') loop
           ...
           v_str:='select SDE.R'||v_myRegID||'.NEXTVAL,'||''''||c_tables.owner||''''||','||''''||c_tables.table_name||''''||','||v_regid;
           v_str:=v_str||',A.'||v_rowid ||',A.UGUID,A.USID,F.EMINX,F.EMINY,F.EMAXX,F.EMAXY ';
           ...
           if length(v_columns) > 0 then
              v_sql:= 'insert into SDE.SCHEM_GUID_POST_DROP_TEMP (OID,OWNER,SCH_TABLE,UCID,UOID,UGUID,USID,EMINX,EMINY,EMAXX,EMAXY,'||v_columns2||') '||v_str||' and ISDISPLAYED<>0';
              dbms_output.put_line (v_sql);
              execute immediate v_sql;
           else
			  execute immediate 'insert into SDE.SCHEM_GUID_POST_DROP_TEMP (OID,OWNER,SCH_TABLE,UCID,UOID,UGUID,USID,EMINX,EMINY,EMAXX,EMAXY) '||v_str||' and ISDISPLAYED <>0';
           end if;
       end loop;
       commit;
   end loop;
