...
begin
   execute immediate 'select count(*) from all_tables where owner = ''SDE'' and table_name = ''SCHEM_GUID_POST_DROP_TEMP'' ' into v_cnt;
   if v_cnt > 0 then
      execute immediate 'drop table SDE.SCHEM_GUID_POST_DROP_TEMP';
   end if;
   execute immediate 'create table SDE.SCHEM_GUID_POST_DROP_TEMP (OID number(38), owner nvarchar2(160),SCH_TABLE NVARCHAR2(160),UCID NUMBER(38),UOID NUMBER(38), UGUID NVARCHAR2(38),USID NUMBER(38), EMINX FLOAT(64),EMINY FLOAT(64),EMAXX FLOAT(64),EMAXY FLOAT(64))';
   v_str:=' ';
       ...
       for c_tables in cur_tables(c_schematics.owner,'SCH'||c_schematics.id||'E_%') loop
           ...
           for c_columns in cur_columns (c_tables.owner,c_tables.table_name) loop
                   if instr(v_str,'~'||c_columns.column_name||'~') = 0 then
                      ...
                      v_sql:='alter table SDE.SCHEM_GUID_POST_DROP_TEMP add ('||c_columns.column_name||' '||c_columns.data_type||'('||c_columns.data_length||'))';    
