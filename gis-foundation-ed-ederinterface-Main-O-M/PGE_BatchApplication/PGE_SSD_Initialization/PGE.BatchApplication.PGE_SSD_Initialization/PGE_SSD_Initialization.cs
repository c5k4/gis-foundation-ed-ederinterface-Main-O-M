...
{
    public class SSD_Initialization
    {
        ...
        private void ExecuteTracingInChildren()
        {
            for (int i = 0; i < Common.numProcessors; i++)
            {
                ...
                AssemblyLocation = AssemblyLocation.Substring(0, AssemblyLocation.LastIndexOf("\\") + 1) + "PGE.BatchApplication.SSD_Initialization.exe";
        ...
        private void TraceForSSDs(IGeometricNetwork geomNetwork, string feederID)
        {
            ...
            try
            {
                ...
                if ((feederSource) != null)
                {
                    ...
                    Dictionary<int, object> SSDGuids = new Dictionary<int, object>();
                    ...
                    GetSSDLists(ref SSDs, ref SSDGuids, ref ProtectiveSSDs, ref AutoProtectiveSSDs, feederID);
                    ...
                    GetUpdateStatement(ref ObjClassEidToOID, ref SSDToJunctions, ref ProtectiveSSDToJunctions, ref AutoProtectiveSSDToJunctions,
                        ref SSDs, ref SSDGuids, ref ProtectiveSSDs, ref AutoProtectiveSSDs, ref updateStatements);
                    ...
                    foreach (string updateStatement in updateStatements)
                    {
                        ...
                        {
                            _logger.Error("Error executing SQL to set the SSD. Message: " + ex.Message + " StackTrace: " + ex.StackTrace);
                        }
                    }
        ...
        private void GetUpdateStatement(ref Dictionary<IObjectClass, Dictionary<int, int>> ObjClassEidToOIDMap, ref Dictionary<int, List<int>> SSDsToJunctions,
            ...
            ref Dictionary<int, object> SSDsList, ref Dictionary<int, object> SSDGUIDsList, 
            ref Dictionary<int, object> ProtectiveSSDsList, ref Dictionary<int, object> AutoProtectiveSSDsList,
            ref List<string> updateStatements)
        {
            foreach (KeyValuePair<IObjectClass, Dictionary<int, int>> kvp in ObjClassEidToOIDMap)
            {
                ...
                string SSDGUIDFieldName = GetFieldName(kvp.Key, SSDGUID);
                if (string.IsNullOrEmpty(SSDGUIDFieldName)) { continue; }
                foreach (KeyValuePair<int, List<int>> kvp2 in SSDsToJunctions)
                {
                    ...
                    object operatingNumber = SSDsList[kvp2.Key];
                    object ssdGUID = SSDGUIDsList[kvp2.Key];
                    string updateStmt = updateStmt1 + " SET " + SSDFieldName + " = '" + operatingNumber + "', " + SSDGUIDFieldName + " = '" + ssdGUID + "' WHERE " + oidfieldName + " in (";
                ...
                foreach (KeyValuePair<int, List<int>> kvp2 in ProtectiveSSDsToJunctions)
                {
                    ...
                    string updateStmt = updateStmt1 + " SET " + SSDFieldName + " = '" + operatingNumber + "' WHERE " + oidfieldName + " in (";
                    string origUpdateStmt = updateStmt;
                    int counter = 0;
                    for (int i = 0; i < EIDsToProcess.Count; i++)
                    {
        ...
        private void GetSSDLists(ref Dictionary<int, object> SSDs, ref Dictionary<int, object> SSDGuids, ref Dictionary<int, object> ProtectiveSSDs,
            ref Dictionary<int, object> AutoProtectiveSSDs, string circuitID)
        {
            foreach (IFeatureClass featClass in SSDClasses)
            {
                ...
                if (SSDClassIDToWhereClause.ContainsKey(featClass.ObjectClassID))
                {
                    ...
                    QueryTableForSSDGUIDs(featClass, whereClause, ref SSDGuids);
                }
                //Get Protective SSDs
        ...
        private void GetSSDClasses(IGeometricNetwork geomNetwork)
        {
            ...
            while ((featClass = featureClasses.Next()) != null)
            {
                ...
                if (hasSSDId && hasProtectiveSSDId && hasAutoProtectiveSSDId)
                {
        ...
        private void QueryTableForSSDGUIDs(IFeatureClass featClass, string whereClause, ref Dictionary<int, object> ToUpdate)
        {
            IQueryFilter qf = new QueryFilterClass();
            qf.WhereClause = whereClause;
            IFeatureCursor featCursor = featClass.Search(qf, false);
            IFeature feat = null;
            int opNumFieldIdx = featClass.FindField("GLOBALID");
