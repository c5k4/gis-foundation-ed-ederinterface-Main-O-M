...
{
    public static class TraceFacade
    {
        #region Enums
        /// <summary>
        /// 
        ...
        public static List<IFeature> GetFirstDownstreamFeatureOfType(IFeature sourceFeature, esriElementType returnElementType, string ignoreModelName = null)
        {
            ...
            DownStreamTrace(startEID, GetElementType(sourceFeature), geomNetwork, null,null,out tracedJunctions, out tracedEdges);
        ...
        public static void DownStreamTrace(int startEID, esriElementType startElementType, IGeometricNetwork geomNetwork, int[] barrierJuncs, int[] barrierEdges, out IMMTracedElements tracedJunctions, out IMMTracedElements tracedEdges)
        {
            if (barrierJuncs == null) { barrierJuncs = new int[0];}
            if (barrierEdges == null) { barrierEdges = new int[0]; }
            ...
            mmElectricTracing.TraceDownstream(
                ...
                null, //The class that implements IMMCurrentStatus

                startEID, //The EID of where the trace should start
                startElementType,  //What kind of EID is startEID
        ...
        public static void UpstreamDownstreamTrace(IGeometricNetwork inGeomNetwork, int startElement, esriElementType elementType, 
            mmPhasesToTrace phasesToTrace, IDictionary<int, IList<int>> BarrierIds, IDictionary<int, esriElementType> BarrierTypes, out IEnumNetEID juncEIDs, out IEnumNetEID edgeEIDs)
        {
            ////////// create an instance of the Feeder Manager class
            ////////// that performs electric tracing functions
            juncEIDs = null; edgeEIDs = null;
            if (elementType == esriElementType.esriETNone || elementType == esriElementType.esriETTurn) return;


            IMMNetworkAnalysisExtForFramework netAnalExt = null;
            ...
            try
            {
                ...
                netAnalExt = new MMNetworkAnalysisExtForFrameworkClass();
                if (BarrierIds != null && BarrierTypes != null && BarrierIds.Count > 0)
                {
                    try
                    {
                        ...
                        if (edgeFeatureIds.Count > 0) netAnalExt.EdgeBarriers = edgeFeatureIds.ToArray<IMMNetworkFeatureID>();
                        if (juncFeatureIds.Count > 0) netAnalExt.JunctionBarriers = juncFeatureIds.ToArray<IMMNetworkFeatureID>();
                    }
                    catch (Exception ex)
                    {
                       // LogUtility.LogExcpetion(ex);
               ...
               elecNetTracing.TraceDownstream(inGeomNetwork, netAnalExt, elecTraceSettingsEx, startElement, elementType, phasesToTrace, out juncEIDs, out edgeEIDs);
            ...
            finally
            {
                ...
                if (netAnalExt != null)
                {
                    System.Runtime.InteropServices.Marshal.ReleaseComObject(netAnalExt);
                    netAnalExt = null;
                }
