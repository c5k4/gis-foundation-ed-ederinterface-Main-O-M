Prompt drop Trigger SM_SUBTRF_BANK_LOAD_INS;
DROP TRIGGER EDSETT.SM_SUBTRF_BANK_LOAD_INS
/

Prompt Trigger SM_SUBTRF_BANK_LOAD_INS;
--
-- SM_SUBTRF_BANK_LOAD_INS  (Trigger) 
--
CREATE OR REPLACE TRIGGER EDSETT.SM_SUBTRF_BANK_LOAD_INS
BEFORE INSERT ON EDSETT.SM_SUBTRF_BANK_LOAD
FOR EACH ROW
BEGIN
  <<COLUMN_SEQUENCES>>
  ...
  BEGIN
    IF INSERTING AND :NEW.ID IS NULL THEN
      SELECT SM_SUBTRF_BANK_LOAD_SEQ.NEXTVAL INTO :NEW.ID FROM SYS.DUAL;
    END IF;
