Prompt drop Procedure RUN_MONTHLY_LOAD;
DROP PROCEDURE EDTLM.RUN_MONTHLY_LOAD
/

Prompt Procedure RUN_MONTHLY_LOAD;
--
-- RUN_MONTHLY_LOAD  (Procedure) 
--
CREATE OR REPLACE PROCEDURE EDTLM."RUN_MONTHLY_LOAD" (
    ...
    -- Disable custom constraints before deleting old records
      ...
      -- Enable custom constraints after deleting old records
      ...
      -- Disable custom constraints before deleting 38 months old records
      ...
      -- Delete records older than 38 motnhs
        DELETE FROM SP_PEAK_GEN_HIST WHERE TRF_PEAK_GEN_HIST_ID IN (SELECT ID FROM TRF_PEAK_GEN_HIST WHERE BATCH_DATE = (SELECT ADD_MONTHS(MAX(BATCH_DATE),-38) FROM TRF_PEAK_GEN_HIST));
        DELETE FROM TRF_BANK_PEAK_GEN_HIST WHERE TRF_PEAK_GEN_HIST_ID IN (SELECT ID FROM TRF_PEAK_GEN_HIST WHERE BATCH_DATE = (SELECT ADD_MONTHS(MAX(BATCH_DATE),-38) FROM TRF_PEAK_GEN_HIST));
        DELETE FROM  TRF_PEAK_GEN_HIST WHERE BATCH_DATE = (SELECT ADD_MONTHS(MAX(BATCH_DATE),-38) FROM TRF_PEAK_GEN_HIST);
        DELETE FROM SP_PEAK_HIST WHERE TRF_PEAK_HIST_ID IN (SELECT ID FROM TRF_PEAK_HIST WHERE BATCH_DATE = (SELECT ADD_MONTHS(MAX(BATCH_DATE),-38) FROM TRF_PEAK_HIST));
        DELETE FROM TRF_BANK_PEAK_HIST WHERE TRF_PEAK_HIST_ID IN (SELECT ID FROM TRF_PEAK_HIST WHERE BATCH_DATE = (SELECT ADD_MONTHS(MAX(BATCH_DATE),-38) FROM TRF_PEAK_HIST));
        DELETE FROM  TRF_PEAK_HIST WHERE BATCH_DATE = (SELECT ADD_MONTHS(MAX(BATCH_DATE),-38) FROM TRF_PEAK_HIST);


      -- Enable custom constraints after deleting 38 months old records
        EXECUTE IMMEDIATE 'ALTER TABLE TRF_PEAK_GEN_HIST ENABLE CONSTRAINT TRF_PEAK_GEN_HIST_FK1';
        EXECUTE IMMEDIATE 'ALTER TABLE TRF_PEAK_GEN_HIST ENABLE CONSTRAINT TRF_PEAK_GEN_HIST_PK';
        EXECUTE IMMEDIATE 'ALTER TABLE TRF_BANK_PEAK_GEN_HIST ENABLE CONSTRAINT TRF_BANK_PEAK_GEN_HIST_FK2';
        EXECUTE IMMEDIATE 'ALTER TABLE TRF_BANK_PEAK_GEN_HIST ENABLE CONSTRAINT TRF_BANK_PEAK_GEN_HIST_PK';
