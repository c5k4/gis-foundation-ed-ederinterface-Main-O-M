Prompt drop View PGE_COUNT_CIRCUITBREAKER;
DROP VIEW EDGIS.PGE_COUNT_CIRCUITBREAKER
/

/* Formatted on 7/2/2019 01:18:05 PM (QP5 v5.313) */
PROMPT View PGE_COUNT_CIRCUITBREAKER;
--
-- PGE_COUNT_CIRCUITBREAKER  (View)
--

CREATE OR REPLACE FORCE VIEW EDGIS.PGE_COUNT_CIRCUITBREAKER
(
    FEEDERID,
    GLOBALID,
    COUNT
    ...
    (  SELECT source.feederid          AS FEEDERID,
         ...
         FROM (SELECT MIN_BRANCH,
                ...
                WHERE TO_FEATURE_FCID =
                      (SELECT OBJECTID
                         FROM sde.gdb_items
                        WHERE     PHYSICALNAME = 'EDGIS.ELECTRICSTITCHPOINT'
                              AND TYPE IN
                                      (SELECT UUID
...
Prompt Grants on VIEW PGE_COUNT_CIRCUITBREAKER TO BO_USER to BO_USER;
GRANT SELECT ON EDGIS.PGE_COUNT_CIRCUITBREAKER TO BO_USER
/
