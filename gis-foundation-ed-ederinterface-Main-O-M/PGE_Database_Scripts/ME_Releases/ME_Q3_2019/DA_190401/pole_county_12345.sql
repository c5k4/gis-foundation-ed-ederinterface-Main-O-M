spool "D:\DQI_Scripts\Logs\SQL_POLE12345_COUNTY.txt"
SET SERVEROUTPUT ON
--set serveroutput on size 100000;
prompt Updating POLE intersect for county objectid like 12345  ;
...
BEGIN
  ...
  exception when others then
    ...
    DBMS_OUTPUT.PUT_LINE ('Error in Pole County update where error is '  || v_code || ' ' || v_errm || ' for sap_equip_id' || cur.sap_equip_id );
    continue;
     end;
...
DBMS_OUTPUT.PUT_LINE(' Process Completed Sucessfully for ' || counter1 || ' records for county pole' );
...
/
prompt Updating POLE intersect FOR county and  objectid like 12345 is complete;
select current_timestamp from dual;
spool off;