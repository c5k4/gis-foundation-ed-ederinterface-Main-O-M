spool D:\DA_58_Scripts\Logs\SQL_14.txt
set serveroutput on
prompt Updating SERVICELOCATION ending with Objectid 6,7;
select current_timestamp from dual;

DECLARE
  x NUMBER := 0;  
  counter NUMBER := 0;
  cursor c1 is select * from edgis.SERVICELOCATION where objectid like ('%6') OR objectid like ('%7');
...
prompt Updating VOLTAGEREGULATOR;
select current_timestamp from dual;

DECLARE
  x NUMBER := 0;  
  counter NUMBER := 0;
  cursor c1 is select * from edgis.VOLTAGEREGULATOR;
  cursor c2 is select * from a116;
BEGIN
  For rw in c1
  LOOP
  BEGIN
    x := rw.objectid;    
     update edgis.VOLTAGEREGULATOR op set op.city  = (select tba.CITY_NAME 
     from edgis.city tba
     where  sde.st_intersects(tba.shape,rw.shape)  = 1 and rownum<2),
     op.county = ( select cnty.code from edgis.countyname_lookup cnty where county_name in 
     (select clp.cnty_name 
  ...
  BEGIN
    x := rw.objectid;    
     update edgis.a116 op set op.city  = (select tba.CITY_NAME 
     from edgis.city tba
     where  sde.st_intersects(tba.shape,rw.shape)  = 1 and rownum<2),
     op.county = ( select cnty.code from edgis.countyname_lookup cnty where county_name in 
     (select clp.cnty_name 
...
END;
/
prompt VOLTAGEREGULATOR update complete;
select current_timestamp from dual;
