spool D:\DA_58_Scripts\Logs\SQL_15.txt
set serveroutput on
prompt Updating SERVICELOCATION ending with Objectid 8,9;
select current_timestamp from dual;

DECLARE
  x NUMBER := 0;  
  counter NUMBER := 0;
  cursor c1 is select * from edgis.SERVICELOCATION where objectid like ('%8') OR objectid like ('%9');
  cursor c2 is select * from a128;
BEGIN
  For rw in c1
  LOOP
  BEGIN
  ...
  BEGIN
    x := rw.objectid;    
     update edgis.a128 op set op.city  = (select tba.CITY_NAME 
     from edgis.city tba
     where  sde.st_intersects(tba.shape,rw.shape)  = 1 and rownum<2),
     op.county = ( select cnty.code from edgis.countyname_lookup cnty where county_name in 
     (select clp.cnty_name 
