spool D:\DA_58_Scripts\Logs\SQL_8.txt
set serveroutput on
prompt Updating SUPPORTSTRUCTURE ending with Objecid 0,1,2;
select current_timestamp from dual;

DECLARE
  x NUMBER := 0;  
  counter NUMBER := 0;
  cursor c1 is select * from edgis.SUPPORTSTRUCTURE where objectid like ('%0') OR objectid like ('%1') OR objectid like ('%2');
  cursor c2 is select * from a144;
BEGIN
  For rw in c1
  LOOP
  BEGIN
  ...
  BEGIN
    x := rw.objectid;    
     update edgis.a144 op set op.city  = (select tba.CITY_NAME 
     from edgis.city tba
     where  sde.st_intersects(tba.shape,rw.shape)  = 1 and rownum<2),
     op.county = ( select cnty.code from edgis.countyname_lookup cnty where county_name in 
     (select clp.cnty_name 
