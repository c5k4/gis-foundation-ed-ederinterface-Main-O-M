...
BEGIN 
for rec in (SELECT username FROM dba_users WHERE username  in  ('EDGIS','EDGISBO','PROCESS','CUSTOMER','PGEDATA')
		    ) loop	
...
SELECT COUNT(1) INTO v_count 
       FROM dba_users WHERE username in  ('EDGIS','SDE','EDGISBO','PROCESS','CUSTOMER','PGEDATA');
     IF v_count != 0 
     THEN
         v_count := -1;
...
echo
command="impdp $account_uc4admin/$password_uc4admin@$ORACLE_SID schemas=sde,edgis,process,edgisbo,CUSTOMER,PGEDATA  directory=UC4_IMPORT_DIR dumpfile=$filename_import_parallel\%U.dmp logfile=$filename_import_log CLUSTER=N PARALLEL=4"
comecho="impdp $account_uc4admin/ password_uc4admin@$ORACLE_SID schemas=sde,edgis,process,edgisbo,CUSTOMER,PGEDATA  directory=UC4_IMPORT_DIR dumpfile=$filename_import_parallel\%U.dmp logfile=$filename_import_log CLUSTER=N PARALLEL=4"
