#!/bin/sh
#============================================================================
# EDG_Copy_WIP_to_DataMart_DC1.sh
#----------------------------------------------------------------------------
...
if [ "$machine" == 'edgisdboraprd01' ] ; then
   echo  " Removing $oracle_sid dump file from edgisdboraprd05 .." >> $file_log_tmp 2>&1
   echo  " Removing $oracle_sid dump file from edgisdboraprd05 .." >> $file_log 2>&1
   ssh -q edgisdboraprd05 'rm /u02/uc4/eddatm1p/import/$oracle_sid.dmp' >> $file_log_tmp 2>&1
else
...
if [ "$machine" == 'edgisdboraprd01' ] ; then
   echo  " Copying $oracle_sid dump file to edgisdboraprd05 .." >> $file_log_tmp 2>&1
   echo  " Copying $oracle_sid dump file to edgisdboraprd05 .." >> $file_log 2>&1
   scp  /u02/uc4/$oracle_sid/export/$oracle_sid.dmp  edgisdboraprd05:/u02/uc4/eddatm1p/import >> $file_log_tmp 2>&1
else
   if [ "$machine" == 'edgisdboraqa01' ] ; then
