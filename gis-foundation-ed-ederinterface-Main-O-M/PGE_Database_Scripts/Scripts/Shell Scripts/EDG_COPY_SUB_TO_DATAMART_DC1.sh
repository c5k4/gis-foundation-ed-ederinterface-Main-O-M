#!/bin/sh
#============================================================================
# EDG_Copy_SUB_to_DataMart_DC1.sh
#----------------------------------------------------------------------------
# USAGE:        EDG_Copy_SUB_to_DataMart_DC1.sh
...
# Copy dump file from 
#----------------------------------------------------------------------------
echo " Copying edsubp1p dump file to edgisdboraprd05 .." >> $file_log_tmp 2>&1
echo " Copying edsubp1p dump file to edgisdboraprd05 .." >> $file_log 2>&1
echo `date +%Y_%m_%d_%H_%M_%S` >> $file_log_tmp 2>&1
echo `date +%Y_%m_%d_%H_%M_%S` >> $file_log 2>&1
scp   /u02/uc4/edsubp1p/export/edsubp1p.dmp  edgisdboraprd05:/u02/uc4/eddats1p/import >> $file_log_tmp 2>&1
echo `date +%Y_%m_%d_%H_%M_%S` >> $file_log_tmp 2>&1
echo `date +%Y_%m_%d_%H_%M_%S` >> $file_log 2>&1
int_error_code=`expr $int_error_code + $?`
