...
password_edtlm=`$file_script_get_pwd         $account_edtlm         $oracle_sid`
...
echo
command="impdp $account_uc4admin/$password_uc4admin@$ORACLE_SID schemas=edtlm directory=UC4_IMPORT_DIR dumpfile=$filename_import logfile=$filename_import_log"
comecho="impdp $account_uc4admin/ password_uc4admin@$ORACLE_SID schemas=edtlm directory=UC4_IMPORT_DIR dumpfile=$filename_import logfile=$filename_import_log"
...
set time on
set timing on
alter user $account_edtlm     identified by "$password_edtlm"     account unlock;
set wrap off
set echo off term off
set serveroutput off
set time off
