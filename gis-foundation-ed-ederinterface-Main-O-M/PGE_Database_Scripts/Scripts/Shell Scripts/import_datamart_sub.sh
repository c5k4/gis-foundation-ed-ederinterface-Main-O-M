...
echo
command="impdp $account_uc4admin/$password_uc4admin@$ORACLE_SID schemas=sde,edgis directory=UC4_IMPORT_DIR dumpfile=$filename_import logfile=$filename_import_log"
comecho="impdp $account_uc4admin/ password_uc4admin@$ORACLE_SID schemas=sde,edgis directory=UC4_IMPORT_DIR dumpfile=$filename_import logfile=$filename_import_log"
...
grant select on edgis.pge_subgeomnetwork_trace to mdssuser_ro;
grant select on edgis.zz_mv_subinterruptingdevice to mdssuser_ro;
set wrap off
set echo off term off
set serveroutput off
set time off
