#!/bin/sh
#============================================================================
# EDG_COPY_BATCH_TO_NAS_LOG_DR.sh
#----------------------------------------------------------------------------
# USAGE:        EDG_COPY_BATCH_TO_NAS_LOG_DR.sh
...
# Copy dump file from 
#----------------------------------------------------------------------------
echo " Copying edgisp4p dump log file to NAS Location .." >> $file_log_tmp 2>&1
echo " Copying edgisp4p dump log file to NAS Location .." >> $file_log 2>&1
echo `date +%Y_%m_%d_%H_%M_%S` >> $file_log 2>&1
echo `date +%Y_%m_%d_%H_%M_%S` >> $file_log_tmp 2>&1
scp  /u02/uc4/edgisp4p/export/edgisp4p.log  EDGISBTCPRD02:\\fairfield07\GIS\public\exta\EDGIS\edgisp4p.log >> $file_log_tmp 2>&1
echo `date +%Y_%m_%d_%H_%M_%S` >> $file_log 2>&1
echo `date +%Y_%m_%d_%H_%M_%S` >> $file_log_tmp 2>&1
int_error_code=`expr $int_error_code + $?`
