...
SELECT 'ALTER INDEX '||owner||'.'||index_name||' '||'REBUILD  parallel (degree $degree ) NOLOGGING;'FROM all_indexes WHERE owner ='$schema_name' and INDEX_TYPE = 'NORMAL' AND TEMPORARY='N' AND NUM_ROWS > 50000 ORDER BY owner, index_name;
spool off
...
SELECT 'ALTER INDEX '||owner||'.'||index_name||' '||'NOPARALLEL;'FROM all_indexes WHERE owner ='$schema_name' and INDEX_TYPE = 'NORMAL' AND TEMPORARY='N' AND NUM_ROWS > 50000 ORDER BY owner, index_name;
spool off
