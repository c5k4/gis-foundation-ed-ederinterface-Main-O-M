...
echo
command="impdp $account_uc4admin/$password_uc4admin@$ORACLE_SID schemas=sde,edgis,process directory=UC4_IMPORT_DIR dumpfile=$filename_import_parallel\%U.dmp logfile=$filename_import_log CLUSTER=N PARALLEL=4"
comecho="impdp $account_uc4admin/ password_uc4admin@$ORACLE_SID schemas=sde,edgis,process directory=UC4_IMPORT_DIR dumpfile=$filename_import_parallel\%U.dmp logfile=$filename_import_log CLUSTER=N PARALLEL=4"
...
grant select on edgis.pge_elecdistnetwork_trace to mdssuser_ro, mdmsuser_ro;
grant select on edgis.pge_feederfednetwork_trace to mdssuser_ro, mdmsuser_ro;
grant select on edsett.sm_circuit_breaker to mdssuser_ro;
set wrap off
set echo off term off
set serveroutput off
set time off
