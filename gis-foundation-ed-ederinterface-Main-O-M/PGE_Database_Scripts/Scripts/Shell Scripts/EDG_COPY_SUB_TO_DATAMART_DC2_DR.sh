#!/bin/sh
#============================================================================
# EDG_COPY_SUB_TO_DATAMART_DC2_DR
#----------------------------------------------------------------------------
# USAGE:        EDG_COPY_SUB_TO_DATAMART_DC2_DR
...
# Copy dump file from 
#----------------------------------------------------------------------------
echo " Copying edsubp4p dump file to edgisdboraprd06 .." >> $file_log_tmp 2>&1
echo " Copying edsubp4p dump file to edgisdboraprd06 .." >> $file_log 2>&1
echo `date +%Y_%m_%d_%H_%M_%S` >> $file_log_tmp 2>&1
echo `date +%Y_%m_%d_%H_%M_%S` >> $file_log 2>&1
scp   /u02/uc4/edsubp4p/export/edsubp4p.dmp  edgisdboraprd06:/u02/uc4/eddats2p/import >> $file_log_tmp 2>&1
echo `date +%Y_%m_%d_%H_%M_%S` >> $file_log_tmp 2>&1
echo `date +%Y_%m_%d_%H_%M_%S` >> $file_log 2>&1
int_error_code=`expr $int_error_code + $?`
