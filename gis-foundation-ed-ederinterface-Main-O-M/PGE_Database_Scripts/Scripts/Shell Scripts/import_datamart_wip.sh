...
# import.sh
#-----------------------------------------------------------------------------
# USAGE:        import_datamart_wip.sh oracle_sid oracle_sid_import datetime
#
# Returns:      0=Script completed action Successfully
# Returns:      1=Fail
#
# Author:       Vince Ulfig vulfig@gmail.com 4157101998
#               cloned Vince script for wip by Twyla
#=============================================================================
...
echo
command="impdp $account_uc4admin/$password_uc4admin@$ORACLE_SID schemas=webr directory=UC4_IMPORT_DIR dumpfile=$filename_import logfile=$filename_import_log"
comecho="impdp $account_uc4admin/ password_uc4admin@$ORACLE_SID schemas=webr directory=UC4_IMPORT_DIR dumpfile=$filename_import logfile=$filename_import_log"
...
grant select on WEBR.JET_JOBS to bo_user;
grant select on WEBR.JET_EQUIPMENT to bo_user;
grant select on WEBR.DELETEDWIP to bo_user;
grant select on WEBR.WIP  to bo_user;
grant select on WEBR.PGE_PMORDER  to bo_user;
CREATE VIEW WEBR.JET_VW_EQUIP 
AS 
SELECT OBJECTID,EQUIPTYPEID,JOBNUMBER,OPERATINGNUMBER,TO_NUMBER(CGC12) AS CGC12,
       SKETCHLOC,INSTALLCD,ADDRESS,CITY,LATITUDE,LONGITUDE,ENTRYDATE,
       LASTMODIFIEDDATE,USERAUDIT,CUSTOWNED,STATUS
...
echo
#  don't have sde's password, trying with uc4admin
...
FOR drop_keysets IN all_keysets LOOP
  ...
  sql_stmt := 'DROP TABLE '||drop_keysets.owner||'.'||drop_keysets.table_name;

  SELECT COUNT(*) INTO valid_sess 
    FROM sde.process_information 
   WHERE owner = drop_keysets.owner AND sde_id = sess_id;
