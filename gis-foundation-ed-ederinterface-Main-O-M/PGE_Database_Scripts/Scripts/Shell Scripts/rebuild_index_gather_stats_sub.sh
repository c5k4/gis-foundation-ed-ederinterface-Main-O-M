#!/bin/sh
#=============================================================================
# rebuild_index_gather_stats_sub.sh
#-----------------------------------------------------------------------------
...
SELECT 'ALTER INDEX '||owner||'.'||index_name||' '||'REBUILD ONLINE parallel (degree $degree ) NOLOGGING;'FROM all_indexes WHERE owner ='$schema_name' and INDEX_TYPE = 'NORMAL' AND TEMPORARY='N' AND NUM_ROWS > 50000 ORDER BY owner, index_name;
spool off
...
prompt exec DBMS_STATS.GATHER_SCHEMA_STATS('$schema_name',estimate_percent=>100,DEGREE=> $degree,CASCADE=>TRUE);
spool off
