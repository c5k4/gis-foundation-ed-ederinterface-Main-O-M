#!/bin/sh
#============================================================================
# EDG_Copy_Pub_DC1_Sub_to_Pub_DC2.sh
#----------------------------------------------------------------------------
# USAGE:        EDG_Copy_Pub_DC1_Sub_to_Pub_DC2.sh
...
# Copy dump file from 
#----------------------------------------------------------------------------
echo " Copying edsubp1p dump file to edgisdboraprd02 .." >> $file_log_tmp 2>&1
echo " Copying edsubp1p dump file to edgisdboraprd02 .." >> $file_log 2>&1
echo `date +%Y_%m_%d_%H_%M_%S` >> $file_log_tmp 2>&1
echo `date +%Y_%m_%d_%H_%M_%S` >> $file_log 2>&1
scp   /u02/uc4/edsubp1p/export/edsubp1p.dmp  edgisdboraprd02:/u02/uc4/edsubp3p/import >> $file_log_tmp 2>&1
echo `date +%Y_%m_%d_%H_%M_%S` >> $file_log_tmp 2>&1
echo `date +%Y_%m_%d_%H_%M_%S` >> $file_log 2>&1
int_error_code=`expr $int_error_code + $?`
