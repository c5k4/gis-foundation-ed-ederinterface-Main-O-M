                ...
                if not assertJsonSuccess(data):          
                    ...
                    for item in dataObj['services']:
                        ...
                        if  fullSvcName ==  "Search.IndexingLauncher" or fullSvcName == "Search.IndexGenerator":
                            continue
                        threadn = FuncThread(FuncStartOrStop, servername,serverPort,FolderName,fullSvcName,operation,params,headers)
                        threadn.start()
...
def FuncStartOrStop(servername,serverPort,FolderName,fullSvcName,operation,params,headers):
    print "Now Processing "+ operation +   " on  " + fullSvcName + " ......"
    httpConn = httplib.HTTPConnection(servername, serverPort)
