Prompt drop Procedure GENERATE_EXT_CCB_METER_LOAD;
DROP PROCEDURE PGEDATA.GENERATE_EXT_CCB_METER_LOAD
/

Prompt Procedure GENERATE_EXT_CCB_METER_LOAD;
--
-- GENERATE_EXT_CCB_METER_LOAD  (Procedure) 
--
...
begin
-- 5/13/19 - tjj4 - modified the procedure to only consider records from the full synch run from CC&B
     ...
     where ( VSP.servicepointid , TO_CHAR(VSP.DATECREATED, 'YYYYMMDDHH24MISS'))
           in ( Select stg1.servicepointid,TO_CHAR(stg1.DATECREATED, 'YYYYMMDDHH24MISS')
                ...
                Select stg2.servicepointid,max(TO_CHAR(stg2.DATECREATED, 'YYYYMMDDHH24MISS'))
                  from PGEDATA.pge_ccbtoedgis_stg stg2  where stg2.DATECREATED is not null --and stg2.action is null -- a3di: changed to prevent daily delta deltion from STG table
...
Prompt Grants on PROCEDURE GENERATE_EXT_CCB_METER_LOAD TO EDGIS to EDGIS;
GRANT EXECUTE, DEBUG ON PGEDATA.GENERATE_EXT_CCB_METER_LOAD TO EDGIS
/

Prompt Grants on PROCEDURE GENERATE_EXT_CCB_METER_LOAD TO GIS_I to GIS_I;
GRANT EXECUTE, DEBUG ON PGEDATA.GENERATE_EXT_CCB_METER_LOAD TO GIS_I
/

Prompt Grants on PROCEDURE GENERATE_EXT_CCB_METER_LOAD TO GIS_I_WRITE to GIS_I_WRITE;
GRANT EXECUTE, DEBUG ON PGEDATA.GENERATE_EXT_CCB_METER_LOAD TO GIS_I_WRITE
/

Prompt Grants on PROCEDURE GENERATE_EXT_CCB_METER_LOAD TO IGPCITEDITOR to IGPCITEDITOR;
GRANT EXECUTE, DEBUG ON PGEDATA.GENERATE_EXT_CCB_METER_LOAD TO IGPCITEDITOR
/

Prompt Grants on PROCEDURE GENERATE_EXT_CCB_METER_LOAD TO IGPEDITOR to IGPEDITOR;
GRANT EXECUTE, DEBUG ON PGEDATA.GENERATE_EXT_CCB_METER_LOAD TO IGPEDITOR
/

Prompt Grants on PROCEDURE GENERATE_EXT_CCB_METER_LOAD TO SDE to SDE;
GRANT EXECUTE, DEBUG ON PGEDATA.GENERATE_EXT_CCB_METER_LOAD TO SDE
/
