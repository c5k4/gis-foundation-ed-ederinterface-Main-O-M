Prompt drop Procedure UPDATE_STL_CCB_GISID;
DROP PROCEDURE PGEDATA.UPDATE_STL_CCB_GISID
/

Prompt Procedure UPDATE_STL_CCB_GISID;
--
-- UPDATE_STL_CCB_GISID  (Procedure) 
--
CREATE OR REPLACE PROCEDURE PGEDATA.UPDATE_STL_CCB_GISID
IS
BEGIN
  UPDATE PGEDATA.v_ccbupdate_GISID
  ...
  COMMIT;
  dbms_output.put_line(' update_stl_ccb_gisid-->updated STREETLIGHT_INV base table' || TO_CHAR(SYSDATE,'dd-mm-yyyy-hh-mi-ss') );
...
END update_stl_ccb_gisid;
/


Prompt Grants on PROCEDURE UPDATE_STL_CCB_GISID TO GIS_I to GIS_I;
GRANT EXECUTE, DEBUG ON PGEDATA.UPDATE_STL_CCB_GISID TO GIS_I
/
