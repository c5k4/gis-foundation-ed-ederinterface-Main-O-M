--------------------------------------------------------
--  DDL for Procedure CCBTOGIS_INITIALIZE_FILTERS
--------------------------------------------------------
set define off;

  CREATE OR REPLACE EDITIONABLE PROCEDURE "PGEDATA"."CCBTOGIS_INITIALIZE_FILTERS" AS
...
--m4jf updated procedure to update Error_description field in staging table
...
--)SPIDNULLERR;
-- UPDATE ERROR_DESCRIPTION FIELD FOR NULL SERVICEPOINTID IN STAGING TABLE
update PGEDATA.PGE_CCBTOEDGIS_STG set error_description = ('Null SPID (' || SERVICEPOINTID || ') - Meter ID = '
|| METERNUMBER || ' - Unique SPID = ' || UNIQUESPID || ', Field In error SERVICEPOINTID')
  where SERVICEPOINTID is null;
commit;
...
--)SPIDERR;
--commit;
--UPDATE ERROR_DESCRIPTION FIELD FOR DUPLICATE SERVICEPOINTID IN STAGING TABLE
--m4jf updated procedure to update Error_description field in staging table
update PGEDATA.PGE_CCBTOEDGIS_STG a set error_description =
  ('Duplicate SPID (' || SERVICEPOINTID || ') - Meter ID = '
|| METERNUMBER || ' - Unique SPID = (' || UNIQUESPID ||'), count of : '||(select count(1) from PGEDATA.PGE_CCBTOEDGIS_STG
  where servicepointid = a.servicepointid and datecreated = a.datecreated) || ', Field in Error - SERVICEPOINTID' )/* CCBTOGIS_INITIALIZE_FILTERS_V1_DUPSPIDRECORD*/
where Servicepointid || DATECREATED in ( Select servicepointid || datecreated from
(
select UNIQUESPID, SERVICEPOINTID, METERNUMBER, DATECREATED, count(SERVICEPOINTID) cnt_spid
from  PGEDATA.PGE_CCBTOEDGIS_STG where (SERVICEPOINTID,DATECREATED) in
(select /*+ SPID_SPID_DATE_CR_IDX */ servicepointid,DATECREATED from PGEDATA.PGE_CCBTOEDGIS_STG
...
--) USPIDERR;
--commit;
--UPDATE ERROR_DESCRIPTION FIELD IN STAGING TABLE FOR DUPLICATE UNIQUESPID
--m4jf updated procedure to update Error_description field in staging table
update pgedata.pge_ccbtoedgis_STG a set ERROR_DESCRIPTION =
('Duplicate Unique SPID (' || UNIQUESPID || ') - Meter ID = '
|| METERNUMBER || ' SPID = ' || SERVICEPOINTID|| ' , Field in Error - UNIQUESPID') /* CCBTOGIS_INITIALIZE_FILTERS_V1_DUPUNIQIDRECORD*/
where UNIQUESPID || DATECREATED || ACTION  in ( Select UNIQUESPID || DATECREATED || ACTION from
(
select UNIQUESPID, SERVICEPOINTID, METERNUMBER, DATECREATED, ACTION, count(UNIQUESPID)
from  PGEDATA.PGE_CCBTOEDGIS_STG where (UNIQUESPID,DATECREATED, action) in
(select UNIQUESPID,DATECREATED, action from PGEDATA.PGE_CCBTOEDGIS_STG
...
update PGEDATA.PGE_CCBTOEDGIS_STG set CUSTOMERTYPE =
    nvl(
        decode(revenueaccountcode, 'ERES','DOM',
           ...
           'ABS-TX5','AGR',
           'ABS-TX3','OTH', 'OTH' )
          ) where trim(customertype) is null AND error_description IS  NULL; --  date 02/13/2022 As discussed with m4jf updating where clause from "is not Null" to "is null"  error_description /* CCBTOGIS_INITIALIZE_FILTERS_V1_PREPCUSTTYPE*/ ;
