--------------------------------------------------------
--  DDL for Package Body PGE_ENOS_TO_EDGIS_DM_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "PGEDATA"."PGE_ENOS_TO_EDGIS_DM_PKG" AS

--create or replace PACKAGE BODY            "PGE_ENOS_TO_EDGIS_DM_PKG" AS
...
BEGIN
  ...
  IF input_dm_di_flag = 'DM' THEN
    ...
    -- Record should be Inserted for data migration
     ...
     SET sm.status           = 'F',
         sm.status_message     = 'Invalid action for generation details' , sm.processedtime = sysdate
    ...
    -- Mark the corresponding equipment record failed
     ...
     SET eq.status       = 'F',
         eq.status_message ='Invalid action for generation details' , eq.processedtime = sysdate
          ...
          AND EXISTS
            (SELECT 2
        	   ...
        	   WHERE sm.service_point_id = eq.service_point_id
      	       AND sm.status               = 'F'
      	       AND DBMS_LOB.substr(sm.status_message)       = 'Invalid action for generation details'
          ) ;

     -- Mark the equipment record failed if action type is other than Insert
     UPDATE pgedata.gen_equipment_stage
     SET status = 'F',
         status_message = 'prefix' || ' Invalid action', processedtime = sysdate
    ...
    -- Mark the other equipment record as failed for the record updated in the above step
     ...
     SET eq1.status  = 'F', eq1.status_message = 'Other equipments failed due Invalid action for any equipment' , eq1.processedtime = sysdate
     WHERE eq1.status  IS NULL
       AND EXISTS
        (SELECT 2
            ...
            AND DBMS_LOB.substr(eq.status_message)   = 'Invalid action'
        ) ;

    -- Mark the the summary record failed for above updated record
      UPDATE pgedata.gen_summary_stage sm
      SET sm.status       = 'F',   sm.status_message = 'Generation failed due to Invalid action in equipment details' , sm.processedtime = sysdate
      WHERE sm.status  IS NULL
       AND EXISTS
       (SELECT 2
        FROM pgedata.gen_equipment_stage eq
...
WHEN MATCHED THEN UPDATE
    SET STATUS ='F', STATUS_MESSAGE = 'Already Generation Exist' , processedtime = sysdate ;


    --Mark the equipment record failed if Generation failed
    UPDATE gen_equipment_stage
    SET status              ='F',
      status_message        = '113 Failed due to Generatin failed' , processedtime = sysdate
    WHERE service_point_id IN
      (SELECT service_point_id
      FROM gen_summary_stage
      WHERE status      ='F'
      AND DBMS_LOB.substr(status_message)='Already Generation Exist'
      );
...
--Added to Fail Records  with Action 'D' in Eqp and I in Summ Start
     update gen_equipment_stage eq set status='W', status_message= 'Invalid Action' , processedtime = sysdate where eq.action='D' and
exists (select 2 from gen_summary_stage sm where sm.action='I' and sm.service_point_id= eq.service_point_id);
    ...
    --Added to Fail Records  with Action 'D' in Eqp and I in Summ End
     ...
     UPDATE pgedata.gen_equipment_stage
     SET status  = 'F', status_message = 'Invalid action for equipment details' , processedtime = sysdate
      WHERE status IS NULL
        AND (Upper (action) NOT IN ('I', 'D', 'U') OR action  IS NULL) ;

     UPDATE pgedata.gen_summary_stage sm
     SET sm.status  = 'F',    sm.status_message  = 'Null GUID with update/delete action for generation' , sm.processedtime = sysdate
     WHERE sm.status  IS NULL
         AND Upper (sm.action) IN ('D', 'U')
         AND sm.guid  IS NULL;

 update pgedata.gen_equipment_stage eq set eq.status='F', eq.status_message= '113 Failed due to generation failed' , eq.processedtime = sysdate
    where eq.status is null and EXISTS
    (select 2 from pgedata.gen_summary_stage sm where sm.status='F' and DBMS_LOB.substr(sm.status_message)='Null GUID with update/delete action for generation'
    and sm.service_point_id= eq.service_point_id);


     UPDATE pgedata.gen_summary_stage sm
     SET sm.status  = 'F',   sm.status_message  ='No generation available for update or delete' , sm.processedtime = sysdate
     WHERE sm.status     IS NULL
     ...
     UPDATE pgedata.gen_equipment_stage eq
     SET eq.status  = 'F',   eq.status_message  = '110 No data available for update or delete' , eq.processedtime = sysdate
     WHERE eq.status     IS NULL
       AND EXISTS
     	 (SELECT 2 FROM pgedata.gen_summary_stage sm WHERE sm.service_point_id= eq.service_point_id
       AND sm.status='F' and DBMS_LOB.substr(sm.status_message)='No generation available for update or delete' ) ;


     UPDATE pgedata.gen_equipment_stage eq
     SET eq.status          = 'F',
         eq.status_message    = '115 Invalid action of equipment details for delete generation' , eq.processedtime = sysdate
     WHERE eq.status     IS NULL
 ...
 -- Check for service point id
  UPDATE pgedata.gen_summary_stage sm
  SET sm.status            = 'F',    sm.status_message = 'Null service point ID for generation' , sm.processedtime = sysdate
   WHERE sm.status       IS NULL
       AND sm.service_point_id IS NULL;

   UPDATE pgedata.gen_equipment_stage eq
   SET eq.status       = 'F',    eq.status_message = '111 Null service point ID for generation' , eq.processedtime = sysdate
    WHERE eq.status  IS NULL
         AND EXISTS
  	  (SELECT 2
      	   FROM pgedata.gen_summary_stage sm
      	  WHERE sm.service_point_id = eq.service_point_id
  	       AND sm.status               = 'F'
    	       AND DBMS_LOB.substr(sm.status_message)       = 'Null service point ID for generation' 
    	 ) ;

   UPDATE pgedata.gen_summary_stage sm
   SET sm.status       = 'F',     sm.status_message = 'Generation SP ID not exist in EDGIS', sm.processedtime = sysdate
   WHERE sm.status  IS NULL
   AND NOT EXISTS
   	 (SELECT 2
   ...
   UPDATE pgedata.gen_equipment_stage eq
   SET eq.status       = 'F',    eq.status_message = '108 Generation SP ID not exist in EDGIS', eq.processedtime = sysdate
   WHERE eq.status  IS NULL
   AND EXISTS
   	 (SELECT 2
       	FROM pgedata.gen_summary_stage sm
      	WHERE sm.service_point_id = eq.service_point_id
   	AND sm.status               = 'F'
    	AND DBMS_LOB.substr(sm.status_message)       = 'Generation SP ID not exist in EDGIS'
  	 ) ;

   UPDATE pgedata.gen_summary_stage sm
   SET sm.status            = 'F',    sm.status_message  = 'Duplicate service point ID for generation', sm.processedtime = sysdate
   WHERE sm.status       IS NULL
   ...
   UPDATE pgedata.gen_equipment_stage eq
   SET eq.status       = 'F',
    eq.status_message = '114 Duplicate service point ID for generation' , eq.processedtime = sysdate
    WHERE eq.status  IS NULL
         AND EXISTS
    (SELECT 2
       FROM pgedata.gen_summary_stage sm
      WHERE sm.service_point_id = eq.service_point_id
    AND sm.status               = 'F'
    AND DBMS_LOB.substr(sm.status_message)       = 'Duplicate service point ID for generation'
    ) ;

   UPDATE pgedata.gen_summary_stage
   SET status   = 'F',    status_message      = 'Multiple records in EDGIS.SERVICEPOINT for generation SPID', processedtime = sysdate
   WHERE status  IS NULL
  ...
  UPDATE pgedata.gen_equipment_stage eq
  SET eq.status       = 'F',    eq.status_message = '107 Multiple records in EDGIS.SERVICEPOINT for generation SPID', eq.processedtime = sysdate
  WHERE eq.status  IS NULL
       AND EXISTS
  	 (SELECT 2
      	 FROM pgedata.gen_summary_stage sm
      	WHERE sm.service_point_id = eq.service_point_id
           	AND sm.status               = 'F'
           	AND DBMS_LOB.substr(sm.status_message)       = 'Multiple records in EDGIS.SERVICEPOINT for generation SPID'
  	  ) ;

  UPDATE pgedata.gen_equipment_stage
  SET status            = 'F',   status_message      =  '111 Null service point ID', processedtime = sysdate
  WHERE status       IS NULL
       AND service_point_id IS NULL;

  UPDATE pgedata.gen_equipment_stage eq
  SET eq.status       = 'F',    eq.status_message =  '102 No generation for this equipment', eq.processedtime = sysdate
  WHERE eq.status  IS NULL
       AND NOT EXISTS
    	(SELECT 2
       	FROM pgedata.gen_summary_stage sm
   ...
   UPDATE pgedata.gen_equipment_stage eq1
   SET eq1.status       = 'F',    eq1.status_message =  'Other equipments failed due to No generation for this equipment error', eq1.processedtime = sysdate
  WHERE eq1.status  IS NULL
       AND EXISTS
   	 (SELECT 2
       	  FROM pgedata.gen_equipment_stage eq
      	  WHERE eq.status       = 'F'
    	      AND eq.service_point_id = eq1.service_point_id
    	     AND DBMS_LOB.substr(eq.status_message)   = ' 102 No generation for this equipment'
  	  ) ;

  UPDATE pgedata.gen_equipment_stage eq
  SET eq.status         = 'F',    eq.status_message   =  'Duplicate SAP equipment ID' , eq.processedtime = sysdate
  WHERE eq.status    IS NULL
  ...
  UPDATE pgedata.gen_equipment_stage eq1
  SET eq1.status       = 'F',    eq1.status_message = 'Other equipments failed due to Duplicate SAP equipment ID' , eq1.processedtime = (select to_char(sysdate,'YYYY-MM-DD HH24:MI:SS') from dual)
  WHERE eq1.status  IS NULL
       AND EXISTS
   	 (SELECT 2
       	 FROM pgedata.gen_equipment_stage eq
      	 WHERE eq.status       = 'F'
    	     AND eq.service_point_id = eq1.service_point_id
    	     AND DBMS_LOB.substr(eq.status_message)   = 'Duplicate SAP equipment ID'
    	) ;

   UPDATE pgedata.gen_summary_stage sm
   SET sm.status       = 'F',    sm.status_message = 'Generation failed due to Duplicate SAP equipment ID for any equipment' , sm.processedtime = sysdate
   WHERE sm.status  IS NULL
        AND EXISTS
   	 (SELECT 2
       	 FROM pgedata.gen_equipment_stage eq
  ...
  -- Invalid gen_tech_cd
   UPDATE pgedata.gen_equipment_stage eq
   SET eq.status           = 'F',    eq.status_message     = 'Invalid GEN_TECH_CD' , eq.processedtime = sysdate
    WHERE eq.status      IS NULL
        AND eq.gen_tech_cd NOT IN ('GEN.SYNCH', 'GEN.INDCT', 'GEN.INVEXT', 'GEN.INVINC', 'PWR.PVPNL', 'PWR.BATT', 'PWR.WIND', 'PWR.GENRTR') ;

   UPDATE pgedata.gen_equipment_stage eq1
   SET eq1.status       = 'F', eq1.status_message = 'Other equipments failed due to Invalid GEN_TECH_CD' , eq1.processedtime = sysdate
   WHERE eq1.status  IS NULL
        AND EXISTS
    	(SELECT 2
       	FROM pgedata.gen_equipment_stage eq
      	WHERE eq.status       = 'F'
    	     AND eq.service_point_id = eq1.service_point_id
    	     AND DBMS_LOB.substr(eq.status_message)   = 'Invalid GEN_TECH_CD'
    	) ;

   UPDATE pgedata.gen_summary_stage sm
   SET sm.status       = 'F',    sm.status_message = 'Generation failed due to Invalid GEN_TECH_CD in equipment details' , sm.processedtime = sysdate
    WHERE sm.status  IS NULL
         AND EXISTS
 	(SELECT 2
       	FROM pgedata.gen_equipment_stage eq
   ...
   UPDATE pgedata.gen_equipment_stage eq1
   SET  eq1.status_message = '106 equipments failed due to No DC generator against INVERTER'
   WHERE eq1.status  IS NULL
        AND EXISTS
    	(SELECT 2
      	 FROM pgedata.gen_equipment_stage eq
      	WHERE eq.status      IS NULL
    	     AND eq.service_point_id = eq1.service_point_id
    	     AND DBMS_LOB.substr(eq.status_message)   = 'No DC generator against INVERTER'
  	  ) ;
   ...
   WHERE eq1.status  IS NULL
    	...
    	(SELECT 2
      	...
      	WHERE eq.status       IS NULL
    	     AND eq.service_point_id = eq1.service_point_id
    	     AND DBMS_LOB.substr(eq.status_message)   = 'Null GEN_TECH_EQUIPMENT for DC generator'
  	  ) ;
   ...
   UPDATE pgedata.gen_equipment_stage eq
   SET eq.status       = 'F',    eq.status_message = '105 No INVERTER against DC generator'
   WHERE eq.status  IS NULL
        AND gen_tech_cd    IN ('PWR.PVPNL', 'PWR.BATT', 'PWR.WIND', 'PWR.GENRTR')
       AND NOT EXISTS
    	(SELECT 2
   ...
   UPDATE pgedata.gen_equipment_stage eq1
   SET eq1.status       = 'F',    eq1.status_message =  'Other equipments failed due to No INVERTER against DC generator' , eq1.processedtime = sysdate
    WHERE eq1.status  IS NULL
        AND EXISTS
       	(SELECT 2
       	FROM pgedata.gen_equipment_stage eq
      	WHERE eq.status       = 'F'
    	    AND eq.service_point_id = eq1.service_point_id
    	    AND DBMS_LOB.substr(eq.status_message)   = '105 No INVERTER against DC generator'
   	 ) ;

   UPDATE pgedata.gen_summary_stage sm
   SET sm.status       = 'F',   sm.status_message = 'Generation failed due to No INVERTER against DC generator' , sm.processedtime = sysdate
   WHERE sm.status  IS NULL
        AND EXISTS
    	(SELECT 2
       	FROM pgedata.gen_equipment_stage eq
     ...
     UPDATE pgedata.gen_equipment_stage eq
   SET     eq.status_message = '109 DC generator not attached with correct inverter'
   WHERE eq.status  IS NULL
        AND eq.gen_tech_cd IN ('PWR.PVPNL', 'PWR.BATT', 'PWR.WIND', 'PWR.GENRTR')
       AND NOT EXISTS
    	(SELECT 2
   ...
   UPDATE pgedata.gen_equipment_stage eq1
   SET    eq1.status_message = '109 DC generator not attached with correct inverter'
    WHERE eq1.status  IS NULL
         AND EXISTS
    	(SELECT 2
       	FROM pgedata.gen_equipment_stage eq
      	WHERE eq.status      IS NULL
    	     AND eq.service_point_id = eq1.service_point_id
    	    AND DBMS_LOB.substr(eq.status_message)   = '109 DC generator not attached with correct inverter'
    	) ;

   UPDATE pgedata.gen_summary_stage sm
   SET   sm.status_message ='prefix' || ' 109 DC generator not attached with correct inverter'
   WHERE sm.status  IS NULL
        AND EXISTS
    	(SELECT 2
       	FROM pgedata.gen_equipment_stage eq
...
update gen_equipment_stage eq set eq.status='F', eq.status_message=  '112 Equipment Failed due Null GEN_TECH_CD' , eq.processedtime = sysdate where gen_tech_cd is null;

update gen_summary_stage sm set sm.status='F', sm.status_message= 'Generation Failed due Null GEN_TECH_CD in Equipment' , sm.processedtime = sysdate
where exists
(
...
update  gen_equipment_stage eq set eq.status='F', eq.status_message=  '103 Other Equipment Failed due to Null GEN_TECH_CD' , eq.processedtime = sysdate
where exists
(
...
BEGIN
  ...
  -- M4JF Update processed time of record .
  ...
  WHEN matched THEN
  UPDATE
  SET stg_1.status = 'S',  status_message  ='Successfully Generator created' , stg_1.processedtime = sysdate
  ...
  WHEN matched THEN
  UPDATE
  SET stg_1.status = 'S',    status_message  =  'Successfully Generation created' , stg_1.processedtime = sysdate
  WHERE stg_1.status IS NULL;
  ...
  LOOP
      ...
      BEGIN --3
        ...
        IF h_action = 'I' THEN
          ...
          IF h_cnt_11              = 0 THEN
             ...
             INSERT   INTO pgedata.pgedata_generationinfo_stage
              (
                ...
                derated,
                telemetryenabled ,---- added on 02 Feb 2021 DERMS change
                communicationtype ,
                controlenabled ,
                controlprogramtype,
                projecttotalexportkw,
                limited 
              )
              VALUES
              (
                ...
                i.telm_cntr_enb,
                i.telm_cntrprg_typ,
                i.project_total_export_kw,---added on 13th October 2021 WG1 changes
                i.limited
              ) ;
            ...
            --start code for insert new in SM_GENERATION data
             INSERT    INTO pgedata.pgedata_sm_generation_stage
              (
                ...
                action              ,
                derated,
                --- added on 02nd  Feb 2021 DERMS changes 
                telm_enbl,
                telm_comm_typ,
                telm_cntr_enb,
                telm_cntrprg_typ,
                project_total_export_kw,---added on 13th October 2021 WG1 changes
                limited 
              )
             SELECT service_point_id                               ,
              globalid                                             ,
          ...
          ELSE
            ...
            UPDATE pgedata.gen_summary_stage
            SET status       = 'F',
                status_message = 'Already generation exist' , processedtime = sysdate
	...
	-- Update the stage table
            ...
            SET datemodified     = SYSDATE               ,
                ...
                action             = 'U'                   ,
                derated               = 'N',
                ---- added on 02 Feb DERMS 2021 change
                telemetryenabled    =i.telm_enbl,
                communicationtype   =i.telm_comm_typ,
                controlenabled      =i.telm_cntr_enb,
                controlprogramtype      =i.telm_cntrprg_typ,
                projecttotalexportkw = i.project_total_export_kw,---added on 13th October 2021 WG1 changes
                limited = i.limited
             WHERE globalid     = i.guid;
--Changes to update Export_KW
             ...
             UPDATE
               SET trgt.service_point_id   = src.service_point_id  ,
                ...
                trgt.derated = src.derated,
                trgt.telm_enbl  =src.telemetryenabled,---added on 18 Jan 2021 DERMS change
                trgt.telm_comm_typ       =src.communicationtype,
                trgt.telm_cntr_enb    =src.controlenabled,
                trgt.telm_cntrprg_typ=src.controlprogramtype,
                trgt.project_total_export_kw = src.projecttotalexportkw,---added on 13th October 2021 WG1 changes
                trgt.limited = src.limited;
     ...
     -- Record loaded into the stage table. Process further
       ...
       -- start code for equipment details
        ...
        LOOP
              ...
              IF h_action_eq = 'I' THEN   -- checking Eqp action
                IF h_action IN ('I', 'U') THEN
                  ...
                  BEGIN --6
                     INSERT   INTO pgedata.pgedata_sm_generator_stage
                      (
                        ...
                        rated_voltage       ,
                        number_of_phases    ,
                        mode_of_inverter    ,--Added to fix SMRT Issue
                        gen_tech_cd         ,
                        pto_date            ,
                        program_type        ,
                        control_cd          ,
                        ...
                        tech_type_cd        ,
                        backup_gen          ,
                        method_of_backup    , --added on 4 feb 20 ENOS Tariff Change
                        grid_transition     ,
                        parallel_time_to_grid,
                        standbygen          ,
                        derated             ,
                        operating_mode      ,
                        export_to_grid      ,
                        non_export_scheme   ,
                        derated_rating_per_unit_kw,                        
                        charge_demand_kw    ,
                        max_storage_capacity,
                        est_ann_usage_aes_kw,
                        limit_export_plan,---added on 13th October 2021 WG1 changes
                        limited_export_pcs,
                        sp_crdpcs_cert,
                        max_expct_export
                      )
                      ...
                      (
                        ...
                        V_bulk_eq (k) .mode_of_inv         ,--Added to fix SMRT Issue
                        h_gen_tech                         ,
                        ...
                        V_bulk_eq (k) .method_of_backup    , --added on 4 feb 20 ENOS Tariff Change
                        V_bulk_eq (k) .grid_transition     ,
                        V_bulk_eq (k) .parallel_time_to_grid,
                        V_bulk_eq (k) .standbygen          ,
                        V_bulk_eq (k) .derated             ,
                        V_bulk_eq (k) .operating_mode      ,
                        V_bulk_eq (k) .export_to_grid      ,
                        V_bulk_eq (k) .non_export_scheme   ,
                        V_bulk_eq (k) .derated_rating_per_unit_kw,                        
                        V_bulk_eq (k) .charge_demand_kw    ,
                        V_bulk_eq (k) .max_storage_capacity,
                        V_bulk_eq (k) .est_ann_usage_aes_kw,
                        V_bulk_eq (k) .limit_export_plan,--- added on 13th Oct 2021 WG1 changes
                        V_bulk_eq (k) .limited_export_pcs,
                        V_bulk_eq (k) .sp_crdpcs_cert,
                        V_bulk_eq (k) .max_expct_export
                  ...
                  WHEN OTHERS THEN
                    ...
                    UPDATE pgedata.gen_equipment_stage
                    SET status       = 'F',  status_message = 'SG Generator creation failed due to exception capture' , processedtime = sysdate
                    WHERE objectid = V_bulk_eq (k) .objectid;

                    UPDATE pgedata.gen_equipment_stage
                    SET status           = 'F',     status_message     =  'Failed due to other eqp failed' , processedtime = sysdate
                    WHERE status      IS NULL
                      AND service_point_id = i.service_point_id;
                ...
                ELSE
                   ...
                   UPDATE pgedata.gen_equipment_stage
                   SET status       = 'F',   status_message =  'Invalid action in generation for respective new equipment' , processedtime = sysdate
                   WHERE objectid = V_bulk_eq (k) .objectid;
                ...
                IF h_action     = 'U' THEN
                  ...
                  BEGIN
                    ...
                    SET sap_egi_notification = V_bulk_eq (k) .sap_egi_notification,
                      ...
                      mode_of_inverter       = V_bulk_eq (k) .mode_of_inv         ,--Added to fix SMRT Issue
                      ...
                      method_of_backup       = V_bulk_eq (k) .method_of_backup    , --added on 4 feb 20 ENOS Tariff Change
                      grid_transition        = V_bulk_eq (k) .grid_transition     ,
                      parallel_time_to_grid  = V_bulk_eq (k) .parallel_time_to_grid,
                      standbygen             = V_bulk_eq (k) .standbygen          ,
                      derated                = V_bulk_eq (k) .derated             ,
                      operating_mode         = V_bulk_eq (k) .operating_mode      ,
                      export_to_grid         = V_bulk_eq (k) .export_to_grid      ,
                      non_export_scheme      = V_bulk_eq (k) .non_export_scheme   ,
                      derated_rating_per_unit_kw  = V_bulk_eq (k) .derated_rating_per_unit_kw,                      
                      charge_demand_kw       = V_bulk_eq (k) .charge_demand_kw    ,
                      max_storage_capacity   = V_bulk_eq (k) .max_storage_capacity,
                      est_ann_usage_aes_kw   = V_bulk_eq (k) .est_ann_usage_aes_kw,
                      limit_export_plan      = V_bulk_eq (k) .limit_export_plan,--- added on 13th Oct 2021 WG1 changes
                      limited_export_pcs     = V_bulk_eq (k) .limited_export_pcs,
                      sp_crdpcs_cert         = V_bulk_eq (k) .sp_crdpcs_cert,
                      max_expct_export       = V_bulk_eq (k) .max_expct_export
                  ...
                  WHEN OTHERS THEN
                    ...
                    UPDATE pgedata.gen_equipment_stage
                    SET status       = 'F',   status_message =  'SG Generator updation failed due to exception capture' ,processedtime = sysdate
                    WHERE objectid = V_bulk_eq (k) .objectid;

                     UPDATE pgedata.gen_equipment_stage
                     SET status           = 'F',    status_message     = 'prefix' || ' Failed due to other eqp failed', processedtime = sysdate
                     WHERE status      IS NULL
                       AND service_point_id = i.service_point_id;

                     UPDATE pgedata.gen_summary_stage
                     SET status       = 'F',  status_message = 'prefix' || ' Failed due to respective eqp failed', processedtime = sysdate
                     WHERE objectid = i.objectid;
                     EXIT;
                  END;
                ELSE
                  ...
                  UPDATE pgedata.gen_equipment_stage
                  SET status       = 'F',   status_message =  'Invalid action in generation for respective equipment' , processedtime = sysdate
                  WHERE objectid = V_bulk_eq (k) .objectid;
                  ...
                  UPDATE pgedata.gen_summary_stage
                  SET status       = 'F',  status_message =  'Failed due to generator failed',processedtime = sysdate
                  WHERE objectid = i.objectid;
              ...
              IF h_action_eq = 'I' THEN   -- checking Eqp action
                IF h_action IN ('I', 'U') THEN
                  ...
                  BEGIN --6
                     INSERT   INTO pgedata.pgedata_sm_generator_stage
                      (
                        ...
                        max_storage_capacity,
                        est_ann_usage_aes_kw,
                        limit_export_plan,--- added on 13th Oct 2021 WG1 changes
                        limited_export_pcs,
                        sp_crdpcs_cert,
                        max_expct_export
                      )
                  ...
                  WHEN OTHERS THEN
                    ...
                    UPDATE pgedata.gen_equipment_stage
                    SET status       = 'F',  status_message =  'IG Generator creation failed due to exception capture' , processedtime = sysdate
                    WHERE objectid = V_bulk_eq (k) .objectid;
              ...
              ELSIF h_action_eq = 'U' THEN
                IF h_action     = 'U' THEN
                  ...
                  WHEN OTHERS THEN
                    ...
                    UPDATE pgedata.gen_summary_stage
                    SET status       = 'F',   status_message =  'Failed due to respective eqp failed', processedtime = sysdate
                    WHERE objectid = i.objectid;
              ...
              IF h_action_eq  = 'I' THEN              -- checking Eqp action
                IF h_action  IN ('I', 'U') THEN
                  ...
                  WHEN OTHERS THEN
                    ...
                    UPDATE pgedata.gen_equipment_stage
                    SET status       = 'F',   status_message =  'INVERTER creation failed due to exception capture' , processedtime = sysdate
                    WHERE objectid = V_bulk_eq (k) .objectid;

                    UPDATE pgedata.gen_equipment_stage
                    SET status           = 'F',   status_message     =  'Failed due to respective inverter failed' , processedtime = sysdate
                    WHERE status      IS NULL
                      AND service_point_id = i.service_point_id;
                  ...
                  IF h_cnt_dc > 0 THEN
                    BEGIN
                       ...
                       SELECT h_gntr_id_3                                            ,
                        ...
                        'I'                                                          ,
                        program_type                                                 ,
                        operating_mode                                               ,--added on 4 feb 20 ENOS Tariff Change
                        export_to_grid                                               ,
                        non_export_scheme                                            ,
                        est_ann_usage_aes_kw,
                        limit_export_plan     ,---added on 11th nov 2021 WG1 changes
                      limited_export_pcs    ,
                      sp_crdpcs_cert        ,
                      max_expct_export       
                        FROM pgedata.gen_equipment_stage
                       ...
                       h_success := 2;
                       UPDATE pgedata.gen_equipment_stage
                       SET status             = 'S',  status_message       =  'DC Generator created successfully' , processedtime = sysdate
                       WHERE Upper (action) = 'I'
                       	 AND status            IS NULL
                      	 AND gen_tech_cd       IN ('PWR.PVPNL', 'PWR.BATT', 'PWR.WIND', 'PWR.GENRTR')
                      	 AND gen_tech_equipment = h_eq_id
                    ...
                    WHEN OTHERS THEN
                      ...
                      UPDATE pgedata.gen_equipment_stage
                      SET status       = 'F',    status_message =  'DC GNTR creation failed due to exception capture' , processedtime = sysdate
                      WHERE objectid = V_bulk_eq (k) .objectid;

                      UPDATE pgedata.gen_equipment_stage
                      SET status           = 'F',  status_message     =  '104 Failed due to respective DC GENTR failed' , processedtime = sysdate
                      WHERE status      IS NULL
                      	AND service_point_id = i.service_point_id;

                      UPDATE pgedata.gen_summary_stage
                      SET status       = 'F',   status_message =  'Failed due to respective DC GENTR failed' , processedtime = sysdate
                      WHERE objectid = i.objectid;
                ...
                ELSE
                  ...
                  UPDATE pgedata.gen_equipment_stage
                  SET status       = 'F',  status_message = 'Invalid action for this inverter' , processedtime = sysdate
                  WHERE objectid = V_bulk_eq (k) .objectid;

                  UPDATE pgedata.gen_equipment_stage
                  SET status           = 'F',  status_message     =  'Failed due to respective inverter action is invalid' , processedtime = sysdate
                  WHERE status      IS NULL
                  	AND service_point_id = i.service_point_id;
                ...
                IF h_action     = 'U' THEN
                 ...
                 BEGIN
                  ...
                  WHEN OTHERS THEN
                    ...
                    UPDATE pgedata.gen_equipment_stage
                    SET status       = 'F',   status_message = 'inverter updation failed due to exception capture' , processedtime = sysdate
                    WHERE objectid = V_bulk_eq (k) .objectid;

                    UPDATE pgedata.gen_equipment_stage
                    SET status           = 'F',   status_message     =  'Failed due to respective inverter updation failed' , processedtime = sysdate
                    WHERE status      IS NULL
                      AND service_point_id = i.service_point_id;
                  ...
                  LOOP
                      ...
                      BEGIN
                         ...
                         INSERT  INTO pgedata.pgedata_sm_gen_equipment_stage
                          ...
                          (
                            ...
                            V_bulk_eq_2 (p).operating_mode                                                ,--added on 4 feb 20 ENOS Tariff Change
                            V_bulk_eq_2 (p).export_to_grid                                                ,
                            V_bulk_eq_2 (p).non_export_scheme                                             ,
                            V_bulk_eq_2 (p).est_ann_usage_aes_kw,
                             V_bulk_eq_2 (p).limit_export_plan,---added on 11th Nov 2021 WG1 changes
                           V_bulk_eq_2 (p).limited_export_pcs,
                          V_bulk_eq_2 (p).sp_crdpcs_cert,
                         V_bulk_eq_2 (p).max_expct_export
                          ) ;
                    ...
                    ELSIF h_action_eq_dc = 'U' THEN
                      ...
                      SET trgt.manufacturer       = V_bulk_eq_2 (p) .manufacturer        ,
                        ...
                        trgt.operating_mode       = V_bulk_eq_2 (p) .operating_mode      ,--added on 4 feb 20 ENOS Tariff Change
                        trgt.export_to_grid       = V_bulk_eq_2 (p) .export_to_grid      ,
                        trgt.non_export_scheme    = V_bulk_eq_2 (p) .non_export_scheme   ,
                        trgt.est_ann_usage_aes_kw = V_bulk_eq_2 (p) .est_ann_usage_aes_kw,
                         trgt.limit_export_plan= V_bulk_eq_2 (p).limit_export_plan,---added on 11th Nov 2021 WG1 changes
                           trgt.limited_export_pcs=V_bulk_eq_2 (p).limited_export_pcs,
                          trgt.sp_crdpcs_cert=V_bulk_eq_2 (p).sp_crdpcs_cert,
                          trgt.max_expct_export=V_bulk_eq_2 (p).max_expct_export
          ...
          WHEN no_data_found THEN
            ...
            UPDATE pgedata.gen_equipment_stage
            SET status       = 'F',   status_message =  'No protection record exist' , processedtime = sysdate
            WHERE objectid = V_bulk_eq (k) .objectid;
         ...
         WHEN OTHERS THEN
            ...
            UPDATE pgedata.gen_equipment_stage
            SET status       = 'F',  status_message =  'Failed due to invalid protection' , processedtime = sysdate
            WHERE objectid = V_bulk_eq (k) .objectid;
        ...
        IF h_success_insert = 4 THEN
         UPDATE pgedata.gen_equipment_stage
          SET status               = 'S',   status_message         =  'Generator Created successfully' ,  processedtime = sysdate
          WHERE service_point_id = i.service_point_id
          	AND status              IS NULL
        ...
        IF h_success IN (1, 2) THEN
          UPDATE pgedata.gen_equipment_stage
          SET status               = 'S',   status_message         =  'Generator updated successfully' , processedtime = sysdate
          WHERE service_point_id = i.service_point_id
          	AND status              IS NULL
          	AND gen_tech_cd         IN ('GEN.SYNCH', 'GEN.INDCT', 'GEN.INVEXT', 'GEN.INVINC') ;

          UPDATE pgedata.gen_equipment_stage
          SET status              = 'S',   status_message        =  'DC Generator updated successfully' , processedtime = sysdate
          WHERE status         IS NULL
             AND service_point_id    = i.service_point_id
             AND gen_tech_cd        IN ('PWR.PVPNL', 'PWR.BATT', 'PWR.WIND', 'PWR.GENRTR')
          ...
          UPDATE pgedata.gen_summary_stage
          SET status       = 'S',  status_message = 'Successfully generation updated(I/D/U)' , processedtime = sysdate
        ...
        ELSE
          ...
          UPDATE pgedata.gen_equipment_stage
          SET status               = 'F',   status_message         =  '101 Failed Due to Invalid Data' , processedtime = sysdate
          WHERE service_point_id = i.service_point_id
          	AND status              IS NULL
         	  AND gen_tech_cd         IN ('GEN.SYNCH', 'GEN.INDCT', 'GEN.INVEXT', 'GEN.INVINC') ;
      ...
      ELSE
        ...
        UPDATE pgedata.gen_equipment_stage
        SET status               = 'F',   status_message         =  '113 Failed due to generation failed', processedtime = sysdate
        WHERE service_point_id = i.service_point_id
        	AND status              IS NULL;
        ...
        WHERE objectid = i.objectid
          ...
          ----Changes for Specific Message Eend
           ---------Enos Supersede  start

           --------Enos Supersede End
      END IF;
      update pgedata.gen_equipment_stage  set GUID= (select GlobalID from edgis.zz_mv_generationinfo  where  sapeginotification = i.sap_egi_notification)
                 where DBMS_LOB.substr(status_message)='113 Failed due to generation failed';
       --update pgedata.gen_equipment_stage  set GUID= (select GlobalID from edgis.zz_mv_generationinfo  where  sapeginotification = i.sap_egi_notification)
               --  where status_message='INVALID ACTION' and action='I';          
  ...
  END LOOP;
   ...
   WHEN matched THEN
     ...
     -- Added on 4 Feb 20 for Derated. ENOS Tariff Change
     update pgedata.pgedata_sm_generator_stage set standbygen='Y' where standbygen='X';

     update pgedata.pgedata_sm_generation_stage set DERATED = 'N'
      where SAP_EGI_NOTIFICATION in (select distinct(sap_egi_notification) from pgedata.pgedata_sm_generator_stage where DERATED='N');

     update pgedata.pgedata_sm_generation_stage set DERATED = 'Y'
      where SAP_EGI_NOTIFICATION in (select distinct(sap_egi_notification) from pgedata.pgedata_sm_generator_stage where DERATED='Y');

    merge INTO pgedata.pgedata_generationinfo_stage trgt USING pgedata.pgedata_sm_generation_stage src ON (trgt.globalid = src.global_id)
   WHEN matched THEN
     UPDATE SET trgt.derated = src.derated WHERE src.derated is not null;
     ---Enos Supersede 
     ---Added on 5th Feb 2021 for Derms Changes 


     update pgedata.pgedata_sm_generation_stage set TELM_ENBL = 'N'
     where SAP_EGI_NOTIFICATION in (select distinct(sap_egi_notification) from pgedata.pgedata_sm_generation_stage where TELM_ENBL is null);

    merge INTO pgedata.pgedata_generationinfo_stage trgt USING pgedata.pgedata_sm_generation_stage src ON (trgt.globalid = src.global_id)
   WHEN matched THEN
    UPDATE SET trgt.TELEMETRYENABLED = src.TELM_ENBL WHERE src.TELM_ENBL is not null;

      update pgedata.pgedata_sm_generation_stage set TELM_CNTR_ENB = 'N'
      where SAP_EGI_NOTIFICATION in (select distinct(sap_egi_notification) from pgedata.pgedata_sm_generation_stage where TELM_CNTR_ENB is null);

   merge INTO pgedata.pgedata_generationinfo_stage trgt USING pgedata.pgedata_sm_generation_stage src ON (trgt.globalid = src.global_id)
   WHEN matched THEN
     UPDATE SET trgt.CONTROLENABLED = src.TELM_CNTR_ENB WHERE src.TELM_CNTR_ENB is not null  ;
     update pgedata.pgedata_sm_generation_stage set date_modified = sysdate where action='U';
     update pgedata.pgedata_sm_generator_stage set date_modified = sysdate where action='U';
     update pgedata.pgedata_sm_gen_equipment_stage set date_modified = sysdate where action='U';

     update pgedata.pgedata_sm_gen_equipment_stage b set action= (select action from pgedata.gen_equipment_stage a where  a.SAP_EQUIPMENT_ID = b.sap_equipment_id
           and  gen_tech_cd IN ('PWR.PVPNL', 'PWR.BATT', 'PWR.WIND', 'PWR.GENRTR'));  
      update pgedata.pgedata_sm_generator_stage b set action= (select action from pgedata.gen_equipment_stage a where  b.sap_equipment_id =a.sap_equipment_id
        and  gen_tech_cd IN ('GEN.SYNCH', 'GEN.INDCT', 'GEN.INVEXT', 'GEN.INVINC'));  
     ---UPDATE pgedata.pgedata_sm_gen_equipment_stage set action ='D' where Action (select 2 from gen_equipment_stage where action='D' and AND gen_tech_cd 
     --IN ('PWR.PVPNL', 'PWR.BATT', 'PWR.WIND', 'PWR.GENRTR')
     --update pgedata.pgedata_sm_generator_stage set action ='D'
--update pgedata.gen_equipment_stage a set GUID= (select b.GlobalID from edgis.zz_mv_generationinfo b where  a.sap_egi_notification=b.sapeginotification)
               --  where status_message='113 Failed due to generation failed' and exists (select b.GlobalID from edgis.zz_mv_generationinfo b where  a.sap_egi_notification=b.sapeginotification);
     ---ENOS SUpersede--
 UPDATE pgedata.gen_equipment_stage
 SET status    = 'E',   status_message = '117 Generator Equipment does not exist in EDGIS' , processedtime = sysdate
          WHERE 
             gen_tech_cd   IN ('PWR.PVPNL', 'PWR.BATT', 'PWR.WIND', 'PWR.GENRTR') AND Action='U'
             AND sap_equipment_id NOT IN
                 (SELECT sap_equipment_id
               FROM pgedata.pgedata_sm_gen_equipment_stage);
UPDATE pgedata.gen_equipment_stage
 SET status    = 'E',   status_message =  '116 Generator does not exist in EDGIS' , processedtime = sysdate
          WHERE 
             gen_tech_cd   IN ('GEN.SYNCH', 'GEN.INDCT', 'GEN.INVEXT', 'GEN.INVINC') AND Action='U'
             AND sap_equipment_id NOT IN
                 (SELECT sap_equipment_id
               FROM pgedata.pgedata_sm_generator_stage);
UPDATE pgedata.gen_equipment_stage
 SET status    = 'E',   status_message =  '117 Generator Equipment does not exist in EDGIS' , processedtime = sysdate
          WHERE 
             gen_tech_cd   IN ('PWR.PVPNL', 'PWR.BATT', 'PWR.WIND', 'PWR.GENRTR') AND Action='D'
             AND sap_equipment_id NOT IN
                 (SELECT sap_equipment_id
               FROM pgedata.pgedata_sm_gen_equipment_stage);
UPDATE pgedata.gen_equipment_stage
 SET status    = 'E',   status_message =  '116 Generator does not exist in EDGIS' , processedtime = sysdate 
          WHERE 
             gen_tech_cd   IN ('GEN.SYNCH', 'GEN.INDCT', 'GEN.INVEXT', 'GEN.INVINC') AND Action='D'
             AND sap_equipment_id NOT IN
                 (SELECT sap_equipment_id
               FROM pgedata.pgedata_sm_generator_stage);
UPDATE pgedata.gen_equipment_stage SET status    = 'W',   status_message = 'INVALID ACTION' , processedtime = sysdate
          WHERE 
             gen_tech_cd   IN ('PWR.PVPNL', 'PWR.BATT', 'PWR.WIND', 'PWR.GENRTR') 
             AND sap_equipment_id IN
                 (select sap_equipment_id from pgedata_sm_gen_equipment_stage group by sap_equipment_id having count(*)>1); 
 update pgedata.gen_equipment_stage a set GUID= (select GlobalID from edgis.zz_mv_generationinfo b where  b.sapeginotification = a.sap_egi_notification and rownum='1')
                 where DBMS_LOB.substr(status_message) = 'INVALID ACTION';          

UPDATE pgedata.gen_equipment_stage SET status    = 'W',   status_message ='INVALID ACTION' , processedtime = sysdate
          WHERE 
             gen_tech_cd   IN ('GEN.SYNCH', 'GEN.INDCT', 'GEN.INVEXT', 'GEN.INVINC') 
             AND sap_equipment_id IN
                 (select sap_equipment_id from pgedata_sm_generator_stage group by sap_equipment_id having count(*)>1); 
 update pgedata.gen_equipment_stage a set GUID= (select GlobalID from edgis.zz_mv_generationinfo b where  b.sapeginotification = a.sap_egi_notification and rownum='1')
                 where DBMS_LOB.substr(status_message) = 'INVALID ACTION'; 
update pgedata.gen_equipment_stage a set GUID= (select GlobalID from edgis.zz_mv_generationinfo b where  b.sapeginotification = a.sap_egi_notification and rownum='1')
                 where DBMS_LOB.substr(status_message) = '101 Failed Due to Invalid Data';               

 -- DELETE from pgedata.pgedata_sm_gen_equipment_stage where action is null;  
 --DELETE from pgedata.pgedata_sm_generator_stage where action is null;  

  DELETE from pgedata.pgedata_sm_gen_equipment_stage where sap_equipment_id IN
                 (select sap_equipment_id from pgedata_sm_gen_equipment_stage group by sap_equipment_id having count(*)>1); 
 DELETE from pgedata.pgedata_sm_generator_stage where sap_equipment_id IN
                 (select sap_equipment_id from pgedata.pgedata_sm_generator_stage group by sap_equipment_id having count(*)>1);
  DELETE from pgedata.pgedata_sm_gen_equipment_stage where sap_equipment_id IN
                 (select sap_equipment_id from pgedata.gen_equipment_stage WHERE 
             gen_tech_cd   IN ('PWR.PVPNL', 'PWR.BATT', 'PWR.WIND', 'PWR.GENRTR') and action='I' and DBMS_LOB.substr(status_message)='101 Failed Due to Invalid Data' );                 
 DELETE from pgedata.pgedata_sm_generator_stage where sap_equipment_id IN
                 (select sap_equipment_id from pgedata.gen_equipment_stage WHERE 
             gen_tech_cd   IN ('GEN.SYNCH', 'GEN.INDCT', 'GEN.INVEXT', 'GEN.INVINC') and action='I' and DBMS_LOB.substr(status_message)='101 Failed Due to Invalid Data');                
