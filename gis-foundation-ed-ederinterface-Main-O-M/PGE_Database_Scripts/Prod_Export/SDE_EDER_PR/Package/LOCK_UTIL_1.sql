--------------------------------------------------------
--  DDL for Package Body LOCK_UTIL
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "SDE"."LOCK_UTIL" 
/***********************************************************************
*
...
IS
      ...
      CURSOR locks_cursor IS
        ...
        WHERE ((state_id = state_lock.state_id AND
               ...
               ((lock_type = SDE.lock_util.C_exclusive_lock_all OR
                 ...
                 state_lock.lock_type != lock_type)) AND /* For Reliance. */
                lock_type <> SDE.lock_util.C_marked_lock;
  ...
  ***********************************************************************/
  ...
  PROCEDURE add_table_lock_list (sde_id      IN  pinfo_util.sde_id_t,
                                 lock_type   IN  lock_type_t,
                                 reg_id_list IN  table_id_list_t)                                  
  /***********************************************************************
  *
  *N  {add_table_lock_list}  --  Add locks to TABLE_LOCKS for a list of
  *                              registration ids, checking for conflicts.
  *
  *:::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
  *
  *P  Purpose:
  *     This procedure inserts new table locks into the TABLE_LOCKS table,
  *   for a list of registration ids, checking for conflicts.
  *E
  *:::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
  *
  *A  Parameters:
  *     sde_id     <IN>  == (pinfo_util.sde_id_t) The sde connection id.
  *     lock_type  <IN>  == (lock_type_t) The type of lock to add.
  *     table_lock <IN>  == (table_id_list_t) List of registration ids
  *                          for tables to add locks for.
  *E
  *:::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
  *
  *X  SDE Exceptions:
  ...
  *H  History:
  *
  *    Sanjay Magal                 08/2014           Original coding.
  *E
  ***********************************************************************/
   IS
   
     reg_id_index       BINARY_INTEGER;
     lock_rec           table_lock_t;
     
   BEGIN

       reg_id_index := reg_id_list.FIRST;
       
       WHILE reg_id_index IS NOT NULL LOOP
         
          lock_rec.sde_id := sde_id;
          lock_rec.lock_type := lock_type;
          lock_rec.registration_id := reg_id_list(reg_id_index);
          add_table_lock (lock_rec);
          reg_id_index := reg_id_list.NEXT(reg_id_index);
