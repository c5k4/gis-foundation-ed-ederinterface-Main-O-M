--------------------------------------------------------
--  DDL for Package Body SPX_UTIL
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "SDE"."SPX_UTIL" 
/***********************************************************************
*
...
IS
   ...
   g_current_user        SDE.st_type_user.a_identifier_t;
...
Return number
IS
 v_idx_id  PLS_INTEGER;  
 v_grid_rec SDE.st_geom_util.grid_info_t;
Begin
    ...
    FOR id IN 1 .. nlayers
      Loop
        If cursor_cache(id).table_name = in_table AND 
          ...
          spx_info_r.grid         := SDE.sp_grid_info(0,0,0);
          v_grid_rec := SDE.st_geom_util.lookup_grid_size(in_owner,in_table,in_spatial); 
          spx_info_r.grid.grid1   := v_grid_rec.g1;
          spx_info_r.grid.grid2   := v_grid_rec.g2;
          spx_info_r.grid.grid3   := v_grid_rec.g3;
          ...
          v_idx_id := SDE.st_geom_util.lookup_index_id(in_owner,in_table,in_spatial); 
          
          If cursor_cache(id).index_id <>  v_idx_id THEN
...
Procedure add_cache_info  (in_owner        IN varchar2,
  ...
  ***********************************************************************/
IS
    spatial_col SDE.st_type_user.a_identifier_t;
    b_add       boolean := False;
...
Begin
 ...
 Procedure close_partition_table_cursor  (curs  IN NUMBER)
  /***********************************************************************
  *
  *n  {close_partition_table_cursor}  --  closes a partition table cursor
  *
  *:::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
  *
  *p  purpose:
  *     this procedure closes a partition_table_cursor.
  *e
  *:::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
  *
  *a  parameters:
  *
  *     self <in > ==  (SDE.st_domain_methods) the partition table cursor.
  *e
  *:::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
  *
  *x   exceptions:
  ...
  *h  history:
  *
  *    Josefina Santiago         06/17/14           original coding.
  *e
  ***********************************************************************/
IS

is_cached_cursor boolean := FALSE;
...
Begin
    ...
    If nlayers > 0 THEN
      ...
      Loop
       If cursor_cache(id).ncurs_array > 0 THEN
         FOR pos IN 1 .. cursor_cache(id).ncurs_array
         Loop
             if cursor_cache(id).curs_array(pos).curs = SDE.spx_util.fetch_env(curs).curs Then
               is_cached_cursor := TRUE;
             end if;                            
         End Loop;
       End If;
      End Loop nlayers;
    ...
    If is_cached_cursor = FALSE Then
       dbms_sql.close_cursor(SDE.spx_util.fetch_env(curs).curs);
    End If;
         
 End close_partition_table_cursor;
    ...
    If srch_geom.srid != spx_info_r.srid Then
     ...
     shape_out.srid   := sp_ref_r.srid;
      ...
      SDE.st_geometry_shapelib_pkg.transform(geotranid,0,0,0,0,0,0,
                                             sp_ref2_r.srid,sp_ref2_r.x_offset,sp_ref2_r.y_offset,sp_ref2_r.xyunits,
...
Begin
    ...
    If optype = st_geom_operation_create THEN
      ...
      loc_spx_rec.index_id := 0;
      rc := SDE.st_type_util.get_geom_index_rec(owner,table_name,spatial_col,loc_spx_rec);
            
      If (loc_spx_rec.index_id > 0) then
        -- An existing record was found in ST_GEOMETRY_INDEX.
        -- Delete the record found. The record in ST_GEOMETRY_INDEX must 
        -- always reflect current index parameters.  
           
        spx_util.delete_index (loc_spx_rec.index_id); 
      End If;
...
IS
    rid             rowid;
    stmt            varchar2(512);
    spcol           SDE.st_type_user.a_identifier_t;
    nrows           integer;
    curs1           integer := 0;
    pos             pls_integer := 0;
    bcached         boolean := False;
...
IS
  ...
  Cursor c_geom_id(in_owner IN varchar2, in_table_name IN varchar2, in_column_name IN varchar2) is
     SELECT  geom_id
     FROM SDE.st_geometry_columns
     WHERE owner = in_owner AND table_name = in_table_name AND
      column_name = in_column_name;

  Cursor c_geom_idx(in_owner IN varchar2, in_table_name IN varchar2, in_column_name IN varchar2,
                      geom_id IN number) is
     SELECT  index_id
     FROM SDE.st_geometry_index
     WHERE owner = in_owner AND table_name = in_table_name AND
      column_name = in_column_name and index_id = geom_id;

  geom_id       NUMBER;
  NO_GEOM_MATCH EXCEPTION;
Begin
  Open c_geom_id(spx_info_r.owner,spx_info_r.table_name,spx_info_r.column_name);
  Fetch c_geom_id INTO geom_id;
  if c_geom_id%NOTFOUND THEN
    RAISE NO_GEOM_MATCH;
  End If;
  Close c_geom_id;

  Open c_geom_idx(spx_info_r.owner,spx_info_r.table_name,spx_info_r.column_name,geom_id);
  Fetch c_geom_idx INTO spx_info_r.index_id;
  if c_geom_idx%NOTFOUND THEN   
    spx_info_r.index_id := geom_id;
    INSERT INTO SDE.st_geometry_index
      (owner,table_name,column_name,grid,srid,commit_int,index_name,index_id)
    VALUES (spx_info_r.owner,spx_info_r.table_name,spx_info_r.column_name,
      ...
      spx_info_r.index_name,spx_info_r.index_id);
    COMMIT;
 End If;
    Close c_geom_idx;
  ...
  EXCEPTION
    ...
    WHEN NO_GEOM_MATCH THEN
      raise_application_error (-20002,'ST_GEOMETRY_COLUMNS '||spx_info_r.owner||'.'||spx_info_r.table_name||'.'||spx_info_r.column_name||' not found.');
      CLOSE c_geom_id;
...
Begin
  ...
  If srch_geom IS NULL Or (srch_geom.numpts = 0 AND srch_geom.len = 0) Then
    ...
    For i In grid_pos..high_pos Loop
     ...
     <<gx_loop>>
      ...
      If is_interior = TRUE And is_boundary = FALSE And 
        'ST_OVERLAPS' != operation And 'ST_TOUCHES' != operation  And 
        'ST_EQUALS' != operation And 'ST_CROSSES' != operation Then
...
IS
  res           integer;
  table_name    SDE.st_type_user.a_identifier_t;
  stmt          varchar2(1000);
Begin
...
Procedure calc_extent (ia           IN sys.odciindexinfo,
  ...
  ***********************************************************************/
  IS
    ...
    TYPE T_ptable IS TABLE OF SDE.st_type_user.a_identifier_t;
    ...
    s_table        SDE.st_type_user.a_identifier_t;
    ...
    spatial_column    SDE.st_type_user.a_identifier_t;
