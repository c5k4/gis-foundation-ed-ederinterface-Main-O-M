--------------------------------------------------------
--  DDL for Package Body ST_GEOM_UTIL
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "SDE"."ST_GEOM_UTIL" 
/***********************************************************************
*
...
Return PLS_INTEGER RESULT_CACHE RELIES_ON(SDE.ST_GEOMETRY_INDEX)
IS
 l_idx_id PLS_INTEGER := 0;

Begin
  Select index_id into l_idx_id
  from SDE.ST_GEOMETRY_INDEX 
  where owner     = owner_in
  and table_name  = table_in
  and column_name = column_in;
...
FUNCTION lookup_grid_size(
      ...
      table_in  IN SDE.ST_GEOMETRY_INDEX.table_name%type,
      column_in IN SDE.ST_GEOMETRY_INDEX.column_name%type)
 Return grid_info_t RESULT_CACHE RELIES_ON(SDE.ST_GEOMETRY_INDEX)
IS
 l_grid_rec grid_info_t;
...
Begin
  ...
  SELECT x.grid.grid1, x.grid.grid2, x.grid.grid3
  INTO l_grid_rec
  FROM SDE.ST_GEOMETRY_INDEX x
  WHERE owner     = owner_in
  AND table_name  = table_in
  AND column_name = column_in;
