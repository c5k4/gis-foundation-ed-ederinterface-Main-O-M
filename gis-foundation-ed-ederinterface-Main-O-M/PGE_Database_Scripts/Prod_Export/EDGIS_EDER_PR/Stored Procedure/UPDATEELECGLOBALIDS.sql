...
set define off;
  ...
  CREATE OR REPLACE EDITIONABLE PROCEDURE "EDGIS"."UPDATEELECGLOBALIDS" AS
--- get a list of feature classes used in the network for lines.
cursor net_fcids is
select OBJECTID,Replace(PHYSICALNAME,'.','.ZZ_MV_') AS PHYSICALNAME from sde.gdb_items where OBJECTID in (select distinct(to_feature_fcid) from EDGIS.PGE_Trace_Temp) AND PHYSICALNAME in (select OWNER||'.'||TABLE_NAME from sde.column_registry where column_name='PHASEDESIGNATION') ORDER BY PHYSICALNAME;
cursor net_fcids_noPhaseDesignation is
select OBJECTID,Replace(PHYSICALNAME,'.','.ZZ_MV_') AS PHYSICALNAME from sde.gdb_items where OBJECTID in (select distinct(to_feature_fcid) from EDGIS.PGE_Trace_Temp) AND PHYSICALNAME not in (select OWNER||'.'||TABLE_NAME from sde.column_registry where column_name='PHASEDESIGNATION') ORDER BY PHYSICALNAME;
cursor net_fcids_underground is
select OBJECTID,Replace(PHYSICALNAME,'.','.ZZ_MV_') AS PHYSICALNAME from sde.gdb_items where OBJECTID in (select distinct(to_feature_fcid) from EDGIS.PGE_UNDERGROUNDNETWORK_TEMP) ORDER BY PHYSICALNAME;
...
BEGIN
	...
	FOR CLASS_NAME in net_fcids_noPhaseDesignation LOOP
		DBMS_OUTPUT.put_line (SYSTIMESTAMP);
    IF CLASS_NAME.PHYSICALNAME ='EDGIS.ZZ_MV_ELECTRICDISTNETWORK_JUNCTIONS' THEN
          CLASS_NAME.PHYSICALNAME := 'EDGIS.ZZ_MV_ELECTRICDISTNETWORK_JUNC';
     END IF;
	...
	FOR CLASS_NAME in net_fcids_underground LOOP
		DBMS_OUTPUT.put_line (SYSTIMESTAMP);
    IF CLASS_NAME.PHYSICALNAME ='EDGIS.ZZ_MV_UNDERGROUNDNETWORK_JUNCTIONS' THEN
          CLASS_NAME.PHYSICALNAME := 'EDGIS.ZZ_MV_UNDERGROUNDNETWORK_JUNCT';
     END IF;
