--------------------------------------------------------
--  DDL for Procedure SEARCH_SAPNOTIFICATION
--------------------------------------------------------
set define off;

  CREATE OR REPLACE EDITIONABLE PROCEDURE "EDGIS"."SEARCH_SAPNOTIFICATION" (P_SEARCHTEXT IN VARCHAR2, P_DIVISION IN VARCHAR2, P_OPERATORS IN VARCHAR2, P_SAPNOTIFICATION_CURSOR OUT SYS_REFCURSOR, P_ALL_LAYERS_CURSOR OUT SYS_REFCURSOR)
AS
...
BEGIN
    ...
    if (UPPER(P_DIVISION) = 'ALL' OR P_DIVISION = '0' OR P_DIVISION IS NULL OR P_DIVISION = '') then
	  ...
	  v_table_name := 'PGE_SAPNOTIFICATION'; 
    v_query_type := 'SAPNOTIFICATION';
    ...
    v_query:= 'WITH T AS (' ||v_lookup_query|| ') select ' || SEARCH_SELFIELDS(v_column_name,'SUBTYPECD','T','m') || ' from  PGE_SAPNOTIFICATION m where  notificationnum '  || v_operator_query || ' and rownum<=500 ';
    v_query := REPLACE(v_query,'$COMMA$',','); 
    open P_SAPNOTIFICATION_CURSOR for v_query;  /* query executed */  
           
    v_query := 'select OBJCLS_NAME as "physicalName", LAYER_NAME as "layerName", displayfieldname as "displayFieldName", layerid as "layerId", database_name as "databaseName", LATESTWKID as "latestWkid", WKID as "wkid", GEOMETRYTYPE as "geometryType" from PGE_SEARCH_LAYERS_ALL where SEARCH_TYPE=''SAPNOTIFICATION'' order by CURSEQ';    
