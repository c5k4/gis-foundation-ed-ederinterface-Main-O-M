--------------------------------------------------------
--  DDL for Procedure TRACING_DISTRIBUTION
--------------------------------------------------------
set define off;

  CREATE OR REPLACE EDITIONABLE PROCEDURE "EDGIS"."TRACING_DISTRIBUTION" (P_TO_FEATURE_GLOBALID IN VARCHAR2, P_FEEDERID IN VARCHAR2, P_FEATURE_CLASS_ID IN VARCHAR2, P_TRACING_TYPE IN VARCHAR2, P_TRACING_ON IN VARCHAR2, P_TRACE_FEEDER_FED_FEEDERS IN BOOLEAN, P_TRACE_A_PHASE IN BOOLEAN, P_TRACE_B_PHASE IN BOOLEAN, P_TRACE_C_PHASE IN BOOLEAN, P_TRACING_CURSOR OUT SYS_REFCURSOR)
AS
  ...
  v_to_feature_fcid_2 edgis.pge_feederfednetwork_trace_vw.to_feature_fcid%TYPE;
...
--      dbms_output.put_line('v_execute when True');
      ...
      IF UPPER(P_TRACING_TYPE) = 'DOWNSTREAM' THEN
        ...
        IF UPPER(P_TRACING_ON) = 'DEVICE' THEN
          v_query := 'select a.ORDER_NUM,a.TO_FEATURE_FCID, a.TO_FEATURE_FEEDERINFO, b.physicalname, a.TO_FEATURE_OID,a.TO_FEATURE_GLOBALID,a.MIN_BRANCH,a.MAX_BRANCH,a.TREELEVEL,a.TO_CIRCUITID,a.FEEDERFEDBY,a.TO_LINE_GLOBALID,a.FEEDERID,a.TO_POINT_FC_GUID from ' || v_table1 || ' a left join sde.GDB_ITEMS b on a.to_feature_fcid = b.objectid where ' || v_sub_query;
        ELSIF UPPER(P_TRACING_ON) = 'PROTECTIVEDEVICE' THEN
          v_query := 'select a.ORDER_NUM,a.TO_FEATURE_FCID, a.TO_FEATURE_FEEDERINFO, b.physicalname, a.TO_FEATURE_OID,a.TO_FEATURE_GLOBALID,a.MIN_BRANCH,a.MAX_BRANCH,a.TREELEVEL,a.TO_CIRCUITID,a.FEEDERFEDBY,a.TO_LINE_GLOBALID,a.FEEDERID,a.TO_POINT_FC_GUID from ' || v_table1 || ' a left join sde.GDB_ITEMS b on a.to_feature_fcid = b.objectid where ' || v_sub_query || ' AND TO_FEATURE_FCID IN (997,998,1005,1003,144,143,140,132)';
        ELSE
          v_query := 'select a.ORDER_NUM,a.TO_FEATURE_FCID, b.physicalname,a.TO_FEATURE_OID,a.TO_FEATURE_GLOBALID,a.MIN_BRANCH,a.MAX_BRANCH,a.TREELEVEL,a.FEEDERFEDBY,a.FEEDERID from ' || v_table2 || ' a LEFT JOIN sde.GDB_ITEMS b ON a.to_feature_fcid = b.objectid where ' || v_sub_query || ' AND TO_FEATURE_FCID IN (1017,1018) AND SUBTYPECD IN (3)';
        END IF;
      ELSE
...
--          dbms_output.put_line('P_TRACE_FEEDER_FED_FEEDERS is False');
        ...
        END IF;
        IF UPPER(P_TRACING_ON) = 'DEVICE' THEN
          v_query := 'select a.ORDER_NUM,a.TO_FEATURE_FCID, a.TO_FEATURE_FEEDERINFO, b.physicalname, a.TO_FEATURE_OID,a.TO_FEATURE_GLOBALID,a.MIN_BRANCH,a.MAX_BRANCH,a.TREELEVEL,a.FROM_CIRCUITID,a.FEEDERFEDBY,a.FROM_LINE_GLOBALID,a.FEEDERID,a.FROM_POINT_FC_GUID from ' || v_table1 || ' a left join sde.GDB_ITEMS b on a.to_feature_fcid = b.objectid  where ' || v_sub_query;
        ELSIF UPPER(P_TRACING_ON) = 'PROTECTIVEDEVICE' THEN
          v_query := 'select a.ORDER_NUM,a.TO_FEATURE_FCID, a.TO_FEATURE_FEEDERINFO, b.physicalname, a.TO_FEATURE_OID,a.TO_FEATURE_GLOBALID,a.MIN_BRANCH,a.MAX_BRANCH,a.TREELEVEL,a.FROM_CIRCUITID,a.FEEDERFEDBY,a.FROM_LINE_GLOBALID,a.FEEDERID,a.FROM_POINT_FC_GUID from ' || v_table1 || ' a left join sde.GDB_ITEMS b on a.to_feature_fcid = b.objectid  where ' || v_sub_query || ' AND TO_FEATURE_FCID IN (997,998,1005,1003,144,143,140,132)';
        ELSE
          v_query := 'select a.ORDER_NUM,a.TO_FEATURE_FCID,b.physicalname,a.TO_FEATURE_OID,a.TO_FEATURE_GLOBALID,a.MIN_BRANCH,a.MAX_BRANCH,a.TREELEVEL,a.FEEDERFEDBY,a.FEEDERID from ' || v_table2 || ' a LEFT JOIN sde.GDB_ITEMS b ON a.to_feature_fcid = b.objectid where ' || v_sub_query || ' AND TO_FEATURE_FCID IN (1017,1018) AND SUBTYPECD IN (3)';
--          dbms_output.put_line(v_query);
  ...
  IF  UPPER(P_TRACING_ON) != 'CONDUIT' AND v_flag = 1 THEN
    ...
    v_counter2 := 0;
    IF UPPER(P_TRACING_TYPE) = 'DOWNSTREAM' THEN
      v_query_final := 'SELECT a.ORDER_NUM,a.TO_FEATURE_FCID, a.TO_FEATURE_FEEDERINFO, b.physicalname, a.TO_FEATURE_OID,a.TO_FEATURE_GLOBALID,a.MIN_BRANCH,a.MAX_BRANCH,a.TREELEVEL,a.TO_CIRCUITID,a.FEEDERFEDBY,a.TO_LINE_GLOBALID,a.FEEDERID,a.TO_POINT_FC_GUID from edgis.PGE_FEEDERFEDNETWORK_TRACE_VW a left join sde.GDB_ITEMS b on a.to_feature_fcid = b.objectid WHERE to_feature_globalid =' || chr(39)|| 'Just-for- column-header' || chr(39);
    ELSE
      v_query_final := 'SELECT a.ORDER_NUM,a.TO_FEATURE_FCID, a.TO_FEATURE_FEEDERINFO, b.physicalname, a.TO_FEATURE_OID,a.TO_FEATURE_GLOBALID,a.MIN_BRANCH,a.MAX_BRANCH,a.TREELEVEL,a.FROM_CIRCUITID,a.FEEDERFEDBY,a.FROM_LINE_GLOBALID,a.FEEDERID,a.FROM_POINT_FC_GUID from edgis.PGE_FEEDERFEDNETWORK_TRACE_VW a left join sde.GDB_ITEMS b on a.to_feature_fcid = b.objectid WHERE to_feature_globalid =' || chr(39)|| 'Just-for- column-header' || chr(39);
    END IF;
...
--        dbms_output.put_line('Execute is True');
        ...
        IF (P_TRACE_FEEDER_FED_FEEDERS) THEN
          IF v_from_line_globalid != null OR v_to_line_globalid != null THEN
            ...
            IF UPPER(P_TRACING_TYPE) = 'DOWNSTREAM' THEN
               v_query2 := 'SELECT a.ORDER_NUM,a.TO_FEATURE_FCID,a.TO_FEATURE_FEEDERINFO,b.physicalname, a.TO_FEATURE_OID,a.TO_FEATURE_GLOBALID,a.MIN_BRANCH,a.MAX_BRANCH,a.TREELEVEL,a.TO_CIRCUITID,a.FEEDERFEDBY,a.TO_LINE_GLOBALID,a.FEEDERID,a.TO_POINT_FC_GUID FROM edgis.PGE_FEEDERFEDNETWORK_TRACE_VW a LEFT JOIN sde.GDB_ITEMS b ON a.to_feature_fcid = b.objectid WHERE TO_FEATURE_GLOBALID = v_to_point_fc_guid AND FEEDERID = v_to_circuitid';
            ELSE
               v_query2 := 'SELECT a.ORDER_NUM,a.TO_FEATURE_FCID,a.TO_FEATURE_FEEDERINFO,b.physicalname, a.TO_FEATURE_OID,a.TO_FEATURE_GLOBALID,a.MIN_BRANCH,a.MAX_BRANCH,a.TREELEVEL,a.TO_CIRCUITID,a.FEEDERFEDBY,a.TO_LINE_GLOBALID,a.FEEDERID,a.TO_POINT_FC_GUID FROM edgis.PGE_FEEDERFEDNETWORK_TRACE_VW a LEFT JOIN sde.GDB_ITEMS b ON a.to_feature_fcid = b.objectid WHERE TO_FEATURE_GLOBALID = v_from_line_globalid AND FEEDERID = v_from_circuitid';        
            END IF;
...
--    OPEN P_TRACING_CURSOR FOR v_query_final;
  ...
  WHEN NULL_STRING THEN
    v_final_query := 'select a.ORDER_NUM,a.TO_FEATURE_FCID,a.TO_FEATURE_FEEDERINFO,b.physicalname, a.TO_FEATURE_OID,a.TO_FEATURE_GLOBALID,a.MIN_BRANCH,a.MAX_BRANCH,a.TREELEVEL,a.TO_CIRCUITID,a.FEEDERFEDBY,a.TO_LINE_GLOBALID,a.FEEDERID,a.TO_POINT_FC_GUID from edgis.PGE_FEEDERFEDNETWORK_TRACE_VW a left join sde.GDB_ITEMS b on a.to_feature_fcid = b.objectid where TO_FEATURE_FCID IN (0)';
