...
set define off;
  ...
  CREATE OR REPLACE EDITIONABLE PROCEDURE "UC4ADMIN"."PROC_REBUILD_INDEXS" (SchemaName IN NVARCHAR2, FreshExecution in BOOLEAN)
AS
...
IF FreshExecution = TRUE THEN
    ...
    FETCH CUR_INDEXES INTO lv_idx_name;
        ...
        BEGIN        
            ...
            dbms_output.put_line('Success:'||'ALTER INDEX ' ||  PROC_REBUILD_INDEXS.SchemaName || '.' || lv_idx_name || ' REBUILD ONLINE parallel (degree 8) NOLOGGING');
            EXCEPTION
            WHEN OTHERS THEN           
            BEGIN            
                dbms_output.put_line('Failed:'||'ALTER INDEX ' ||  PROC_REBUILD_INDEXS.SchemaName || '.' || lv_idx_name || ' REBUILD ONLINE parallel (degree 8) NOLOGGING');
...
ELSE
    ...
    FETCH CUR_INDEXES2 INTO lv_idx_name2;
        ...
        BEGIN        
            ...
            EXECUTE IMMEDIATE 'ALTER INDEX ' ||  PROC_REBUILD_INDEXS.SchemaName || '.' || lv_idx_name2 || ' REBUILD ONLINE parallel (degree 8) NOLOGGING';        
            DELETE FROM UC4ADMIN.FAILED_INDEXES WHERE INDEX_NAME = lv_idx_name2 AND OWNER = PROC_REBUILD_INDEXS.SchemaName;        
            dbms_output.put_line('Success:'||'ALTER INDEX ' ||  PROC_REBUILD_INDEXS.SchemaName || '.' || lv_idx_name2 || ' REBUILD ONLINE parallel (degree 8) NOLOGGING');
            COMMIT;  
            EXCEPTION
            WHEN OTHERS THEN           
            BEGIN
                dbms_output.put_line('Failed:'||'ALTER INDEX ' ||  PROC_REBUILD_INDEXS.SchemaName || '.' || lv_idx_name2 || ' REBUILD ONLINE parallel (degree 8) NOLOGGING');
