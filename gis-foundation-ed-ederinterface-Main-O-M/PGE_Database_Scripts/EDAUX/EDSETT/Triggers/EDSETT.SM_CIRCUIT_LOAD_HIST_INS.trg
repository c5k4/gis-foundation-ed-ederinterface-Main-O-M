Prompt drop Trigger SM_CIRCUIT_LOAD_HIST_INS;
DROP TRIGGER EDSETT.SM_CIRCUIT_LOAD_HIST_INS
/

Prompt Trigger SM_CIRCUIT_LOAD_HIST_INS;
--
-- SM_CIRCUIT_LOAD_HIST_INS  (Trigger) 
--
CREATE OR REPLACE TRIGGER EDSETT.SM_CIRCUIT_LOAD_HIST_INS
BEFORE INSERT ON EDSETT.SM_CIRCUIT_LOAD_HIST
FOR EACH ROW
BEGIN
  <<COLUMN_SEQUENCES>>
  ...
  BEGIN
    IF INSERTING AND :NEW.ID IS NULL THEN
      SELECT SM_CIRCUIT_LOAD_HIST_SEQ.NEXTVAL INTO :NEW.ID FROM SYS.DUAL;
    END IF;
