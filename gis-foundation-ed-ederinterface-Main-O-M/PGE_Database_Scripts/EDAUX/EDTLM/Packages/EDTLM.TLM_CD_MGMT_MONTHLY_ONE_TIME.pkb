Prompt drop Package Body TLM_CD_MGMT_MONTHLY_ONE_TIME;
DROP PACKAGE BODY EDTLM.TLM_CD_MGMT_MONTHLY_ONE_TIME
/

Prompt Package Body TLM_CD_MGMT_MONTHLY_ONE_TIME;
--
-- TLM_CD_MGMT_MONTHLY_ONE_TIME  (Package Body) 
--
CREATE OR REPLACE PACKAGE BODY EDTLM."TLM_CD_MGMT_MONTHLY_ONE_TIME"
AS
  AppType              CHAR(1);
...
BEGIN
  ...
  DECLARE
    ...
    IS
      ...
      ORDER BY Id;
  BEGIN
   -- TLM Changeset# - (ONLY FOR ONE TIME ACTIVITY) mark records in TRANSFORMER base table as deleted if these are not present in CD_TRANSFORMER_TEMP table
    DBMS_OUTPUT.put_line('Marking records in TRANSFORMER as deleted if these ae not present in CD_TRANSFORMER_temp table');
    UPDATE TRANSFORMER SET REC_STATUS = 'D' WHERE global_id NOT IN (SELECT global_id FROM cd_transformer_temp);
    COMMIT;
        ...
        IF cd_trf_rec.trans_type='U' OR cd_trf_rec.trans_type='D' THEN
          ...
          WHEN NO_DATA_FOUND THEN
            ...
            ELSE -- Delete
              ...
              UPDATE CD_TRANSFORMER_TEMP T SET PROC_FLG='P' WHERE T.ID= cd_trf_rec.ID;
        ...
        IF v_trans_type = 'I' THEN
          ...
          INTO TRANSFORMER
            ...
            (
              ...
              SUBSTR(TO_CHAR(cd_trf_rec.COAST_INTERIOR_FLG),1,1),
...
BEGIN
  ...
  -- TLM Changeset#1 Change Trans_date format from YYYY/MM/DD HH:MI:SS.FF AM to YYYY-MM-DD HH24:MI:SS throughout
    ...
    LOOP
      BEGIN
        ...
        IF cd_trf_bank_rec.trans_type='U' OR cd_trf_bank_rec.trans_type='D' THEN
          ...
          WHEN NO_DATA_FOUND THEN
            ...
            ELSE -- Delete
              ...
              UPDATE CD_TRANSFORMER_BANK_TEMP T
              SET PROC_FLG='P'
        ...
        IF SQL%ROWCOUNT>0 THEN
          ...
          UPDATE CD_TRANSFORMER_BANK_TEMP B
          SET B.PROC_FLG='P'
    ...
    END LOOP;
    -- TLM Changeset#10 - (ONLY FOR ONE TIME ACTIVITY) mark records in TRANSFORMER_BANK base table as deleted if these are not present in CD_TRANSFORMER_BANK_TEMP table
    DBMS_OUTPUT.put_line('Marking records in TRANSFORMER_BANK as deleted if these are not present in CD_TRANSFORMER_BANK_TEMP table');
    UPDATE TRANSFORMER_BANK SET REC_STATUS = 'D' WHERE global_id NOT IN
      (SELECT global_id FROM cd_transformer_bank_temp);
    COMMIT;
...
BEGIN
  ...
  -- TLM Changeset#1 Change Trans_date format from YYYY/MM/DD HH:MI:SS.FF AM to YYYY-MM-DD HH24:MI:SS throughout
    ...
    LOOP
      ...
      /*  END IF; */
        IF cd_meter_rec.trans_type='U' OR cd_meter_rec.trans_type='D' THEN
          ...
          WHEN NO_DATA_FOUND THEN
            ...
            --  END IF;
            ELSE -- Delete
              UPDATE CD_METER_TEMP SET PROC_FLG='P' WHERE ID= cd_meter_rec.ID;
      ...
      dbms_output.put_line('Done');
    END LOOP;
    -- TLM Changeset#9 - (ONLY FOR ONE TIME ACTIVITY) mark records in METER base table as deleted if these are not present in CD_METER_TEMP table
    DBMS_OUTPUT.put_line('Marking records in Meter as deleted if these are not present in CD_METER_TEMP table');
    UPDATE METER SET REC_STATUS = 'D'
    WHERE global_id NOT IN (SELECT global_id FROM cd_meter_TEMP);
    COMMIT;
    dbms_output.put_line('Commit DONE');
...
BEGIN
  IF trans_type<>'D' THEN
    IF(val IS NULL)
      ...
      LOG_ERRORS(global_id,error_code,ErrorMsg,to_timestamp(NVL(trans_date,sysdate),'YYYY-MM-DD HH24:MI:SS'), trans_type,app_type,procedure_for,rec_id);
...
END TLM_CD_MGMT_MONTHLY_ONE_TIME;
/
