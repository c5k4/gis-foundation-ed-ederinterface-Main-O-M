Prompt drop Trigger TRF_BANK_PEAK_INS;
DROP TRIGGER EDTLM.TRF_BANK_PEAK_INS
/

Prompt Trigger TRF_BANK_PEAK_INS;
--
-- TRF_BANK_PEAK_INS  (Trigger) 
--
CREATE OR REPLACE TRIGGER EDTLM.TRF_BANK_PEAK_INS BEFORE
  INSERT ON EDTLM.TRF_BANK_PEAK FOR EACH ROW
BEGIN <<COLUMN_SEQUENCES>>
    DECLARE clid VARCHAR2(30);
  BEGIN
    IF INSERTING AND :NEW.ID IS NULL THEN
      SELECT EDTLM.TRF_BANK_PEAK_SEQ.NEXTVAL INTO :NEW.ID FROM SYS.DUAL;
    END IF;
